...
// opensource plugin = tieroneplan; premium plugin = tiertwoplan or tierthreeplan or enterpriseplan
:pluginminimumplan: <tier>
// <plugincode>: adds the plugin to an editor configuration. Reference on this page with the {plugincode} variable
...
//  If the menu items are the same as another plugin,
//  assign the other plugin’s identifier to `altplugincode`
//  [what does the above mean, specifically?]
   :pluginpage: <plugincode.adoc> // not all extant entries have this. ??
2. add a file, /partials/menu-item-ids/<plugincode>-menu-items.adoc
   a. document the plugin’s menu items herein.

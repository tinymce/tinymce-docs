...
The *quickbars_selection_toolbar* option configures the Quick Selection toolbar provided by the link:{baseurl}/plugins/quickbars[quickbars plugin]. To change the buttons on the Quick Selection toolbar, provide a space-separated string of link:{baseurl}/advanced/editor-control-identifiers/#toolbarcontrols[toolbar button names]. The Quick Selection toolbar is intended for buttons related to formatting content, but any link:{baseurl}/advanced/editor-control-identifiers/#toolbarcontrols[{productname} toolbar buttons] or link:{baseurl}/ui-components/toolbarbuttons[custom toolbar buttons] are allowed.

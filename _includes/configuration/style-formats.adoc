...
* *Style:* The item must have a `title` and either a `format` with refers to a pre-registered editor link:{{ site.baseurl }}/configure/content-formatting/#formats[format] or the necessary fields to link:{{ site.baseurl }}/configure/content-formatting/#formattype[specify a new format]. The item will be rendered in the UI as a clickable item that applies the given format.
...
The link:{{ site.baseurl }}/demo/format-custom/[Custom formats] demo contains examples of custom style formats, including how to specify new formats and group headings.

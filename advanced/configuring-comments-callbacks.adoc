...
*Callback mode* is the default mode for link:{{site.baseurl}}/plugins/comments/comments_2.0/[Comments 2.0]. In the callback mode, the user needs to configure storage to be able to save comments on the server. The Comments functions (create, reply, edit, delete comment, delete all conversations, and lookup) are configured differently depending upon the server-side storage configuration.
...
For more information on Comments commercial feature, visit our link:{{ site.baseurl }}/enterprise/tiny-comments/[Premium Features] page.

...
Another point is `setup` callback - see how we put all of our code into it? It is the callback that TinyMCE will link:{{ site.baseurl }}/configure/integration-and-setup/#setup[automatically invoke] for every initialised editor instance. It will receive a reference to the instance as the first argument. We can use `setup` callback to customize editor to our taste - here we are adding a button, but we could also add custom keyboard shortcuts, menus and everything that has to be added _before_ the editor is ready.
...
* *cmd* - _editor command to invoke, when button is clicked (command link:{{ site.baseurl }}/api/tinymce/tinymce.editorcommands/#addcommands[should be registered] prior to this, either by editor or by you)_
...
So basically we need a way to monitor the cursor position and disable our button when it's inappropriate. For this purpose, we can hook onto a link:{{ site.baseurl }}/advanced/events/#nodechange[NodeChange event], that gets fired when cursor jumps from one node to another.
...
By the way, notice how the code for our example gets bigger and bigger. It has almost reached the boundaries of simplicity already. This is the moment when you should ask yourself whether it's better to link:{{ site.baseurl }}/advanced/creating-a-plugin/[bundle this feature as a plugin] instead.

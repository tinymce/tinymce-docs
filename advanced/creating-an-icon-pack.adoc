...
The icon pack name will be used with the link:{baseurl}/configure/editor-appearance/#icons[icons] option to apply the icons in {productname}.
...
{productname} does not resize the SVGs provided, relying on the size defined in the SVG. This allows icons of different sizes to be used in the editor. The Toolbar button sizes are independent of the icon sizes. To change button sizes, a link:{baseurl}/advanced/creating-a-skin/[custom skin] is required.

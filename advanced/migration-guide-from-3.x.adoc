...
These are the new plugins in 4.0: link:{{ site.baseurl }}/plugins/anchor[anchor], link:{{ site.baseurl }}/plugins/charmap[charmap], link:{{ site.baseurl }}/plugins/compat3x[compat3x], link:{{ site.baseurl }}/plugins/hr[hr], link:{{ site.baseurl }}/plugins/image[image], link:{{ site.baseurl }}/plugins/link[link], link:{{ site.baseurl }}/plugins/emoticons[emoticons], link:{{ site.baseurl }}/plugins/code[code] and link:{{ site.baseurl }}/plugins/textcolor[textcolor].
...
TinyMCE produces HTML5 loose output as of 4.0. This means it can handle all new HTML5 elements and also allow the deprecated HTML4 attributes and elements. If you want to use html4 or html5-strict output check the link:{{ site.baseurl }}/configure/content-filtering/#schema[schema] option for details.

== Using the compat3x plugin

As a last resort one can use the link:{{ site.baseurl }}/plugins/compat3x[compat3x plugin] it mimics parts of the 3.x API contains a few files removed from 4.x. We only recommend this as a last resort, and it won't work for all plugins. But most plugins should work right out of the box we tested it with some of our old 3.x plugins.

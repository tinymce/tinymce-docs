...
* Added the `origin` property to the `ObjectResized` and `ObjectResizeStart` events, to specify which handle the resize was preformed on.
...
* Added a new `contextmenu_avoid_overlap` setting to allow contextmenus to avoid overlapping matched nodes.
...
* Changed the editor to cleanup loaded CSS stylesheets when all editors using the stylesheet have been removed.
...
* Changed the default table styles in the content CSS files to better support the styling options available in the tables dialog.
...
* Fixed pressing escape not focusing the editor when using multiple toolbars.
...
* Fixed an exception being thrown when pressing enter inside pre elements while `br_in_pre` setting is false.

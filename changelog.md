...
* Fixed an issue where Backspace/Delete keys after/before page break elements wouldn't move the caret.
...
* Updated the `textpattern` plugin to support nested patterns properly and to allow running a command with a value for a pattern with a start and an end.

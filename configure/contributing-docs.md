...
Simply branch the [docs GitHub repository](https://github.com/tinymce/tinymce-docs) and submit a PR, or reach out to the [{{site.productname}} docs team](mailto:marketing@tiny.cloud) if you have a specific question.
...
Use the capitalized version of {{site.productname}} when referring to the open source project or the editor. Use lowercase when referring to the `tinymce` JavaScript object.
...
The exception to this rule is when we are talking about the code itself (see the note above about {{site.productname}} v. tinymce). For example, write "HTML is great" when talking about the HTML language, but write, "escape your `html`" when talking about action with or within a codebase.

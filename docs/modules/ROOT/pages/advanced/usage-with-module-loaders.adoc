...
A *skin* consists of some fonts and CSS files used by the editor and {productname} will not work without them. By default {productname} looks for these files in a `/skins` directory located in your root directory. This can  be link:{modulesDir}/configure/editor-appearance/#skin_url[configured in the init object].

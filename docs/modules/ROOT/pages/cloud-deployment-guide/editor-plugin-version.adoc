...
Use the URL provided to specify the {productname} version when deploying via {cloudname}. Refer to the link:{modulesDir}/cloud-deployment-guide/editor-and-features[{productname} editor via the {cloudname}] for more information.
...
This channel deploys the absolute latest version of {productname} as documented in link:{modulesDir}/changelog/[{productname} changelog]. The current version of {productname} available through the `dev` channel https://cdn.tiny.cloud/1/no-api-key/tinymce/5-dev/version.txt[can be found here].
...
Use the URL query parameters to specify the version of each premium plugin. This is used when deployment is through the link:{modulesDir}/cloud-deployment-guide/editor-and-features[{productname} editor and premium plugins via {cloudname}] or deploying link:{modulesDir}/cloud-deployment-guide/features-only[only premium plugins from {cloudname}].

[discrete#mentions]
==== Mentions

* link:{modulesDir}/plugins/mentions/[Developer documentation]
...
* link:{modulesDir}/plugins/drive/[Developer documentation]
...
* link:{modulesDir}/plugins/comments/[Developer documentation]
...
* link:{modulesDir}/plugins/pageembed/[Developer documentation]
...
* link:{modulesDir}/plugins/permanentpen/[Developer documentation]
...
* link:{modulesDir}/plugins/formatpainter/[Developer documentation]
...
* link:{modulesDir}/plugins/powerpaste[Developer documentation]
...
* link:{modulesDir}/plugins/tinymcespellchecker[Developer documentation]
...
* link:{modulesDir}/plugins/a11ychecker[Developer documentation]
...
* link:{modulesDir}/plugins/advcode/[Developer documentation]
...
* link:{modulesDir}/plugins/mediaembed/[Developer documentation]
...
* link:{modulesDir}/plugins/linkchecker/[Developer documentation]
...
// Additional configuration is required when serving [only premium plugins from {cloudname}]({modulesDir}/cloud-deployment-guide/features-only) and from a self-hosted installation. URL query parameters and the special version name "SDK" can also be used in the configuration.

If you're deploying link:{modulesDir}/cloud-deployment-guide/features-only[only premium plugins from {cloudname}], you may want to have some features served from {cloudname} and some features served from your self-hosted installation. This is also possible with URL query parameters and the special version name "SDK".

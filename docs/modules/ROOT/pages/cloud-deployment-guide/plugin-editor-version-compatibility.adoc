...
Use the following matrix in conjunction with the optional parameters for specifying editor version and link:{modulesDir}/cloud-deployment-guide/editor-plugin-version[plugin version] to ensure compatibility.
...
IMPORTANT: Ensure the plugins are supported in your editor version when deploying premium plugins only using link:{modulesDir}/cloud-deployment-guide/features-only[{cloudname}].

...
The link:{modulesDir}/plugins/linkchecker/[Link Checker plugin] provides a link checking service right within the {productname} editor.
...
Your have two deployment options. One is super easy with {cloudname}, the other is to go the more traditional Self-hosted route, requiring an additional link:{modulesDir}/enterprise/server/[server-side component] to be installed and configured.
...
For information on configuring the server-side component, see: link:{modulesDir}/enterprise/server/[Configure server-side components].

You will also find more Self-hosted setup information on the link:{modulesDir}/plugins/linkchecker/[Link Checker plugin page].

[#link-checker-cloud-setup]
== Link Checker cloud setup

link:{modulesDir}/cloud-deployment-guide/editor-and-features/[{cloudname}] makes setting up Link Checker a breeze. Simply include the `linkchecker` parameter in your `tinymce.init` and have the {cloudname} services do the work for you.

...
. The `application.conf` file is incorrect. Please go back and follow the steps listed in the link:{rootDir}enterprise/server.html#step3createaconfigurationfile[installation guide]. This is the most common problem - often the origins are specified without the port numbers and this can cause things to fail, eg: use `+http://localhost:8080+` instead of `+http://localhost+`. After making changes to the `application.conf` file, please restart your Java web server (e.g. Jetty or Tomcat).
...
. The `application.conf` file is correct, and the services are working, but the server-side component URLs that the editor uses are not quite right. Refer to the link:{rootDir}plugins/tinymcespellchecker.html[Spell Checker Pro], link:{rootDir}plugins/imagetools.html[Image Tools], link:{rootDir}plugins/mediaembed.html[Enhanced Media Embed] and link:{rootDir}plugins/linkchecker[Link Checker] plugin pages for help.
. The browser is sending a different origin than expected (and configured in `application.conf`). Refer to step 6 of link:{rootDir}enterprise/server/troubleshoot.html#usingbrowsertoolingtoinvestigateservicesissues[Using browser tooling to investigate services issues]
...
image::{rootDir}/images/spell-checking-browser-tools.png[Using browser tools to investigate services issues]
...
image::{rootDir}/images/spell-checking-server_status.png[Spell Checking Server Status]
...
image::{rootDir}/images/spell-checking-jetty-settings.png[Spell Checking Server Settings on Jetty]
...
+
image::{rootDir}/images/spell-checking-curl-windows-1.png[Curl on Windows step 1]

. Copy the path of the folder to where the 'curl.exe' is in:
+
image::{rootDir}/images/spell-checking-curl-windows-2.png[Curl on Windows step 2]
...
+
image::{rootDir}/images/spell-checking-curl-windows-3.png[Curl on Windows step 3]

. Once in the folder enter 'curl --version' (without quotes) and ensure you get a valid version
+
image::{rootDir}/images/spell-checking-curl-windows-4.png[Curl on Windows step 4]

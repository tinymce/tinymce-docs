...
Local images that are added through other means are also uploaded using this function, such as images added by drag and drop when using the link:{modulesDir}/plugins/paste/#paste_data_images[`paste_data_images`] configuration property, or using the {companyname} link:{modulesDir}/plugins/powerpaste/[*PowerPaste* plugin].
...
An example PHP upload handler implementation is available link:{modulesDir}/advanced/php-upload-handler/[here].
...
The link:{modulesDir}/advanced/php-upload-handler/[PHP Upload Handler Script] provided here configures CORS in the `$accepted_origins` variable. Configure CORS at the http://www.w3.org/wiki/CORS_Enabled#At_the_Web_Application_level...[web application layer] or the http://www.w3.org/wiki/CORS_Enabled#At_the_HTTP_Server_level...[HTTP server layer].

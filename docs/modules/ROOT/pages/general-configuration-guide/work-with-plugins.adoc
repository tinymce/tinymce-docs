...
For most developers, the real power of {productname}'s functionality is found in its plugins. link:{modulesDir}/plugins/[Plugins] either extend default editor functionality or add new functionality. For example, the link:{modulesDir}/plugins/advlist/[Advanced List] plugin adds extra options to the toolbar's existing list controls, while the link:{modulesDir}/plugins/code/[Code] plugin adds entirely new functionality.
...
The link:{modulesDir}/plugins/code/[Code] plugin is one of the more popular plugins because it can expose the editable area's raw `html` to the user. It also provides an excellent example because it adds a menu item and toolbar button as well as having advanced customization options.
...
The link:{modulesDir}/plugins/advlist/[Advanced List] plugin extends the default unordered and ordered list toolbar controls by adding CSS `list-style-type` styled number formats and bullet types to the controls.
...
Don't worry if you can't remember all these values. The link:{modulesDir}/plugins/[plugins section] goes into great detail explaining all the plugins options, toolbars, and menu controls.
...
As we noted at the start {productname} is an incredibly powerful, flexible, and customizable rich text editor. Once you have finished the Get Started guide, deep dive into the link:{modulesDir}/plugins/[plugins] and link:{modulesDir}/configure/[configuration options].
...
| *7*
| Advanced Code
| link:{modulesDir}/plugins/advcode/[advcode]

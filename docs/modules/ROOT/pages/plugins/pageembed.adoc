...
*Result*: The image:{imagesDir}pageembed.png[**Page Embed**](*Page Embed*) button appears in the toolbar menu.
...
. Click on the image:{imagesDir}pageembed.png[**Page Embed**](*Page Embed*) toolbar button.
. The *InsertEdit Iframe* dialog box appears.
image:{imagesDir}insert-iframes.png[InsertEdit Iframe]
. In the *General* tab, enter the url in the *Source* field, or click on the image:{imagesDir}source.png[Source](*Source*) icon to upload a file.
...
* *Inline Value* is used to insert an iframe within a line. To set the dimensions of an inline iframe, provide the values in pixels in the *Width* and *Height* input box.
     * The default values are *Width* `350px` and *Height* `260px`. Click the image:{imagesDir}constrainprop.png[Constrain Proportions](*Constrain Proportions*) lock icon to change the default values.
...
* Click on the image:{imagesDir}source.png[Source](*Long description URL*) icon to upload a description file.

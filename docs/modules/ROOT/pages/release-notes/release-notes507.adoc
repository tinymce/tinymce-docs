...
For more information on the new `help_tabs` setting and API refer to the link:{modulesDir}/plugins/help/[documentation].
...
For more information on `imagetools_fetch_image` refer to the link:{modulesDir}/plugins/imagetools/#imagetools_fetch_image[documentation].
...
For more information on new toolbar button and menu item refer to the link:{modulesDir}/plugins/table/#table_grid[documentation].
...
For more information and demos on the new *Premium Skins and Icon Packs* refer to the full link:{modulesDir}/enterprise/premium-skins-and-icon-packs/[documentation].

[#tiny-drive]
=== Tiny Drive

*Tiny Drive* 1.3.0 comes with significant changes to the previous versions. Improved solutions have been introduced for unnecessary and obsolete configuration options. A comprehensive list of the updates and fixed issues is available in the link:{modulesDir}/tinydrive/changelog/[changelog].
...
For more information on *Tiny Drive* refer to the full link:{modulesDir}/tinydrive/[documentation].

There is also a demo provided to explore the *Tiny Drive* capabilities link:{modulesDir}/tinydrive/introduction/#demo[here].
...
For more information on *Enhanced Media Embed* refer to the full link:{modulesDir}/plugins/mediaembed/#mediaembed_inline_styles[documentation].
...
The new TinyMCE 5.0.7 editor comes with significant changes to the previous versions. Improved solutions have been introduced for unnecessary and obsolete configuration options. A comprehensive list of the updates and fixed issues is available in the link:{modulesDir}/changelog/#version507june52019[changelog].

[#autocompleter]
==== Autocompleter

The autocompleter functionality builtin to TinyMCE and used by the link:{modulesDir}/plugins/mentions/[*Mentions*], link:{modulesDir}/plugins/charmap/[*Charmap*] and link:{modulesDir}/plugins/emoticons/[*Emoticons*] plugins, has been improved to highlight matched text results, search across fragmented text nodes and support matching results that contain one or more spaces.

For more information on the autocompleter refer to the link:{modulesDir}/ui-components/autocompleter/[documentation].
...
For more information on the configuration options for `link_assume_external_targets` refer to the link:{modulesDir}/plugins/link/#link_assume_external_targets[documentation].
...
Using the cloud hosted premium content skins, in combination with the link:{modulesDir}/plugins/preview/[preview] plugin, will cause nothing to render in the preview dialog. This is because the `Origin` header sent to the server is `null`, due to how the preview dialog is implemented in TinyMCE. This will be fixed in the next release of TinyMCE.

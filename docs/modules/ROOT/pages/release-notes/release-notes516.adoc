...
* The `image` plugin now respects the link:{modulesDir}/configure/file-image-upload/#automatic_uploads[`automatic_uploads`] setting when uploading local images.
...
* The link:{modulesDir}/plugins/mediaembed/#mediaembed_max_width[`mediaembed_max_width`] setting will now constrain the max width correctly on Internet Explorer 11.

...
For information on using the `toolbar_location` setting, see: link:{modulesDir}/configure/editor-appearance/#toolbar_location[User interface options - `toolbar_location`].
...
For information on the `toolbar_mode` option, see: link:{modulesDir}/configure/editor-appearance/#toolbar_mode[User interface options - `toolbar_mode`].
...
image::{modulesDir}/images/toolbar-group-example.png[**Group toolbar button example**]

For information on using the `toolbar_groups` setting, see: link:{modulesDir}/configure/editor-appearance/#toolbar_groups[User interface options - `toolbar_groups`].
For information on using the `addGroupToolbarButton` API, see: link:{modulesDir}/ui-components/typesoftoolbarbuttons/#grouptoolbarbutton[Types of toolbar buttons - Group button].
...
For a list of the available user interface languages, see: link:{modulesDir}/configure/localization/#language[Localization options - `language`].
...
For information on using the `placeholder` setting, see: link:{modulesDir}/configure/editor-appearance/#placeholder[User interface options - `placeholder`].
...
* link:{modulesDir}/plugins/a11ychecker/#a11y_advanced_options[The Accessibility Checker plugin - `a11y_advanced_options`].
* link:{modulesDir}/plugins/image/#a11y_advanced_options[The Image plugin - `a11y_advanced_options`].
...
For information on using the `quickbars_image_toolbar` setting, see: link:{modulesDir}/plugins/quickbars/#quickbars_image_toolbar[Quick Toolbars plugin - `quickbars_image_toolbar`].
...
For information on using the `codesample_global_prismjs` setting, see: link:{modulesDir}/plugins/codesample/#codesample_global_prismjs[Code sample plugin - `codesample_global_prismjs`].
...
For information on using the `TextSeeker` API, see: link:{modulesDir}/api/tinymce.dom/tinymce.dom.textseeker/[{productname} APIs - tinymce.dom.TextSeeker].
...
For an updated list of {productname} icons, see: link:{modulesDir}/advanced/editor-icon-identifiers/[Editor icon identifiers].
...
* For information on using this setting with the Autolink plugin, see: link:{modulesDir}/plugins/autolink/#link_default_protocol[Autolink plugin - `link_default_protocol`].
* For information on using this setting with the Link plugin, see: link:{modulesDir}/plugins/link/#link_default_protocol[Link plugin - `link_default_protocol`].
...
For information on the `a11y_advanced_options` setting, see: link:{modulesDir}/plugins/a11ychecker/#a11y_advanced_options[Accessibility Checker plugin].
...
For information on the checklist plugin, see: link:{modulesDir}/plugins/checklist/[Checklist plugin].
...
For information on the `linkchecker_preprocess` option, see: link:{modulesDir}/plugins/linkchecker/#linkchecker_preprocess[Link Checker plugin - `linkchecker_preprocess`].
...
For information on inlining page embed styles, see: link:{modulesDir}/plugins/pageembed/#inlinestylesforpageembeds[Page Embed plugin - Inline styles for page embeds].
...
*PowerPaste* 5.2.1 provides a fix for PowerPaste not respecting the link:{modulesDir}/configure/integration-and-setup/#cache_suffix[`cache_suffix` setting].
...
* 3 new skins: link:{modulesDir}/enterprise/premium-skins-and-icon-packs/naked-demo/['naked'], link:{modulesDir}/enterprise/premium-skins-and-icon-packs/outside-demo/['outside'], and link:{modulesDir}/enterprise/premium-skins-and-icon-packs/snow-demo/['snow'].
* 1 new icon pack: 'thin'.

For information on using premium skins and icon packs, see: link:{modulesDir}/enterprise/premium-skins-and-icon-packs/[Premium Skins and Icon Packs].
...
For information on the Spell Checker Pro plugin, see: link:{modulesDir}/plugins/tinymcespellchecker/[Spell Checker Pro plugin].
...
Set `toolbar_drawer` or `toolbar_mode` to an alternative mode until the issue is resolved. For a list of alternative toolbar modes, see: link:{modulesDir}/configure/editor-appearance/#toolbar_mode[User interface options - `toolbar_mode`].

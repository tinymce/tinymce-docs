...
* The `toolbar_location` option, see: link:{modulesDir}/configure/editor-appearance/#toolbar_location[User interface options - toolbar_location].
* {productname} in inline mode, see: link:{modulesDir}/general-configuration-guide/use-tinymce-inline/#enablinginlineeditingmode[Setup inline editing mode].
...
For information on toggle menu items, see: link:{modulesDir}/ui-components/menuitems/#togglemenuitems[Custom menu items - Toggle menu items].
...
image::{modulesDir}/images/find-replace-in-selection.png[Find and Replace dialog with new "Find in selection" option highlighted]

For information on enabling the Find and Replace dialog, see: link:{modulesDir}/plugins/searchreplace/[the Search and Replace plugin].
...
* The Quick Toolbars plugin, see: link:{modulesDir}/plugins/quickbars/[The Quick Toolbars plugin].
* Creating custom context toolbars, see: link:{modulesDir}/ui-components/contexttoolbar/[Context toolbars].
...
For information on using the language setting and a list of supported languages for {cloudname} and premium self-hosted deployments, see: link:{modulesDir}/configure/localization/#language[Localization options - language].
...
For information on the Accessibility Checker plugin, see: link:{modulesDir}/plugins/a11ychecker/[Accessibility Checker plugin].
...
For information on the Comments plugin, see: link:{modulesDir}/plugins/comments/[Comments plugin].

For a list of valid menu items, see: link:{modulesDir}/advanced/editor-control-identifiers/#menucontrols[Editor control identifiers - Menu controls].
...
* The PowerPaste plugin, see: link:{modulesDir}/plugins/powerpaste/[PowerPaste plugin].
* The PowerPaste `paste_preprocess` option, see: link:{modulesDir}/plugins/powerpaste/#paste_preprocess[PowerPaste plugin - `paste_preprocess`].
* The PowerPaste `paste_postprocess` option, see: link:{modulesDir}/plugins/powerpaste/#paste_postprocess[PowerPaste plugin - `paste_postprocess`].
...
For information on Spell Checker Pro plugin, see: link:{modulesDir}/plugins/tinymcespellchecker/[Spell Checker Pro plugin].
...
* The Spell Checker Pro plugin, see: link:{modulesDir}/plugins/tinymcespellchecker/[Spell Checker Pro plugin].
* The Link Checker plugin, see: link:{modulesDir}/plugins/linkchecker/[Link Checker plugin].
* The Image Tools plugin, see: link:{modulesDir}/plugins/imagetools/[Image Tools plugin].
* The Enhanced Media Embed plugin, see: link:{modulesDir}/plugins/mediaembed/[Enhanced Media Embed plugin].
* Deploying the server-side components, see: link:{modulesDir}/enterprise/server/[Server-side component installation].
...
From {productname} 5.3, premium self-hosted bundles include the files required to build and deploy the server-side components as Docker containers. For information on Docker deployments of the server-side components, see: link:{modulesDir}/enterprise/server/dockerservices/[Containerized service deployments].

For information on deploying the server-side components using Docker, see: link:{modulesDir}/enterprise/server/dockerservices/[Containerized service deployments].

[#regular-expression-support-for-allowed-origins]
=== Regular Expression support for allowed-origins

Version 2.3.0 of the {productname} server-side components adds support for using regular expressions to specify `allowed-origins`. For information on using regular expressions for specifying allowed origins, see: link:{modulesDir}/enterprise/server/configure/#regularexpressionssupport[Configure server-side components - Regular Expressions support].
...
For information on configuring the same-origin option, see: link:{modulesDir}/enterprise/server/configure/#allowed-originssame-originoptional[Configure server-side components - allowed-origins.same-origin (optional)].
...
For information on configuring the alternative http timeout settings, see: link:{modulesDir}/enterprise/server/configure/#alternativehttptimeoutsettings[Configure server-side components - Alternative http timeout settings].
...
For information on configuring the Link Checker service, see: link:{modulesDir}/enterprise/server/configure/#link-checkingoptional[Configure server-side components - `link-checking`]
...
* The `images_dataimg_filter` option, see: link:{modulesDir}/configure/file-image-upload/#images_dataimg_filter[Image & file upload options - images_dataimg_filter].

...
. Change the `apiKey` in `tinydrive-dotnet-mvc-starter/appsettings.Development.json`. You get the api key from the `API Key Manager` {accountpage} page at http://tiny.cloud.

. Create a `tinydrive-dotnet-mvc-starter/private.key` file containing your generated private RSA key. You get the private RSA key from the `JWT Key Manager` {accountpage} page at http://tiny.cloud.

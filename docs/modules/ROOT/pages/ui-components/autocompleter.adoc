...
An `autocompleter` enables users to quickly find and select from a pre-populated list of values as they type, leveraging searching and filtering. It provides suggestions to insert while the user is typing into the content. For example, with the link:{modulesDir}/plugins/charmap/[charmap] plugin enabled, typing *:amp* should show the ampersand item in the menu. Pressing `esc` should close the autocomplete menu.

...
When a new sidebar is registered, a corresponding toolbar button for toggling the sidebar open and close is also created using the same name. This button can then be included in the toolbar by adding the sidebar name to the link:{modulesDir}/configure/editor-appearance/#toolbar/[`toolbar`] option.
...
The `icon` specifies an icon for the sidebar toggle button. The icon should be the name of an icon provided by the {productname} skin or a link:{modulesDir}/api/tinymce.editor.ui/tinymce.editor.ui.registry/#addicon/[custom icon].

...
NOTE: The identifier used to create the menu item must be included in the link:{modulesDir}/configure/editor-appearance/#menu[`menu`] option in the {productname} configuration for it to be added to the menubar's menus. It will not be added to the menubar's menus if `menu` is not configured correctly.
...
###include::{partialsDir}misc/shortcut-os-mappings.adoc[]

...
`onSetup` is used to listen to the editor's link:{modulesDir}/advanced/events/#nodechange[`NodeChange` event] to disable the button when the cursor is inside a `time` element (or "node"). This ensures it is not possible to insert a `time` element into another `time` element.
...
Use the following demo link:{modulesDir}/demo/custom-toolbar-menu-button/[here] for help using the menu toolbar button.
...
A group toolbar button is a toolbar button that contains a collection of other toolbar buttons that are displayed in a pop-up when clicked. The style of toolbar shown is based on the current link:{modulesDir}/configure/editor-appearance/#toolbarmode[toolbar mode]. For example, if `toolbar_mode` is set to `floating`, the toolbar group pop-up will appear in a floating shelf.
...
| required
| A string of space separated toolbar button names, or an array of link:{modulesDir}/configure/editor-appearance/#addingtoolbargrouplabels[labelled toolbar buttons].

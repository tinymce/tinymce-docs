...
The link:{rootDir}enterprise/tiny-comments.html[Comments 2.0] is a premium plugin built on the link:{rootDir}advanced/annotations.html[Annotations API] to create comment threads (conversations). Comments 2.0 provides the ability to start or join a conversation by adding comments to the content within the TinyMCE editor.

Refer to the link:{rootDir}plugins/comments.html[Comments 2.0] documentation, for more information.
...
The context menu supports both individual menu items and dynamic context menu sections. Refer to the link:{rootDir}ui-components/contextmenu.html[context menu] documentation, for more information.
...
Refer to the link:{rootDir}ui-components/contexttoolbar.html[Context toolbar] documentation, for more information.
...
Refer to the link:{rootDir}ui-components/customsidebar.html[Custom sidebars] documentation for more information.

[[dialog]]
=== Dialog

The link:{rootDir}ui-components/dialog.html[Dialog] API allows showing dialogs (sometimes referred to as modals) in the user application. For example, the plugin that is used to view the source code is an example of a simple dialog that displays the HTML code from the content.
...
The link:{rootDir}enterprise/formatpainter.html[Format Painter] premium plugin allows copying and pasting formatting (such as font style and size) from one location to another. The Format Painter is capable of working with a wide variety of formats and styles such as inline & block formats and table styles.

The Format Painter may be used through keyboard shortcuts or a toolbar button. Refer to the link:{rootDir}plugins/formatpainter.html[Format Painter] documentation, for more information.
...
The link:{rootDir}enterprise/mentions.html[Mentions] plugin makes it possible for users to send notifications using *@username* in the document.
...
The link:{rootDir}enterprise/pageembed.html[Page Embed] premium plugin allows inserting content from another source, such as an advertisement, on a Web page.
...
Refer to the link:{rootDir}plugins/pageembed.html[Page Embed] documentation, for more information.

[[permanent-pen-1-0]]
=== Permanent Pen 1.0

The link:{rootDir}enterprise/permanentpen.html[Permanent Pen] is a premium plugin that allows adding comments or responses in emails or other documents, without having to change the text color or style.
...
Refer to the link:{rootDir}plugins/permanentpen.html[Permanent Pen] documentation, for more information.
...
`Border` input field in the table properties dialog is now called `Border width` for clarity. Refer to the link:{rootDir}plugins/table.html[Tables] documentation, for more information. Tables are enriched with the following new additions:
...
Refer to the link:{rootDir}ui-components/toolbarbuttons.html[Toolbar buttons] documentation, for more information.

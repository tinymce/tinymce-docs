...
* link:{modulesDir}/plugins/charmap/[Character Map].

Plugins relying on the link:{modulesDir}/ui-components/autocompleter/[autocompleter API] or keypress events do not work on Android. Android intercepts content and processes content prior to insertion into the editor.
...
NOTE: This issue affects the link:{modulesDir}/plugins/comments/[Comments plugin].
...
For the comments plugin, the comments sidebar can be collapsed using the `showcomments` toolbar button (image:{imagesDir}icons/comment.svg[comment.svg]).

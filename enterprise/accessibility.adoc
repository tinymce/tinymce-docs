...
For most developers, accessibility has become critical to the usability of their solutions. {productname}`'s link:{baseurl}/plugins/a11ychecker/[accessibility checker] (based on technology built by link:{url}[{companyname}]) makes checking for common accessibility issues incredibly easy for content creators by using an interface that steps them through each error, and in most cases also recommends a solution.

...
To set mobile specific settings, add the setting to the link:{baseurl}/docs/configure/editor-appearance/#mobile[`mobile`] configuration, such as:
...
* link:{baseurl}/plugins/table/#table_grid[`table_grid`] - Table grid is not supported on mobile devices and defaults to `false`. When creating tables on mobile, a dialog is shown instead of the table grid.
...
* link:{baseurl}/plugins/table/#table_grid[`table_grid`].
...
The plugins supported by legacy mobile theme are limited to the link:{baseurl}/plugins/autosave/[autosave], link:{baseurl}/plugins/autolink/[autolink], and link:{baseurl}/plugins/lists/[lists] plugins.
...
NOTE: The `list` functions require the link:{baseurl}/plugins/lists/[lists] plugin and `styleselect` requires configuring link:{baseurl}/configure/editor-appearance/#style_formats[style_formats].

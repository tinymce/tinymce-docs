...
module.exports = function() {
  this.blockMacro(function() {
    ...
    this.process((parent, target, attrs) => {
      ...
      const rootLiveDemoDir = path.join(docAttrs['page-origin-start-path'], 'modules', docAttrs['page-module'], 'live-demos');
      const type = attrs.type || 'tinymce';
      const contentData = loadContent(engine, rootLiveDemoDir, target, docAttrs);
      const initialTab = attrs.tab || 'run';
      const scriptUrl = attrs.script_url_override || getScript(type, docAttrs);

      // Render the template
      const renderedContent = engine.renderFileSync('live-demo.adoc', {
        site: {
          ...docAttrs
        },
        liveDemo: {

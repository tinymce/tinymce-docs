...
1. `isDisabled()` functions are now `isEnabled() functions.
   ...
   Configurations that called `isDisabled`() must now call `isEnabled()`. Also, the returned or set value must be interpreted in terms of the new function name.

   For example, if `isDisabled(false)` was previously called, the equivalent new call is `isEnabled(true)`.
...
==== `fire` function re-named in all {productname} APIs

The `fire()` function in
...
has been re-named to `dispatch()`.

`fire` has been aliased to `dispatch` but has also been marked as _deprecated_.

=== `new` keyword

{productname} plugins (and themes and some other add-ons) were once classes. And, as such, they could be constructed by:

1. taking a function and then;

2. initialising the plugin with the `new` keyword (eg `new Plugin(editor)`).

However the `new` keyword cannot be used with arrow functions. Also, it is not necessary to instantiate a plugin with the `new` keyword, since they are not classes, and have not been classes for some time.

In {productname} 6.0 the `new` keyword can no longer be used to create instances of plugins and the like.

This makes it safe to use both older functions and newer arrow functions when instantiating add-ons.
...
Also, the `editimage_proxy` option has been re-named to `export_image_proxy_service_url`. The option was re-named to make it easier to configure when using https://tiny.cloud/docs/enterprise/server/[Tiny services].
...
In {productname} 5.x, editors without a `height` value specified in the {productname} configuration defaulted to a 200px high text entry area. In {productname} 6.0, editors without a `height` value specified in the {productname} configuration default to a 400px high text entry area.
...
NOTE: while {productname} 6.0 honours the `height` setting in {productname} configuration, to prevent text entry areas from disappearing during re-sizing, the `min-height` value (100px) cannot be over-ridden.
...
[id=ui-elements-and-components-tables-positioned-using-margins-not-float]
...
Alternatively, the https://tiny.cloud/docs/api/tinymce/tinymce.formatter/#register[Formatter API] can be used to register a custom format using `editor.formatter.register(<name>, <format>)`.

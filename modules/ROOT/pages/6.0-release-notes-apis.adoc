...
`fire` has been aliased to `dispatch`, so existing configurations should still work as expected.

`fire` has also, however, been marked as _deprecated_. Editing configurations to use `dispatch()` is recommended.

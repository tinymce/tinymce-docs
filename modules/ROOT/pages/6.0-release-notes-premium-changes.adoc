...
| `formatpainter_blacklisted_formats` | `formatpainter_ignored_formats`  | Option                              | NB: _Image Tools_ is now _Enhanced Image Editing_, a https://tiny.cloud/tinymce/features/#productivity[Premium plugin].
...
// to do above
// link out to each plugin page
//
// pre-requisite
// xref or https as link syntax?
...
//
// pre-requisites
// 1. xref or https as link syntax?
// 2. edit each opening sentence to provide text amenable to being wrapped in xref or https anchoring.
...
The `getRowType` function would sometimes incorrectly parse elements in the `tfoot` section of a table.

This led to `mceTableRowType` being incorrectly returned as a header if all cells in the section were `th` elements.

With this update, `getRowType` parses elements in the `tfoot` section correctly and `mceTableRowType` is correctly returned as a footer.


=== Comments

The following fixes were applied to the Comments plugin:
...
The following improvements and changes were made to Format Painter.
...
The following improvements, changes, and fixes were made to Real-time Collaboration.
...
// xref:partial$configuration/inline-boundaries.adoc[`inline_boundaries`] ?

When provider values are incorrect, the returned error messages are now less opaque.

For example, the error message `'key' was not a string` is now the error message `Expected field 'key'`.

Previously, if a {productname} editor instance with Real-time Collaboration loaded contained a Horizontal Rule (ie an `+<hr>+` element), attempting to move the insertion point over the Horizontal Rule using arrow keys threw an exception: `Uncaught Error: Unsupported node type: [object DocumentType]`.
...
This update to Spell Checker Pro includes the following fixes.

...
==== sub-menu items now read by screen readers when using Google Chrome

Previously, in newer versions of Google Chrome, navigating through {productname} menus using the keyboard did not cause screen readers to read out menu items.

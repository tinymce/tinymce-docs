...
If multiple table rows were selected in a {productname} table with two or more columns and the cell properties dialog was invoked, deleting, for example, the `height` value in the dialog box had no effect. However, if each cell was selected individually, invoking the cell properties dialog and deleting the `height` value worked as expected.

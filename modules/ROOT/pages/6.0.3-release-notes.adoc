...
=== When multiple rows were selected, removing values in the cell properties dialog did not work
...
Previously, when `format_empty_lines` was set to `true`, lines that included no text (ie, a line consisting of just a Return) did not preserved inline formatting when saved.
...
=== Some inline text elements specified via the schema were not removed when empty by default

// any further information on this greatly appreciated.

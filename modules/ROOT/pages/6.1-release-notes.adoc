...
When a {productname} document had *View > Show invisible characters* checked and text was selected in this manner, and the user paused for a second or two, pausing the selection extension, the `moveToBookmark` function passed the current selection to `getBookmark` and then restored the selection in the document.

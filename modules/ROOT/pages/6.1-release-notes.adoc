...
The {productname} vnumtxt release includes an accompanying release of the **A11ychecker** premium plugin.

**A11ychecker** 3.0.2 fixes the following UI bug:

The *Tools > Accessibility Checker* command opens a dialog. As such, its menu title should be *Accessibility Checker…*.

The trailing ellipsis indicates the command will not execute without further user input and is required for all menu commands that present dialogs.

This update corrects both the menu title and the equivalently mis-presented toolbar tool tip text. Both strings now include the required ellipsis and present as *Accessibility Checker…*, as expected.
...
The {productname} vnumtxt release includes an accompanying release of the **Comments** premium plugin.
...
A new API, `tinymce.activeEditor.plugins.tinycomments.getEventLog()` is now available.
...
* when a coment is edited (and who edited it).
...
[source,js]
////
tinymce.activeEditor.plugins.tinycomments.getEventLog(
...
The default icon presented next to user names in comments can now be replaced by a custom avatar.
...
In callback mode, the avatar is supplied by the `avatar` option of `setConverstation()`:

[source,js]
----
const avatarUrl = "https://example.com/image.ext";

/*

much non-avatar-related configuration here
...
* must be 36px by 36px.

* can be any filetype able to be wrapped in an `<img>` element.

* will present as the browser’s default ‘image not found’ icon if the avatar resource is not available at the nominated URL.

NOTE: Enablng this feature depends on changes made to the Core {productname} editor. As a consequence, showing avatars next to the user-name in comments requires both {productname} 6.1 or later and version 3.1.0 or later of the xref:comments.adoc[Comments Premium Plugin].
...
For a customised comments sidebar, setup the sidebar in the `setup` section of `tinymce.init`:

[source,js]
----
	setup: (editor) => {
	  editor.ui.registry.addSidebar('showcomments', {
		tooltip: 'showcomments',
		icon: 'comment',
		onSetup: (api) => {
...
NOTE: The comment sidebar presents with no animation or other effect. It is just there, along with any other initialised {productname} editor components. Also, as noted above, the sidebar does not take focus, which remains with the main editor.
...
When comments are showing, a 3px wide yellow (hex color #FFE89D) rectangle presents around any element with a comment attached.

NOTE: Enablng this feature depends on changes made to the Core {productname} editor. As a consequence, adding comments to these elements requires both {productname} 6.1 or later and version 3.1.0 or later of the xref:comments.adoc[Comments Premium Plugin].


==== The label name for the Save Comment button can now be customized

Previously, the Save Comment was labeled *Save*. After field reports that end-users were interpreting this to mean the entire {productname} document would be saved on pressing this button, two changes have been made.
...
2. This default label is now customisable.
+
_How does one customise this? I presume it’s a value one sets as part of `tinymce.init` but which one? And where in this configuration file?_
+
_Or is it a matter of an integrator editing the `.pot` file — `src/text/pot/plugins/tinycomments/tinymce_tinycomments.pot` — that comes along for the ride?_
...
The {productname} vnumtxt release includes an accompanying release of the **Enhanced Media Embed** premium plugin.

**Enhanced Media Embed** 3.0.1 fixes the following bug:

==== Enhanced Media Embed tests failed in several browsers

An incorrect `RealKeys()` setting caused the Enhanced Media Embed webdriver tests to fail in several browsers (including Firefox, Microsoft Edge, and Safari).

With this update, this setting has been corrected, from `RealKeys.text(\n)` to `RealKeys.combo({}, 'enter')`. As a consequence, the Enhanced Media Embed webdriver tests now pass in all supported browsers, as expected.

For information on the **Enhanced Media Embed** plugin, see: xref:enhancedmediaembed.adoc[Enhanced Media Embed plugin].


=== Page Embed 2.1.0

The {productname} vnumtxt release includes an accompanying release of the **Page Embed** premium plugin.

**Page Embed** 2.1.0 includes the following bug fix:

==== Portions of the Page Embed dialog were not fully translated

Four elements in the Page Embed dialog presented in English when the configured language was not English.
...
With this update, these four elements are now translated into all configurable languages, and appear correctly when {productname} is switched to another language, as expected.
...
The {productname} vnumtxt release includes an accompanying release of the **PowerPaste** premium plugin.
...
Equivalent results occured when URL paths contained the colon character (for example, `https://example.com/:colon`).
...
NOTE: exclamation points and colons are still boundary characters for URL detection and delmiting. If the string `https://example.com/colon:` is pasted or typed into a {productname} document, automatic link detection will produce a link that does not include the trailing colon character.
...
For information on the **PowerPaste** plugin, see: xref:powerpaste.adoc[PowerPaste plugin].


=== Real-Time Collaboration 2.1.0

The {productname} vnumtxt release includes an accompanying release of the **Real-Time Collaboration** premium plugin.

**Real-Time Collaboration** 2.1.0 adds one new features and fixes two bugs:

==== Real-Time Collaboration can now disable inline boundaries

xref:content-behavior-options.adoc#inline_boundaries[*Inline boundaries*] are highlights applied to certain inline elements. The exact elements highlighted can be configured with `inline_boundaries_selector` but the elements highlighted by default are `a` (anchor tags), `code`, and `.mce-annotation`.
...
NOTE: the equivalent behavior could be achieved by setting `inline_boundaries_selector` to an empty string. And this setting was, previously supported by Real-time Collaboration (and still is). But setting `inline_boundaries` to `false` is the preferred method of turning inline element highlighting off.

==== Lists selected with Select All (Ctrl+A or Cmd+A) did not provide a valid fragment for `tinymce.activeEditor.selection.getContent()`

If a {productname} document contained only a list, and the entire document was selected using *Edit > Select All* (or by typing Ctrl+A or Cmd+A), the selection did not present as valid to `tinymce.activeEditor.selection.getContent()`.
...
For information on the **Real-Time Collaboration** plugin, see: xref:real-timecollaboration.adoc[Real-Time Collaboration plugin].


=== Spell Checker Pro 3.0.3

The {productname} vnumtxt release includes an accompanying release of the **Spell Checker Pro** premium plugin.

**Spell Checker Pro** 3.0.3 includes the following bug fix:

==== An issue with the `DomDescent.toLeaf` API could result in Spell Checker Pro’s ‘preserve selection’ logic setting the selection when it did not need to

The `DomDescent.toLeaf` API can walk inside void elements (for example `+<br>+`). The Core editor’s selection logic, by contrast, does not allow this.

Consequently, when comparing selections where one is from `editor.selection.getRng()` and the other is created by `DomDescent.toLeaf`, the comparison can return `false` when it should be `true`.

When this happens, code runs when it does not need to: in this case, Spell Checker Pro’s ‘preserve selection’ logic.

With this update, SpellcheckerPro now correctly handles `DomDescent.toLeaf` being able to walk into a void element, making the range comparison more accurate, and ensuring the preserve selection code does not run when not needed.

For information on the **Spell Checker Pro** plugin, see: xref:spellcheckerpro.adoc[Spell Checker Pro plugin].


=== Table of Contents 1.1.0

The {productname} vnumtxt release includes an accompanying release of the **Table of Contents** premium plugin.
...
The **Premium Skins and Icon Packs** release includes the following bug fix:

When using a Premium skin and the xref:comments.adoc[Comments plugin], avatars in the comments overflowed their bounds and covered the comment text in the Comments side-bar.

This **Premium Skins and Icons Packs** release has been re-built to pull in the changes required to correct this.

When a Premium skin and the Comments plugin are now used together, avatars stay within their bounds as they do in the {productname} default skin, and comment text is not hidden.
...
=== Links opened when Alt-Return or Option-Return was typed even when `preventDefault()` was called on the keydown event
...
Previously, when an image was one of the selected objects in a {productname} document, the image did not present with the _Selected_ background color like text objects do. (In most browsers the _show the object is selected_ colour is, by default, blue.)

The image was selected, but there was no visual feedback to show this.

With this update, images that are selected as part of a mult-object selection show the same _Selected_ background color as other objects.
...
When re-sizing the viewport while a {productname} dialog is open, the dialog did not start re-sizing until the viewport boundary begins encroaching on the dialog. As a consequence, the dialog was partially cut off in the re-sized viewport (specifically, the bottom border and several pixels of the dialog’s buttons were over-shot) and it could become difficult to distinguish the dialog from the viewport.
...
In investigating and fixing this issue (see immediately below) it was also found that some function types in the `tinymce.dom.TreeWalker` class did not detect that the class could return an `undefined` value.
...
{productname} responds to the `KeyDown` event and exits the active element when the *esc* key is pressed. Previously, however, the consequent `KeyUp` event (generated when the user stops pressing the *esc* key) was propogated to the {productname} editor. This should not have happened.

The `KeyDown` event does not propagate outside the component listening for it and neither should the `KeyUp` event.

As of this update, this is the behavior: pressing the *esc* key closes or exits an active user-interface component and neither the `KeyDown` or `KeyUp event are propogated outside the component listening for these events.


=== Selection direction was not stored/restored when getting/setting selection bookmarks
...
When a {productname} document had *View > Show invisible characters* checked and text was selected in this manner, and the user paused for a second or two, pausing the selection extentsion, the `moveToBookmark` function copied the current selection into a bookmark and then restored the selection in the document.

In doing this, however, the selection direction was lost, because `movetoBookmark` did not store the selection direction state.
...
As of this update, `moveToBookmark` now stores and restores selection direction. And pausing and restarting a selection as described above works as expected.
...
`null` is not a valid value for `input` and, as a consequence, an exception was thrown.

With this update, the `editor.selection.isForward()` API bug has been corrected, invalid selections no longer occur, a valid `input` value is passed to `input.parentNode.removeChild(input)`, and exceptions are no longer thrown.
...
With this update, this behavior has been corrected. Selecting a string and running `tinymce.activeEditor.execCommand('insertParagraph');` now replaces the selection with a paragraph break, as expected.
...
Also, the list type change did not need to be from an unordered to an ordered list. The same erroneous list-creation happend if a round-bullet bulleted list was changed to a square-bullet bulleted list.
...
As a consequence, it is now possible to add custom elements to empty lines in a {productname} document. added to


=== Pasting columns in tables could, in some circumstances, result in an invalid table

Errors in the logic of both the *Copy Column* and *Paste Column* commands could, in some circumstances, cause a Copy and Paste of table cells to produce an invalid table.

With the insertion point in, for example, a cell in the third column of a 4x4 table, selecting *Table > Column > Copy Column* and then *Table > Column > Paste Column Before* produced a table that appeared to be a 5x4 table. But the now-5th column

NOTE: When the column was pasted in, the table would appear to change as expected. But attempt to change material in some cells would fail, because the underlying table DOM was invalid.
...
was pasted into a {productname} document, the URL detection would cause the pasted text to present as
...
The underlying cause was Autocompleter using `dom.getRoot()` as its root element. As of this update, Autocompleter uses the nearest parent block element as its root element rather than `dom.getRoot()`. And, consequently, trigger characters cause the suggestions display to present in nested list items inside populated list items, as expected.
...
In some cases it did not escape settings values for safe use within a regex and, in one case, incorrect handling caused word boundaries — `+\b+` — to not work as expected.

These errors have been corrected. In {productname} 6.1, template settings values are escaped properly. As well the incorrect handling has been corrected, and word boundaries work as expected.
...
This capture meant that the neither {productname}’s built-in copy-and-paste, nor the PowerPaste plugin’s copy-and-paste could intercept the copy event and act to copy (and, consequently, paste) the selected material.
...
Previously, when {productname} was in any dark mode (ie `skin_url` pointed to a dark themed skin), choosing *View > Preview* presented a background in the preview that was not consistent with the dark themed content.
...
Previously, if an image was uploaded and then immediately deleted, the removal triggered a second change event, which, in turn, changed the undo stack.
...
Peviously, if a {productname} editor had an HTML comment as its first element, the `editor.selection.normalize()` API set the selection to within the comment element.
...
NOTE: In the HTML version of such a document (viewable by choosing *View > Source Code*) , each Return was represented by a paragraph tag containing a non-breaking space entity: `+<p>&nbsp;</p>+`. This representation, which accurately presents the empty paragraphs as white space is unchanged in {productname} 6.1.
...
As of this update, the `end_container_on_empty_block` has been changed such that pressing the *Return* key twice will exit the insertion point from a `blockquote` element. This is the same action that exits other block elements, such as lists.
...
=== The `autocompleter` menu element is now positioned instead of the wrapper

The `autocompleter` menu element — made available to a {productname} instance by the `editor.ui.registry.addAutocompleter()` function — presents a menu of formatting options when the `+/+` key is pressed in an active editor.

If the current viewport is too small to present the entire menu, access to the ‘overflow’ is ensured by presenting a scrollbar in the menu.
...
In some circumstances this error resulted in the top or bottom menu borders to scroll out of view, making the meny appear unusable.

This update corrects this. Rendering of the `autocompleter` menu height is now assigned to `tox-menu` and its menu borders do not scroll out of view.


=== Link menu and toolbar buttons now always execute the `mceLink` command

Selecting *Insert > Link…* or clicking the Link toolbar button did not, previously, execute the `mceLink` command. Equivalently, if the `link_quicklink` option was enabled, using the Ctrl+K or ⌘+K shortcut to open the Contextual Link Toolbar did not execute `mceLink` either.

This was not consistent with other, related, actions. For example, clicking the Unlink toolbar button executed the `unlink` command. As well, not executing a command meant it was not possible to track the *Link…* dialog being opened, and the command could not be over-ridden with `addCommand`.
...
Consequently, screen readers will, correctly, report the options in these menus as mutally exclusive.
...
=== All help dialog links to `https://tiny.cloud` now include `rel="noopener"` to avoid potential security issues

`rel` is an attribute for the `<a>` element (and other elements). And setting this attribute to `rel="noopener"`  instructs the browser to load the resource referenced in the anchor tag in the same browsing context (ie, it instructs the browser to not open a new window).
...
This provides basic protection against window opener attacks.
...
* `block`
+
`block` sets {productname} to treat all four keystrokes as if they are *Return*.
  
* `linebreak`
+
`linebreak` sets {productname} to treat all four keystrokes as if they are *Shift-Return*.
  
* `invert`
+
`invert` swaps the behavior or *Return* and *Shift-Return* (and *Enter* and *Shift-Return*). That is, pressing *Return* results in *Shift-Return* behavior, and vice versa.
...
NOTE: Two other commands that, in their effect, add new lines to a {productname} document are `insertParagraph` and `InsertLineBreak`. Their behavior is not affected by any `newline_behavior` setting.
...
The new `iframe_template_callback` option, provided by https://github.com/Namstel[Namstel], is an addition to the media plugin.

The `iframe_template_callback` option takes a function and replaces the default callback when the source URL matches a valid iframe.

It works equivalently to the `audio_template_callback` and `video_template_callback` options, but for iframes.


=== New `dispatchChange` UndoManager API to fire the change with current editor status as `level` and current undoManager layer as `lastLevel`
...
This pseudo-selector is supported in current and recent versions of all major browsers, but it is not supported in older browser versions.
...
=== If a `codesample` is the first element added to a {productname} document, the selection is incorrectly set to inside the `contenteditable="false"` block

When adding a `codesample` (by, for example, choosing *Insert > Code sample…*), the added material is placed within a `contenteditable="false"` block.

This block can be selected and copied but not edited. And, once copied, pasting it back adds a further `contenteditable="false"` block containing the added material, to the document.

In {productname} 6.1, however, if a `codesample` is the first material added to a document, {productname} incorrectly sets the selection to the added material within the block, and not to the block itself.

If this material is then copied and pasted, it is added to the document as editable text. This is not the expected behavior.

There is a workaround: adding a blank line to an otherwise empty {productname} 6.1 document before adding a `codesample` results in the selection being sets to the added block, as expected.

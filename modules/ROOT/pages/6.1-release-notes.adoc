...
NOTE: Enabling any of these features depends on changes made to the Core {productname} editor. As a consequence, version 3.1.0 of the xref:comments.adoc[Comments Premium Plugin] requires {productname} 6.1 or later. 
...
In callback mode, the avatar is supplied via the `tinycomments_create` and `tinycoments_reply` callbacks.

...
2. This default label is now customisable.
   ...
   [How does one customise this? I presume it’s a value one sets as part of `tinymce.init` but which one? And where in this configuration file?]
   
   [Or is it a matter of an integrator editing the `.pot` file — `src/text/pot/plugins/tinycomments/tinymce_tinycomments.pot` — that comes along for the ride?]

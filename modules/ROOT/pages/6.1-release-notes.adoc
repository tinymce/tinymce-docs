...
This update corrects this. In Comments 3.1.0, deleting any or all comments fires a `change` event. And a `changed` message is, consequently, printed to the console.
...
==== Creating or editing a comment was not blocking the UI while saving the data in callback mode
...
A bug in this new feature, however, made it possible to, at least apparently, add a comment to an `+<hr>` block.
...
With this update, `+<hr>` blocks specifically cannot have comments added to them.


==== Previous comment replies were retained in the comment placeholder after the comment had been submitted

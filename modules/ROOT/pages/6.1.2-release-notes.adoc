...
With this update, the exception is no longer thrown in this circumstance. In {productname} 6.1.2, selecting all of a {productname} document that includes a `contenteditable="false"` property at the beginning or end of the document and deleting the selected contents produces no console error.
...
With the {productname} 6.1.2 update, this visual misbehavior has been corrected. When the Comments sidebar is set to show on initialization, the Show comments button, if present, is now highlighted.
...
This update corrects this. In {productname} 6.1.2, the `autolink` plugin both fires an `ExecCommand` event and adds link-creation to the undo stack, as expected.

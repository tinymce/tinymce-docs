...
The Real-Time Collaboration plugin validates that parameters, used to set the token provider among other settings, are valid link:https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise[Promises].
...
If a `list` has a `contenteditable` attribute value of its own, this is, of course the closest ‘inheritable’ attribute, and the list and its child elements takes that value.

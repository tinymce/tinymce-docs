...
=== MenuButtons can now contain a search field

MenuButtons, buttons on the {productname} toolbar that show a menu when clicked, can now present a search field at the top of the menu.

The button refetches items as the search field is updated.

The new xref:mergetags.adoc[Merge Tags] Premium plugin includes a MenuButton with this new search field.
...
This new capability is also configurable using the new `format_noneditable_selector` option.
...
The Spell Checker Pro plugin fragments text nodes as part of its normal workings. Previously however, this fragmentation broke the URL text detection of the Autolink plugin.
...
There were two related causes for this behaviour: an attribute not being assigned to a `<br>` tag when required, and {productname} not removing unnecessary elements when parsing the content.
...
Previously, when a {productname} editor was not in view and was set to auto focus, the editor was focused but did not scroll into view.

This only occurred if the host browser was Google Chrome.
...
{productname} 6.2 deals with these issues and typing a space immediately after a a block with a `contenteditable="false"` attribute now adds that space, as expected.
...
For this update the remove formatting logic was changed. In {productnmae 6.2 all non-{productname} specific or non-{productname} internal `data-*` custom attributes are considered content that must be preserved.
...
The handling of spaces inserted into the xref:dialog-components/#urlinput[UrlInput] dialog component, specifically its implementation in the xref:link[Link Dialog], was inconsistent.
...
In {productname} 6.2, the xref:dialog-components/#urlinput[UrlInput] dialog component no longer allows leading or trailing spaces. Only spaces within other text content are now allowed.
...
the {productnanme} 6.2 parsing logic was updated and {productname} can now handle large documents without throwing this error.
...
In {productname} 6.2, list functionality in this regard is dependent on the value of the list’s `contenteditable` attribute.

And an editable list element’s functionality is now dependent on the value of the `contenteditable` attribute in its parent list.

////
More specifically, this is dependent on the contenteditable state, meaning that the parent list may not have a contenteditable attribute (no attribute specified, by default it would have inherit) but the first parent of that list with a contenteditable attribute determines the contenteditable state of that list, for example, a noneditable div means any list within that div without a contenteditable attribute would be noneditable. Another example would be a contenteditable true list would be editable, even within a noneditable div. Just wanted to clarify that this isn't always determined by the attribute, but by the state of the first parent with the attribute.
////


=== Removed extra bottom padding in the context toolbar of the TinyMCE 5 skin.

In {productname} 6.1.2, extra padding was added to the context toolbar and, consequently, separator lines were incorrectly drawn on multi-line toolbars when using either the `tinymce-5` skin or premium skins.
...
Use the `trigger` property instead.
...
{productname} 6.2 adds a new UI option to MenuButtons. These toolbar objects can now present a search field at the top of the menu.

...
Currently there is no wordaround for this issue, however.
The issue can be currently avoided by not swithching to another `lang` after `en_GB` is set.

==== Toobar more button disappear after exit from inline code editor (P2)
...
==== Fix applying typography to quotes for French language (P2)
...
==== Safari focus visible not applying a blue outline to all buttons (P2)
...
==== Color swatch checkmark is not displayed when accessed forecolor or backcolor from menu (P3)
...
==== Backcolor not showing selected color-picker in dialog when inline styles (B/I/U..) applied before applying backcolor (P3)
...
==== Resolve conflict with typography and spellchecker (P3)

// https://ephocks.atlassian.net/browse/TINY-9413
// @bforte review required
// cause
There is a notable conflict with the typography and spellchecker where the spellcheck would affect the typography plugin when different `langs` where selected.

There is currently no workaround for this issue. 

==== Fix removing spaces between ! ! (P3)

...
// @bforte review required
In {productname} 6.3, improvements for the accessibility checker were added to provide additional support for users when trying to identify different alerts such as `info | warning or error` icons.
...
Another benefit from the introduction of this improvement made in {productname}'s 6.3 release is that users requiring visual aids will be able to enjoy the benefits of an improved user experience if they choose to upgrade to {product name} 6.3.
...
// @bforte review required.
Prior to the release of {productname} 6.3, It was reported that a number of calls to the `DomFactory.fromHtml` were escaping the text being substituted into the HTML `string`, resulting in an isolated `alert/notification` being displayed within the {productname} editor viewport. 

{productname} 6.3 introduced `dompurify` into to the core dependancies to reduce the risks associated with XSS attacks.
...
:awaiting update for content 

==== Fix issues with quotation marks when switching to en_GB and de

// when the new xref:advanced-typography.adoc[Advanced Typography] Premium plugin is used.
// https://ephocks.atlassian.net/browse/TINY-9332
// @bforte review required
There is a current known issue in {productname} 6.3, that stops the user from switching to any other languages after Advanced Typography `lang` is set to `en-GB`, as it causes issues with some rules when applied after the switch.

As a consequence, quotation marks will not be applied to any text selection after swithing to any other `lang` from `en_GB` specific when the Typography plugin button is pressed.

Currently there is no wordaround for this issue, however the issue can be currently avoided by not switching to another `lang` after `en_GB` is set.

==== Toobar more button disappear after exit from inline code editor

//https://ephocks.atlassian.net/browse/TINY-9419
// @bforte review required

When {productname} 6.3 is running `advcode_inline: true` and `toolbar_mode: sliding` specifically the `more` button disappears after clicking on the _cancel_ button in the advanced code viewport.

This instance only occurs when the toolbar is forced into a viewport small enough to render the `more` button inside the editor.

Example below, shows the viewport `width` is less than the toolbar width.
...
The current issue is specific to all browsers that are setup using `advcode_inline: true` and `toolbar_mode: sliding` specific.
A current workaround for this issue is to adjust the browser frame which will cause the button to reapear in the {productname} viewport.


==== Fix applying typography to quotes for French language

// https://ephocks.atlassian.net/browse/TINY-9434
// @bforte review required
Applying typography for French language unexpectedly removes characters.

The French language uses Guillemets `« »` as punctuation marks, which causes conflicts with the existing typography functionality, but specifically any complex formatting cases for content when the user applys the Advanced Typography plugin to any text selection while using the French `lang`.  

No workarounds currently for available for this.

==== Safari focus visible not applying a blue outline to all buttons

// https://ephocks.atlassian.net/browse/TINY-9330
// @bforte review required
Unusual focus issues have been idenified while in _developer_mode_ for _Safari_v_16.1_ for some elements when rendered into HTML.

As an example, open _developer_mode_ in _Safari_v_16.1_, apply Accessability Checker dialog via the toolbar, hover over the `down` button should trigger the toggle `:focus-visible` which should enable a blue outline around the button.
In some situations such as the above example, the outline is not applying to all buttons in {productname} 6.3 when using  _Safari_v_16.1_ or higher versions.

Currently there are no workarounds for this issue specifically when using _Safari_v_16.1_.

==== Color swatch checkmark is not displayed when accessed forecolor or backcolor from menu

// https://ephocks.atlassian.net/browse/TINY-9395
// @bforte review required
With the update to {productname} 6.3, a low level issue was found where the color swatch checkmark would not display in the `forecolor` and `backcolor` dialog when accessed by the {productname} menu toolbar.

As a result, users will only be able to view the color swatch checkmark via the toolbar dialog buttons as the current workaround.

==== Backcolor not showing selected color-picker in dialog when inline styles (B/I/U..) applied before applying backcolor

// https://ephocks.atlassian.net/browse/TINY-9410
// @bforte review required
// cause
A regression was found where the `backcolor` would not render the tick inside the selected custom color from the `backcolor` dropdown dialog when {productname} is used specifically on MacOS + Firefox + _Inline_mode_.
// consequence
As a consequence, specific sequences would result in the tick not applying such as when B/I/U is applied prior to the `forecolor` and `backcolor`. 
// workaround (if any)
The only current known workaround for users would be  to apply the underline font-styling + `forecolor` and `backcolor` which will result in the tick being rendered inside the dialog.

==== Resolve conflict with typography and spellchecker

// https://ephocks.atlassian.net/browse/TINY-9413
// @bforte review required

There is a notable conflict with the typography and spellchecker where the spellcheck would affect the typography plugin when a different `lang` is selected.

Currently there is no workaround for this issue. 

==== Fix removing spaces between ! !

// https://ephocks.atlassian.net/browse/TINY-9398
// @bforte review required
:awating content as we dont support `common/space/delBetweenExclamationMarks` currently`
// cause
// consequence
// workaround (if any)
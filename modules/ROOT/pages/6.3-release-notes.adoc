...
=== Accessibility Checker <accessibility checker version number>

The {productname} 6.3 release includes an accompanying release of the **Accessibility Checker** premium plugin.

**Accessibility Checker** <accessibility checker version number> <type(s) of change here>:

// accessibility checker change documentation here.
...
{productname} 6.3 also includes the following improvement:
...
{productname} 6.3 also incorporates the following change:
...
=== Overscroll issue in floating toolbar in Firefox.

// https://ephocks.atlassian.net/browse/TINY-9237
The floating toolbar background dividers were not following the overscroll behavior set for the toolbar's content when the browser size was reduced, forcing the toolbar to stack.

In {productname} 6.3, the toolbar separator lines behavior was updated to fix the issue. 
...
=== are there any known issues fixes in this release?

// https://ephocks.atlassian.net/browse/[ticket-number-for-any-known-issues]
// cause
// consequence
// fix
// result

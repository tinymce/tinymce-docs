...
// https://ephocks.atlassian.net/browse/TINY-8964 
==== Apply new design options around advcode for `inline` mode users.

In {productname} 6.3, users now have the option to force the Advanced Code Editor within {productname}’s existing editor space, rather than displaying it in a seperate dialog box. 

The benefits for using `advcode` in {productname} 6.3, is that mobile and narrow screen users is the improved benefits to the UX as there is no need to have a `ToggleView` button in the viewport.

Basic setup for `inline` mode users as below.
...
tinymce.init({
  ...
  plugins: [
	...
	advcode_inline: true, // To use this option, set value to "true".
    toolbar: "code"
});
----
...
The `ToggleView` command can, as its name suggest, toggle view visibility.

As well, the `ToggleView` command can be queried for it current state.
...
In messages for the `WindowManager.alert` and `WindowManager.confirm` APIs now sanitize properly.

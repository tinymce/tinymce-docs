...
Previously only a predefined list of merge tags could be used. When the new `mceMergeTag` command is added to `text_patterns`, any text selection content can be converted into a merge tag.

As well, configuring the inline text_pattern with this command allows merge tags to be entered by manual typing.

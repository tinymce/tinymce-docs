...
==== Spelling plugin can crash the service
//#TINY-9391

Previously, the Spell Checker Pro service could be crashed by large requests or by the Spell Checker Pro server not being able to manage the number of requests made in a given time. This latter issue could be exacerbated by the plugin immediately re-sending a failed query.

With this update, Spell Checker Pro now caches outgoing and in-queue requests; filters duplicate requests; and retrieves returned responses for duplicate queries from the cache.

This both limits and reduces the number of requests made to the Spell Checker Pro service, improving the reliability of the service and the plugin.
...
After the issue was fixed, copied content containing a valid white-space property now retains its original formatting when pasted in {productname}.
...
=== Backcolor - Color swatch - Checkmark on selected color is not applied
//#TINY-9435
...
=== Cannot press enter after pasting mention on a newline.
//#TINY-9432
// Mentions
...
=== Encode error message in image dialog
//#TINY-9421
// Plugin - Image
...
=== Lists of different nesting levels should not be under different OLs but merged into the same OL
//#TINY-9333
// Powerpaste
...
=== Lists are not imported correctly
//#TINY-8523
// Powerpaste
...
=== Nested lists result is incorrect numbering when pasting from word
//#TINY-6446
// Powerpaste

...
In {productname}, Templates are now parsed before preview and insertion to make preview consistent with inserted template content and prevent XSS from occuring.
...
After some improvements to the `fontsizeinput` toolbar item, pressing enter in the input field now sets the focus back to the {productname} editor viewport, which allows the new font size to be used within the content body. 

In {productname} 6.4, pressing enter inside the `inputfontsize` input now returns the focus back into the editor content.
// https://notion.so/tinycloud/Four-words-to-live-by-96e307110aaa4137b94f9ba12874d480
...
As a consequence, {productname} introduced a new UI component that give users an option to set the default unit of measure (UOM) for font sizes via the new input field or the plus `+` or minus `-` buttons within the toolbar when the `fontsizeinput` is set within the tinymce.init() such as the below:
...
The release of {productname} 6.4 provides users with new features, such as the impovements to the Advanced Code plugin dialog, which have introduced a group of new buttons to the view bar. A new togglable button has also been added, allowing users to change its icon and active status with the click of a button, and is available in both the modal footer and view header.
...
In {productname} 6.4, when the sidebar is either visible or hidden now passes the accessibility test with Axe-DevTools.
...
{productname} 6.4, now prevents the user from having the ability to drag and drop any descendants of a `contenteditable=false element` within another *noneditable element*.
...
In {productname} 6.4, this issues has be rectified, which now ensures that the foreground and background menu icons now accurately reflect the last used color within the menu dropdown swatch.
...
// https://ephocks.atlassian.net/browse/TINY-9497
In previous versions of {productname} up until the changes and improvements were made with the release of v6.4, the foreground and background menu icons would not properly update to display the last used color, as outlined in the above.
...
In the previous versions of {productname}, when utilizing the `forwarddelete` or delete keys, they would delete the contents inside `noneditable` elements. 

As a consequence, elements nested within a `noneditable` root element set to `contentEditable = false;`, were able to be deleted from the `noneditable` root element.

To fix this issue, {productname} 6.4 introduced various improvements to block all commands and keyboard functions such as `delete/forwarddelete` from being applied to `noneditable` content.  The changes made to {productname} with the release of v6.4, included updates to the editor selection `isEditable` function, which now prioritizes fake selections first.

As a result, in {productname} 6.4, when using the `forwarddelete` or `delete` keys on `noneditable` elements within a `noneditable` element, the contents within the `noneditable` elements will no longer be deleted.
...
// https://ephocks.atlassian.net/browse/TINY-9477
In the previous versions of {productname}, when utilizing the backspace or delete keys, they would delete the contents inside `noneditable` elements.

As a consequence, elements nested within a `noneditable` root element set to `contentEditable = false;`, were able to be removed/deleted from the `noneditable` root element. To fix this issue, {productname} 6.4 introduced various improvements to block all commands and keyboard functions such as `delete/backspace` from being applied to `noneditable` content. 
...
As a result, in {productname} 6.4, when using the backspace or delete function keys on `noneditable` elements within a `noneditable` element, the contents within the `noneditable` elements will no longer be deleted. 
...
In previous versions of {productname}, inserting newlines inside an `editable` element while it was inside a `noneditable` root would sometimes try to split the `editable` element. 

As a consequence, when child block elements set as `editable` such as a `<p>` tag were nested within the `noneditable` element root, the editor would try to split the child block element when the user hit the `enter` command. 
...
In this particular instance, child elements in {productname} such as the `<p>` tag would split the block element resulting in a new block (such as a new paragraph) to be created inside the paragraph when a user attempted to edit it inside a `noneditable` root element. 
...
As a result, when a root element is set to `noneditable`, any `editable` elements inside that root element such as `<p>` tag will no longer try to split the `editable` element. 
...
In previous versions of {productname}, dragging and dropping `noneditable` elements on table borders would create issues during the drop action.

As a consequence, this would result in the `noneditable` element being deleted after the  `action` when dropped onto the borderline within the table cell.
...
In previous version of {productname}, dragging `noneditable` elements into a `noneditable` root would remove the elements `contentEditable = 'false';` attributes.

As this is not the expected behaviour for a `noneditable` element or root, this bug in particular, created issues in situations such as: *(a)* when the editor body was set to `contentEditable = 'false';`, and *(b)* when a `non editable` element is dragged and dropped into a table element that is within a `non editable` root, which would result in the `noneditable` attributes blocks being deleted from the element that was dropped.
...
In {productname} 6.4, `noneditable` elements now preserve their `contentEditable = 'false';` attributes if they are dragged and dropped within a `noneditable` root.
...
In a `noneditable` root, when selecting multiple lines from lists, it was possible to add formatting to the selection such as `*bold*` and `_italic_`. As a consequence, this would allow contents that are considered `noneditable` to be modified which is not the expected behaviour for `noneditable` elements within the root.

To fix this bug, {productname}, disabled list formatting, if the list is in within a `noneditable` root

In {productname} 6.4, a selection of multiple lines from lists can no longer be formatted when inside a `noneditable` root.
...
While applying a fix for this issues, it was identified that this was a regression created when {productname} fixed issue: _`Change the annotation api to avoid dom mutation in plugins using it`_, which adjusts the selection in the way that it is _impossible_, to retain the selected word. 

However, with the release of {productname} 6.4, after immediately deleting annotations after they are created no longer will be retained on the text selection.
...
In {productname} 6.4, pasting blocks that are invalid children of a `editable` block will no longer split that block, but instead unwrap the blocks to fit within the specific `editable` block.
...
In {productname} 6.4, dragging and dropping elements that are `noneditable` out of they're parent blocks now correctly pad the parent block element.

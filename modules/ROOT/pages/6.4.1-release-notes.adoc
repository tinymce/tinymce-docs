...
To fix this issue, {productname} 6.4.1, when the editor content window is `infocus`, the editor will render a visible `highlighted` border around the content window when the option is added to the `tinymce.init()` config as below.

...
The {productname} 6.4.1 release includes an accompanying release of the **Accessibility Checker** premium plugin.

**Accessibility Checker** 3.1.1 includes the following addition:

==== Updated the **Accessibility Checker** dropdown menu styles to match the default {productname} styles
//#TINY-9630

Previously, the **Accessibility Checker** premium plugin used browser-native elements for some dropdown menus.
...
=== Advanced Table 2.0.3

The {productname} 6.4.1 release includes an accompanying release of the **Advanced Table** premium plugin.

**Advanced Table** 2.0.3 includes the following addition:

==== Updated the **Advanced Table** dropdown menu styles to match the default {productname} styles
//#TINY-9630

Previously, the **Advanced Table** premium plugin used browser-native elements for some dropdown menus.

With this update, these menus are now created using the equivalent {productname} custom element, making these dropdown menus present with the same styling as other {productname} UI elements.

For information on the **Advanced Table** plugin, see xref:advtable.adoc[Advanced Table].
...
As a result, the input box for the **Comments** premium plugin now offers these improvements with the most recent release of {productname} 6.4.1.
...
In previous versions of {productname}, the `mceMergeTag` command would not wrap the selected content within the merge tag prefix and suffix when the content length exceeded the character limit for the merge tag.

As a consequence, when the `text_patterns` feature was enabled and a user entered a too-long merge tag, the prefix and suffix of the tag were excluded from the inserted content.

To address this problem, improvements to the `mceMergeTag` command were introduced with the release of {productname} 6.4.1. Now, if the length of the selected content exceeds the character limit for the merge tag, the content will be inserted as a plain text, including the prefix and suffix.

[NOTE]
If any content is embraced with a merge tag prefix and suffix but exceeds the character limit for merge tag content length, it will not be converted into an official merge tag.
...
The {productname} 6.4.1 release includes an accompanying release of the **Page Embed** premium plugin.

**Page Embed** 2.1.1 includes the following addition:

==== Updated the **Page Embed** dropdown menu styles to match the default {productname} styles
//#TINY-9630

Previously, the **Page Embed** premium plugin used browser-native elements for some dropdown menus.
...
The {productname} 6.4.1 release includes an accompanying release of the **Permanent Pen** premium plugin.

**Permanent Pen** 2.0.3 includes the following addition:

==== Updated the **Permanent Pen** dropdown menu styles to match the default {productname} styles
//#TINY-9630

Previously, the **Permanent Pen** premium plugin used browser-native elements for some dropdown menus.
...
Improvements were made to add a content filter for lists within {productname} 6.4.1. This improvement meant that any _invalid child nodes_ nested within `<li>` elements would be wrapped with this filter to avoid the `Uncaught TypeError:`.

In {productname} 6.4.1, direct _invalid child text nodes_ of list elements are now wrapped in list item elements, which allow the user to now `Tab` nested text nodes that are within `<ol>/<li>` tags.
...
In {productname} 6.4.1, Templates are now parsed before preview and insertion to make preview consistent with inserted template content and prevent XSS from occuring.
...
In {productname} 6.4.1, pressing enter inside the `fontsizeinput` input now returns the focus back to the editor content body.
...
In previous versions of {productname}, when the body was set to `contenteditable="false"` content could still be inserted with the `insertContent` API.

Example content:
[source, HTML]
----
<body contenteditable="false"><p>Try to insert content here</p></body>
----

After selecting the space between "insert" and "content", executing `tinymce.activeEditor.insertContent('text')` would incorrectly insert the text which should not be possible.

[source, HTML]
----
<body contenteditable="false"><p>Try to inserttextcontent here</p></body>
----

{productname} 6.4 includes a new `isEditable` method on both the `editor.selection` and `editor.dom` APIs which are used to block the `editor.insertContent` API and `mceInsertContent` command when the selection covers a `contenteditable="false"` element.
...
The new additions and improvements with the introduction of {productname} 6.4.1, include a new `isEditable` API been added to the `editor.dom` which now blocks the `editor.insertContent` API and `mceInsertContent` command when the contents return a `true` value if the editor detects the element is a `contenteditable="false";` in a `contenteditable="false";` root.
...
In {productname} 6.4.1, two new additions were added to the menu button API.
...
As a result, in {productname} 6.4.1, menu items can change based on the parent menu button `api` properties.
...
To fix this issue, {productname} 6.4.1, introduced some improvements to ensure that when the editor content window is `infocus`, the editor will render a visible `highlighted` border around the content window when the below option is added to the `tinymce.init()` config.
...
In {productname} 6.4.1, when the ` highlight_on_focus: true` option is set, the editor will render the highlighted border for the active editor window.
...
A focusing event that was in the initial *FloatingToolbarButton* config, in previous versions of {productname} in (2019) was causing `focus` issues with the *ToggleToolbarDrawer*. As a consequence, this meant that while the editor was scrolled into `focus`(viewport position), and the *ToggleToolbarDrawer* command `skipFocus: boolean` was set, the toolbar draw could still take `focus` when it was not meant to.

In {productname} 6.4.1, an improvement was made to the focusing `event` which is now removed after toggling the toolbar.
...
To fix this bug, {productname} 6.4.1 introduced a new check that now verify if the link plugin is opened, which will now block the context menu on `table of contents` from opening upon when the `toc` is clicked. 

As a result, the context menu for the `table of contents` now behaves as expected in {productname} 6.4.1.
...
In {productname} 6.4.1, when the sidebar is either visible or hidden it now passes the accessibility test with Axe-DevTools.
...
In {productname} 6.4.1, all `renderUI` property loading issues have been resolved.
...
In {productname} 6.4.1, when the color picker in the `toolbar` is updated when changing either the `forecolor` or `backcolor` from the `menubar`.
...
As a result, the checklist `icon` in {productname} 6.4.1, related to the `advlist` plugin will only appear in a `active` state, when the `node` checklist meets the new conditions implemented.
...
Changes were implemented in {productname} 6.4.1 to fix this issue to ensure that any invalid child anchor elements that try to reproduce this action are unwrapped when the editor verifies another transparent has been dropped into another transparent (anchor) element.

In {productname} 6.4.1, dragging content such as `links` into other `links` will now produce *valid* HTML `output` after a drag operation occurs which will be consistent across all browsers.
...
In {productname} 6.4, two new key overrides have been added to address the second and third method:
...
* For Method 3: When one of the keyboard shortcuts from Method 3 is detected on `keyup`, the override creates an unformatted caret at the current selection. Performing the override on `keyup` allows the native behaviour of the deletion shortcuts to occur. Then, applying an unformatted caret at the selection after deletion will prevent the browser from recreating any formats.
...
In order to fix this bug, {productname} 6.4.1, not detects if the user inserts a `#` symbol before the hex `value`, which removes the leading `#` prefix before rendering to the color picker component.
...
{productname} 6.4.1, introduced a blocker that is executed when tables are inside a `contenteditable="false"` element root.

In {productname} 6.4.1, all table UI actions, commands and selections have been `disabled` for tables when either the table or the editor body have been set as `contenteditable="false"` elements.
...
To fix this {productname} 6.4.1, added a new function to the Utils API that filters any content that is considered as a `fake caret` element, which now disables the users ability to add/insert or modify the content contained within the `contenteditable="false"` element node space.

As a result, any `contenteditable="false"` elements that are nested within another `contenteditable="false"` element root that are nested within `table` elements, can no longer be edited in {productname} 6.4.1.
...
To correct this issue, {productname} 6.4.1, introduced a new text-area wrapper that fixes the issues with the `border-color, radius, style and width` that directly affects the `focus` render element for the dialog.  In addition `overflow: hidden`, has been added to clip the scrollbars to the border-radius.

As a result, the `focus` dialog border now renders correctly in {productname} 6.4.1.
...
Inline boundaries for `contenteditable="false"` elements were rendering styling in previous versions of {productname} 6.4.1 after the user `selected` before and after the `contenteditable="false"` elements node. The expected behaviour is that `contenteditable="false"` elements should *not* have inline boundaries when a caret is inside an `contenteditable="false"` anchor or root.
...
In {productname} 6.4.1, it is now not possible to highlight inline boundary elements that are classified as a `contenteditable="false"` content element.
...
To resolve this issue, {productname} 6.4.1 included several enhancements to prevent all commands and keyboard functions, including delete/forwarddelete, from affecting `contenteditable="false"` content.

Consequently, in {productname} 6.4.1, using the forwarddelete or delete keys on `contenteditable="false"` elements within a `contenteditable="false"` element will no longer delete the contents within the `contenteditable="false"` elements.
...
As a consequence, elements nested within a `contenteditable="false"` root element set to `contenteditable="false"`, were able to be removed/deleted from the `contenteditable="false"` root element. To fix this issue, {productname} 6.4.1 introduced various improvements to block all commands and keyboard functions such as `delete/backspace` from being applied to `contenteditable="false"` content. 


As a result, in {productname} 6.4.1, when using the backspace or delete function keys on `contenteditable="false"` elements within a `contenteditable="false"` element, the contents within the `contenteditable="false"` elements will no longer be deleted. 
...
In {productname} 6.4.1, anchor elements are included inside `lists` while nested within table cells.
...
To fix this issue, {productname} 6.4.1, introduced a new `isActive` api to the handler for dragger, that checks if the event `isActive` or not, which then hides the resize bar during the dragging event, which now prevents content from being dropped onto table bars.

In {productname} 6.4.1, dragging an inline `contenteditable="false"` element onto the border inside a table, now moves it into a neighbouring cell instead of deleting it.
...
A fix for this bugs was implemented in {productname} 6.4.1, that corrected this issue, resulting in the `inline` dialog working as expected with this release.
...
As a fix, {productname} 6.4.1, added a new condition which validates a set of conditions the check if the parent block element is within a `parentNode` and is a block element, in which it will trigger a action to remove elements with padding that meet this condition. 
...
In {productname} 6.4.1, the events are now simulated based on the mouse event that produced them, meaning that the target and other properties, such as mouse coordinates, are now properly produced for the simulated drag and drop events.
...
In {productname} 6.4.1, a fix was applied that replaced the previous selector, `aria-checked='true'``, with `tox-collection__item--enabled`. This fix, ensures that the checkmark is only displayed on the enabled colors.
...
With the release of {productname} 6.4.1, the new `ui_mode` option provides improvements to editors within scrollable contexts, including hiding the toolbar when the editor is out of view when the `ui_mode` option is set to 'split'.

=== `color_cols`: 0 Memory Leak
//#TINY-9649

In previous versions of {productname}, the color swatch would attempt to display 0 columns of colors when the relevant `color_cols`, `color_cols_foreground` or `color_cols_background` options were set to 0. As a consequence, the tab would crash when opening that color swatch.
...
Recently an issue was discovered which affects keyboard navigation of the toolbar. The issue occurs when the "More" toolbar is opened and contains a "Menu Toolbar Button", such as the `table` or `language` toolbar buttons, when a user attempts to use the "tab" key to navigate between toolbar groups, any Menu Buttons not at the beginning of a toolbar group will incorrectly be navigable. The correct behaviour would involve these buttons only being navigable via the arrow keys.

...
In {productname} 6.5, instructions for adjusting the size of the editor using arrow keys are now announced upon focusing the editor resize handle, while using voice assistant software.

NOTE: In {productname} 6.5 Enterprise, this announcement is only available in English. Additional translations will be added in a future release.
...
=== Screen readers are currently only supported in English ('en') when announcing instructions for resizing the editor.
//#TINY-9793


In {productname} 6.5, introduced announcing instructions for adjusting the editor's size using arrow keys to support screen readers.

However, there is a known issue where this new instruction is currently only available in English ('en') when resizing the editor. 

NOTE: that future releases of {productname} will include additional translations to provide broader support.
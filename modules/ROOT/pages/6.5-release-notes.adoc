...
=== The `copy code` button wasn't correctly styled within the Advanced Code Editor.
//#TINY-9698

In {productname}, implementation for the new "Copy code" button was styled incorrectly which meant it was missing the grey background similar to the other buttons.

This issue was a oversight which meant the setting for `borderless: true` should have been set to `false`. As a consequence, the button wasn't correctly styled. 

In {productname} 6.5, the `borderless: true` setting has now been removed and the button is now correctly styled.

...
=== Quickbar toolbars was shown for non-editable contents in a non-editable root.
//#TINY-9460

In previous versions of {productname}, when the document root is set to `contenteditable="false"` the quickbars toolbar would appear when you select `contenteditable="false"` content.

A fix was applied to the quickbars toolbar in {productname} 6.5 which will set itself to `hidden` if the current selection is in a `contenteditable="false"` element such as the root.

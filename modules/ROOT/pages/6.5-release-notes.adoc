...
In previous versions of {productname}, an issue was identified when users right-clicked on a merge tag.

The result of this action was that the highlights within the merge tag selection would vary based on the browser, leading to an inconsistent experience for users. To address this problem, {productname} 6.5 implemented a fix by removing the browser's default highlighting of the merge tag. Instead, {productname} introduced our own styles specifically designed to highlight the merge tag consistently across different browsers.

As a result, users can now enjoy a uniform and improved merge tag selection experience across all browsers.

...
=== Context toolbars would display the incorrect status for the `advlist` plugin buttons.
//#TINY-9680

In previous versions of {productname}, a issue was caused by a discrepancy between the setup handler for the `advlist` quickbar button in the `advlist` plugin compared to the one in the `list` plugin.

Consequently, the status of the `advlist` quickbar button was not being updated correctly during the setup.

To resolve this, the setup handler for the `advlist` plugin was aligned with the one used in the `list` plugin. As a result of this fix, the status of the buttons is now accurately rendered.

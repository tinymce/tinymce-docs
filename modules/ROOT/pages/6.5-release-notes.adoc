...
In previous versions of {productname}, after pressing backspace occasionally would delete an image after the cursor instead of the expected deletion before it.

This issue occurred when the cursor was positioned between two image elements and as a consequence, the wrong image was unexpectedly deleted.

As a result of the fix applied in {productname} 6.5, the expected image is now deleted as intended.

...
Previously, when a {productname} document had an inline element with a `contenteditable="false"` attribute set inside a block block element that had a text direction of right-to-left, the insertion point would move in the wrong direction when traversing the inline element using arrow keys.

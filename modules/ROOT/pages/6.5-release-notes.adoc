...
In previous versions of {productname}, an issue was identified in the close buttons where invalid markup was present. Specifically, a div element was being used within the buttons, which violated HTML standards. This caused problems when running Tiny's HTML code through markup checkers, resulting in the generation of errors.

In {productname} 6.5, the markup was fixed by replacing the div element with a `<span>` element, aligning it with proper HTML guidelines. As a result, the updated close buttons now pass HTML markup checkers without any errors. 

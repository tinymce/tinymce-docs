...
In {productname} 6.5, a `beforeinput` event and an `input` event, both with an “insertFromPaste” `inputType` property, are now fired in order when content is pasted into {productname}. In addition, the `beforeinput` event will also hold a `DataTransfer` object which contains the `text/html` data being pasted.

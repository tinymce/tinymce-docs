...
=== Initializing the editor with a table at the start would display resize handles even when the editor wasn't focused.
//#TINY-9748

In previous versions of {productname}, resize handles would show up for tables in the editor even when they weren't focused. This happened because the cursor automatically landed in the table during the initialization process for the editor during startup.

In {productname} 6.5, the resize handles have been adjusted to prevent their display when the caret is inside a table without the editor being focused.

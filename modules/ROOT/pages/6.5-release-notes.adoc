...
=== It was possible to modify `contenteditable = "false"` text elements using formatpainter
//#TINY-9472

In previous versions of {productname}, the format painter tool previously allowed users to modify 'contenteditable = "false"' text elements.

As a consequence, the format painter would apply formatting changes to the 'contenteditable = "false"' content.

In {productname} 6.5, the format painter no longer affects 'contenteditable = "false"' text elements and the original formatting of the 'contenteditable = "false"' content remains intact.

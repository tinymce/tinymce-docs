...
=== The destination category list would include the template's own category in the "Move template" dialog.
//#TINY-9774

In previous versions of {productname}, when users attempted to move a template to another category, they encountered a "Move to" dialog with a dropdown list of categories. However, this list included a redundant category that matched the parent category of the template being moved. If the selected destination category in the "Move to" dialog matched the original template category, the operation had no effect, resulting in wasted time.

In {productname} 6.5, the destination category list now filters out the category of the template being moved.

With this fix, users will no longer see the parent category of the template in the destination category list.

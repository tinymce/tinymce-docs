...
**Power Paste** 6.1.5 includes one fix:

=== Existing tables were not filled out when pasting tables from Word and Excel.
//#TINY-9500

In previous versions of {productname}, when the PowerPaste plugin was active and a table was copied from Microsoft Word or Excel into a {productname} editor, the copied table would be pasted as a whole table into one cell, rather than merging the content of the copied table into the cells of the existing table.

In {productname} 6.5, this issue has been fixed.

As a result, when a table is copied and pasted into an existing table within {productname} editor, it correctly fills the cells of the existing table.

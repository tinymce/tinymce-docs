...
In previous versions of {productname}, when the document has a `contenteditable = "false"` element in between the `RTL` text in a `RTL` block element, the caret would incorrectly move to the wrong direction when moving over the `contenteditable = "false"` element.

To fix this issue, updates were made that now flips the direction if the block element is `RTL`.

As a result of this fix, the caret now moves in a reverse logical order when moving over a `contenteditable = "false"` elements similar to how the visual caret movement works when moving inside `RTL` text.

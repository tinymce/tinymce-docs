...
In the latest version, {productname} 6.5, the Directionality plugin has been improved. It now takes into account the `direction` style while still giving priority to the `dir` attribute. First, any inline `direction` style is removed if present. Then, the standard `dir` attribute logic is applied. Finally, it determines if an inline `direction` style is needed for the desired visual change in `direction`. The final addition of an inline `direction` style often occurs if the element derives a different style from a stylesheet.

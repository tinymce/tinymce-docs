...
In previous versions of {productname}, there was an issue with exiting a blockquote element when the cursor was at its end. This problem arose because when inserting a new block, the check to determine if the block is empty considered nodes with zero-width space (zwsp) as non-empty.

Consequently, a block with only an empty format was mistakenly regarded as non-empty. To address this, the check has been updated to recognize (zwsp) as an empty block.

As a result, blocks with empty formats in {productname} 6.5 are now correctly recognized as empty.

...
=== Applying lists did not work if the selection included a `contenteditable=false` block element.
//#TINY-9823

In previous versions of {productname}, applying lists would not work if the selection included a `contenteditable="false"` block element. This was due to the fact that if there was a non-editable element in the text-selection, inserting or applying a bullet or numbered list to the selection would fail. As a consequence, when the user pressed the insert list button while a non-editable element was included in the selection, nothing would happen.

To resolve this issue, the logic for inserting lists was updated in the latest version of {prouductname}. The new approach ensures that everything within the selection, except for the non-editable element, is turned into a list. With this fix in place, when the user presses the insert list button, all elements within the selection, excluding the non-editable one, will be transformed into a list.

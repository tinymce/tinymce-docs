...
In previous versions of {productname} it was mandatory for the `DialogFooterToggleButton` to have an icon prop. As a consequence the `DialogFooterToggleButton` would not function without the icon.

Improvements were made in {productname} 6.5, that now allows the icon prop to be optional.

...
In previous versions of {productname}, when transparent elements had whitespace between them, it would mistakenly convert it into an empty paragraph. This issue occurred when adding a document with multiple adjacent block anchors at the document's root.

As a consequence, an extra paragraph would be inserted between each block.

In {productname} 6.5, the extra paragraph is no longer inserted between the block elements.

...
=== A selected item within its parent directory in tree components, would not retain as a highlighted selection after collapsing its directory.
//#TINY-9715

In previous versions of {productname}, selecting a leaf component within its parent directory would loose its state selection after collapsing and re-expanding the parent directory.

A fix for this was implemented in {productname} 6.5, which now tracks the active state for a selected leaf component before and after collapsing or re-expanding the parent directory.

...
=== No longer allow attempting to fix a table's lack of headers if the table has no cells at all.
//#TINY-9753

In previous versions of {productname}, fixing a table's lack of headers is now **disabled** if the table is empty.

Previously, attempting to fix this error without any cells resulted in an internal error, although the user didn't notice any difference.

In {productname} 6.5, this fix now prevents users from attempting to fix the error when there are no rows in the table.

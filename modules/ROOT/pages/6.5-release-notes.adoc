...
In previous versions of {productname}, when using voice assistant software when focused on the editor's resize handle it would not announce any instructions to the user.

In {productname} 6.5, the voice assistant software will now announce instructions  to the user for adjusting the size of the editor, by using directional arrow keys such as `↑` arrow up, `↓` arrow down ,  `→` arrow right,  and  `←` arrow left, when the editor resize handle is focused.

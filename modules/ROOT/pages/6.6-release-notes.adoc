...
As of this update, it is possible to configure a custom dialog to not render the footer section.

When defining a modal or inline dialog, setting the `buttons` property to `undefined` or to `[]` (an empty array), results in a dialog that displays without rendering the section.

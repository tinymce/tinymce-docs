...
During rapid, intermittent, content updates in an iframe dialog component with `streamContent: true` set, an error — "Uncaught TypeError: Cannot read properties of null" — would sometimes be thrown when trying to maintain scroll at the bottom of the iframe component. This was due to the iframe body being inconsistently `null` depending on whether it has loaded when the programmatic scrolling occurs, which relies on the iframe body's scrollHeight as the scroll value.

...
// TINY-10213
Previously, when a user attempted to indent or outdent an element nested within a list, the list was converted into a model which resulted in it being recreated from that model.

As a consequence of this, the model would not handle comments at the `<li>` element level, which resulted in content deletion of comments nested within `<li>` structures like:
...
In addition, when the editor would try to manage comments within a `<li>` element, a console error would be thrown resulting in all other elements being ignored.

...
The {productname} 6.7.3 release includes accompanying changes affecting the {productname} **self-hosted** services for the following plugins:

* For **Enhanced Media Embed** plugin `mediaembed` and the **Export** plugin `export`.
** FIXED: https://nvd.nist.gov/vuln/detail/CVE-2023-44487[CVE-2023-44487] for Netty. Upgrading to version 4.1.100.Final
** Service codebase now it's based on Scala 2.13.12 (instead of Scala 2.12.16).
* For **Enhanced Image Editing** plugin `editimage` and the **Link Checker** plugin `linkchecker`.
** FIXED: https://nvd.nist.gov/vuln/detail/CVE-2023-44487[CVE-2023-44487] for Netty. Upgrading to version 4.1.100.Final
** Service codebase now it's based on Scala 2.13.12 (instead of Scala 2.12.16)
** Server side cache implementation was refactored to use Scaffeine, a thin Scala wrapper for Caffeine which is very well known and stable Java caching library offering a great performance and support
** Include code refactors to improve code abstraction and encapsulation
* For **Spell Checker Pro** plugin `tinymcespellchecker` and the **Spelling Autocorrect** plugin `autocorrect`.
** Service codebase now it's based on Scala 2.13.12 (instead of Scala 2.12.16).
** Major code refactors to improve maintainability and performance.
** Server side cache implementation was refactored to use Scaffeine, a thin Scala wrapper for Caffeine which is very well known and stable Java caching library offering a great performance and support.

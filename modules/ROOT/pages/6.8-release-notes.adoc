...
As a consequence, the `isInSummary` predicate could not detect the selection at the beginning of the details element and would fail to toggle the Accordion component on **Enter** key press.

{productname} 6.8 addresses this issue, now, the predicate considers that the selection can be right before the summary element, so the Accordion toggling can works as expected on Safari.

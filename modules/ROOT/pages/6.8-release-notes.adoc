...
This property allows the integrator to define shortcuts with `selection: true` which should only operate on context, and when the user has selected content. Alternatively, shortcuts with `selection: false` will only be used with no selection, allowing the integrator to define shortcuts for generating content from a specific prompt, without any context.

...
// TINY-10178
Previously, when running **Powerpaste 6.2.3** and copying content from Microsoft Word that contains text content such as the below:
...
after pasting the content into the {productname} editor, the editor would **insert** a unexpected `<s>` tags around the `+&nbsp;+` such as `<p><s>&nbsp;</s></p>` which would render a visual `-` "strikethough" between the two paragraph's.

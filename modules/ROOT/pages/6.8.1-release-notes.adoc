...
// #TINY-10385
Previously the `schema.isInline` and `schema.isWrapper`, would also consider elements such as `#text` as `valid_elements`.

As a consequence, in some cases the editor would try to append an element inside a `#text` element, which is forbidden.

{productname} 6.8.1 addresses this issue, now, the `schema.isInline` and schema.isWrapper exclude elements when there name starts with `#` such as: `#text` and `#comment`.

As a result, these elements are no longer considered as a valid wrapper, and can no longer append an element inside of elements such as these.
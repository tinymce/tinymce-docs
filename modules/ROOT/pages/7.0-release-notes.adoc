...
Currently, tab navigation within the Enhanced Code Editor is not working as expected when Safari is the host browser, resulting in a loss of tab functionality.

Currently, there is no workaround for this issue.

=== Revision History highlights `table` content with `colgroup` as new.

When a `table` element has `colgroup` attribute, Revision History highlights it as new even though it has no change.

Currently, there is no workaround for this issue.

=== Revision History has no integration for saving version.

TODO//

For now, a workaround is to add a custom toolbar button.

...
{productname} 7.0 introduces enhancements to the existing xref:content-filtering.adoc#custom_elements.adoc[`custom_elements`] option in {productname}, now featuring the `addCustomElements` API. This update refines the structure of custom elements, offering users more flexibility and customization options.
...
For more information and details for `addCustomElements` see xref:content-filtering.adoc#improved_custom_elements.adoc[improved_custom_elements].

...
During the integration process, {productname} made necessary adjustments to ensure compatibility between {productname} 7.0 and CodeMirror 6. Specifically, refining our CSS reset rules to accommodate the dynamically generated styles of CodeMirror 6.

More information about CodeMirror 6 will be included in future patch releases.

=== Moved CodeMirror 6 inside ShadowDom.
// #TINY-10561

With the release of {productname} 7.0, CodeMirror 6 has been integrated into the shadow DOM. This update from CodeMirror 5 to v6 should have no impact on customers utilizing the Enhanced Code Editor feature.
...
==== Pressing the Escape key while having the autocompleter open will no longer close the code dialog.
// #TINY-9205

...
[NOTE]
{productname} 7.0 is _licensed under GPL Version 2 or later_. This version introduces a new xref:license-key.adoc[licence_key]  configuration setting that gives self-hosted users the ability to select a usage under the GPL or to authenticate their paid license with Tiny.
...
There are four known issues in {productname} 7.0.
...
=== Tab navigation inside the Enhanced Code Editor currently does not function as expected when the host browser is Safari.
// #TINY-10596

Currently, tab navigation within the Enhanced Code Editor is not working as expected when Safari is the host browser and the editor configuration is set to `advcode_inline: true`, resulting in a loss of tab functionality.

.Example of configuration affected.
[source, js]
----
tinymce.init({
...
* There is no integration for saving a version yet. For now, a workaround is to add a custom toolbar button. For example:

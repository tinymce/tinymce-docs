...
In {productname} 7.0, the `media_url_resolver` has been refactored to use promises instead of callbacks. This change brings standardization to the code base and improves error handling.

// TODO: demo to show the function changing from `(data, resolve, reject) => void` to `(data) => Promise<result>`

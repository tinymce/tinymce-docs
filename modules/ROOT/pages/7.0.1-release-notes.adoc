...
In {productname} {release-version}, this issue has been fixed. The editor now checks for non-default root nodes, such as `forced_root_block: div`, when creating new list items. As a result, the caret is placed correctly after the creation of the new list item, allowing the editor to add a list item instead of generating a new list.

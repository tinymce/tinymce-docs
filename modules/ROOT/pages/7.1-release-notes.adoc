...
{productname} {release-version} introduces the new `revisionhistory_diff_classes` option. This option allows the integrator to customize the CSS classes applied to change annotations in the Revision History feature. This configuration works in conjunction with the `revisionhistory_css_url` option, which provides a way for integrators to define custom styles for changes such as additions, removals, and modifications.

[NOTE]
The default configuration uses predefined class names (tox-revisionhistory__annotation--added, tox-revisionhistory__annotation--removed, and tox-revisionhistory__annotation--modified), which can be overridden with custom classes to suit the specific style requirements, such as the below example.
...
tinymce.init({
  ...
  revisionhistory_css_url: './revisionhistory.css', //required

...
In {productname} {release-version}, a new `revisionhistory_fetch_revision` option has been added to enable updating a revision after the initial fetch. If configured, the Revision History only uses the option to update for when either the selected and the closest revisions have no content. This occurs before generating the differences between the two revisions.

NOTE: As part of this addition, the `content` property is now optional. For the fetched revisions, empty string is considered as valid content.

NOTE: The integrators still need to configure the `revisionhistory_fetch` option to provide the initial revisions.
...
This option would be helpful when dealing with revisions that have large content or preferring to only load the content when required.

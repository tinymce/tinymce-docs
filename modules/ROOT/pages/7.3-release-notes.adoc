...
Previously in {productname}, sequential HTML comments would generate unwanted elements in the editor. For example, inserting the following HTML into the editor would result in an `+&nbsp;+` element being inserted:

.Unwanted elements generated in the editor
[source, html]
----
<div>a</div>
<!-- comment -->
...
As a consequence, whitespace was added between the comments, resulting in extra paragraphs being generated in the editor.

In {productname} {release-version}, this issue has been resolved. The editor now correctly handles sequential HTML comments by removing the whitespace before parsing.

.Resolved sequential HTML comments whitespace
[source, html]
----
<div>a</div>
<!-- comment --><!-- comment -->

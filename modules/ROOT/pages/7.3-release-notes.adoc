...
In {productname} {release-version}, the **Revision History** premium plugin now includes labels for the `initial` and `draft` revisions. These labels are displayed in the Revision History sidebar to help users identify the `initial` and `draft` revisions if they are present.

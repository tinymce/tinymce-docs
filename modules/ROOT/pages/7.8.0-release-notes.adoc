...
In {productname} {release-version}, a new `+tinycomments_fetch_author_info+` callback option has been introduced, enabling integrators to provide author information in callback mode. This enhancement ensures accurate author data is displayed in new conversation cards. Additionally, author data is now pre-fetched during comment initialization to ensure immediate availability.

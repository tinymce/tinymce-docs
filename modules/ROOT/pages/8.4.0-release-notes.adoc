...
Behind the UI sits a production-ready **TinyMCE AI** service with JWT authentication, granular permissions, and multiple models, plus the **TinyMCE AI API** for the same capabilities in server-side workflows, automation, and anywhere outside the editor.

**What this means for users** (the outcomes the interface is built for):

* Move first drafts forward and refine tone without leaving the editor.

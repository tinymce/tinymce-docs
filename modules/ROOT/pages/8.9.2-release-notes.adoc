...
Previously, the xref:revisionhistory.adoc#diff[`+diff+`] API compared the two HTML strings exactly as supplied, so the whitespace used to format them registered as content. Two fragments that rendered identically, such as the same table indented differently, were reported as a replacement instead of as unchanged content.

In {productname} {release-version}, the `+diff+` API normalizes both strings before comparing them, so formatting whitespace no longer registers as a difference. Fragments that render identically are now reported as unchanged.

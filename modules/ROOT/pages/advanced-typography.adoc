...
The {pluginname} plugin, depending on it setting, automatically or interactively replaces common typewriter-style characters with their professional typesetter’s equivalents.

For example, it can automatically replace the double-hyphen (`--`) with an em-dash (`—`). And it can automatically replace the teardrop apostrophe (`'`) with opening and closing quote marks (`‘` and `’`).

...
The following tables show the existing editor commands. These commands are provided by `tinymce` and not by the browser's internal commands. These commands can be executed using the link:{baseurl}/api/tinymce/tinymce.editorcommands/#execcommand[execCommand] function.
...
The following commands require the xref:plugins/premium/comments.adoc[Comments (`tinycomments`)] plugin.
...
| mceToggleEditor
| Runs mceAddEditor if an editor is not detected for the specified ID, otherwise it runs either link:{baseurl}/api/tinymce/tinymce.editor/#hide[hide] if the editor is visible or link:{baseurl}/api/tinymce/tinymce.editor/#show[show] if it is not visible.
|===
...
The following command states can be queried using the link:{baseurl}/api/tinymce/tinymce.editorcommands/#querycommandstate[queryCommandState] API.
...
The following command values can be queried using the link:{baseurl}/api/tinymce/tinymce.editorcommands/#querycommandvalue[queryCommandValue] API.

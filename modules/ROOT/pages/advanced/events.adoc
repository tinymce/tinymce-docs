...
| SwitchMode
| `{ mode: string }`
| Fired when the editor mode is changed. The available modes are "design" and "readonly". Additional modes can be registered using {productname} API link:{baseurl}/api/tinymce/tinymce.editormode/#register['tinymce.activeEditor.mode.register()'].
...
| Dirty
| N/A
| Fired when the editor transitions from a "pristine" state to a "dirty" state. The editor is "dirty" when link:{baseurl}/api/tinymce/tinymce.undomanager/[an undo level] has been _created_ since initialization or the last saved state. To check if the editor is "dirty", use the link:{baseurl}/api/tinymce/tinymce.editor/#isdirty[`editor.isDirty` API].
...
| Fired when a change to the editor progress state is scheduled using the {productname} API link:{baseurl}/api/tinymce/tinymce.editor/#setprogressstate[`tinymce.activeEditor.setProgressState()`].
...
| Fired when an attribute is updated using the editor link:{baseurl}/api/tinymce.dom/tinymce.dom.domutils/[DOMUtils API].

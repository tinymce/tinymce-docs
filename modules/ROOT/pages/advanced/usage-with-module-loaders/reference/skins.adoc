...
IMPORTANT: The handling of content CSS files (such as `content.css` or `content.min.css`) varies between bundling tools. View the relevant guide for the required syntax at xref:advanced/usage-with-module-loaders/index.adoc[Bundling {productname} with a module loader].

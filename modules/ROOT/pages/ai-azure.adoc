...
:navtitle: Azure OpenAI Service integration guide
...
Azure OpenAI co-develops the APIs with OpenAI. Therefore, the following examples should be compatible if using Azure OpenAI REST API endpoints or OpenAI API endpoints.
...
A proxy is required if making `fetch` calls directly to the Azure OpenAI REST API as otherwise CORS errors arise. In general, using a proxy is highly recommended even if network errors did not occur. See the xref:ai-proxy.adoc[AI Proxy Server reference guide] for information on how to setup a proxy server for use with the {pluginname}.
...
const AZURE_OPENAI_ENDPOINT = '<INSERT_ENDPOINT_URL_HERE>'; // e.g. https://{your-resource-name}.openai.azure.com/openai/deployments/{deployment-id}/chat/completions?api-version={api-version}

...
:keywords: example, demo, custom, plugin, ai, assistant, guide, google, gemini
...
Vertex AI provides a `systemInstruction` field in the request body that allows controlling the output of the AI model. Google AI Studio does not currently provide this field, so the system messages are provided as part of the user prompt.

TIP: Google AI Studio and Vertex AI can both be used to generate string and streaming responses.
...
This example demonstrates how to integrate the {pluginname} plugin with the https://ai.google.dev/[Google AI Studio] REST API to generate string responses. Google AI Studio is ideal for quickly integrating the Google Gemini models into an application with the minimal setup required.

See https://ai.google.dev/gemini-api/docs/get-started/rest[Google AI Studio Gemini API] for more information on the Google AI Studio Gemini API.
...
This example demonstrates how to integrate the {pluginname} plugin with the https://cloud.google.com/vertex-ai[Google Cloud - Vertex AI] REST API to generate streaming responses. Vertex AI is more powerful and flexible than Google AI Studio, but requires more setup to integrate the Google Gemini models into an application.

The https://www.npmjs.com/package/google-auth-library[`google-auth-library`] can be used to programmatically generate an access token for the Google Cloud API. The access token is used in the Authorization header to authenticate the request to the Google Cloud API. The access token should be stored securely and not exposed to the client side integration.

Alternatively, the https://cloud.google.com/nodejs/docs/reference/vertexai/latest[Vertex AI SDK] can be used to generate the access token and make the API calls.

See https://cloud.google.com/vertex-ai/generative-ai/docs/model-reference/gemini[Google Cloud - Vertex AI Gemini API] for more information on the Vertex AI Gemini API.

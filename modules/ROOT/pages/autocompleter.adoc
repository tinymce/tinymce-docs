...
:title_nav: Autocompleter :description: Add a custom autocompleter to TinyMCE 6. :keywords: autocomplete
...
|highlight |array |Optional |When using link:#cardmenuitem[CardMenuItems], use the highlight option to specify which link:#cardtext[CardText] items to highlight the matched text pattern on.
|===
...
The `+fetch+` function is called when the trigger `+char+` is pressed and the `+matches+` predicate (condition function) returns `+true+`. The `+fetch+` function is passed the matched text pattern and returns a https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise[Promise] containing matching results, specified as either link:#autocompleteitem[AutocompleteItems] or link:#cardmenuitem[CardMenuItems]. This allows for asynchronous fetching of the items. The `+fetchOptions+` parameter passed to the `+fetch+` function is by default an empty object, however using the link:#api[reload] API, additional options can be passed to fetch a different set of results.

There are two types of items:

* link:#autocompleteitem[AutocompleteItem]
* link:#cardmenuitem[CardMenuItem]
...
|icon |string |optional |Name of the icon to be displayed. Must be a either single unicode character or an icon from: the link:{{site.baseurl}}/how-to-guides/creating-custom-ui-components/editor-icon-identifiers/[icon pack], a link:{{site.baseurl}}/how-to-guides/customizing-the-editor-appearance/creating-an-icon-pack/[custom icon pack], or an icon added using the link:{{site.baseurl}}/apis/tinymce.editor.ui/tinymce.editor.ui.registry/#addicon[`+addIcon+` API].
|===
...
|===
|Name |Value |Requirement |Description
|items |array |required |An array of link:#carditems[CardItems]
|value |string |optional |Value of the item. This will be passed to the `+onAction+` callback when selected.
...
* link:#cardcontainer[CardContainer]
* link:#cardtext[CardText]
* link:#cardimage[CardImage]
...
This example uses link:#cardmenuitem[CardMenuItems] and will show when a user types a hyphen (`+-+`) character and at least one additional character.

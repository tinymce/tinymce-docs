...
|icon |string |optional |Name of the icon to be displayed. Must be a either single unicode character or an icon from: the link:editor-icon-identifiers.html[icon pack], a link:creating-an-icon-pack.html[custom icon pack], or an icon added using the link:tinymce.editor.ui.registry.html#addicon[`+addIcon+` API].
|===
...
\{% comment %} TODO: DOC-719 and TINY-6593: Move CardMenuItem/subitems specification to menuitems.md
\{% endcomment %}

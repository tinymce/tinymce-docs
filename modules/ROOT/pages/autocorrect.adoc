...
+
For example, if *cna* (which the {pluginname} plugin treats as a mis-typing of *can*) is a desired string in a particular use-case, turning {pluginname} off is the currently recommended setting.

. End-users cannot, currently, turn Autocorrect on or off on hand-held devices.
...
+
For example, {pluginname} auto-corrects *teh* to *the*, but does not, currently, auto-correct *Teh* to *The*.
...
. {pluginname} over-rides auto-capitalization.
+

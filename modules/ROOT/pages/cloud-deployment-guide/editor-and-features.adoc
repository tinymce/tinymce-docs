...
If you wish to use a language other than English (US), please link:{product_homepage}i18n[download a language pack] and then link:{rootDir}configure/localization/#language_url[specify its location] with the `language_url` configuration option.
...
If you have any custom created plugins (or if you've modified any of the existing plugins), in order for these plugins to continue operating with your Cloud deployment, you'll need to reference these plugins via link:{rootDir}configure/integration-and-setup/#external_plugins[external_plugins] rather than the regular link:{rootDir}general-configuration-guide/work-with-plugins.html[plugins] configuration element.

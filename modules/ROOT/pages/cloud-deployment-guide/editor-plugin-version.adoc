...
Each {productname} version is bundled with a set of premium plugins, but it is possible to specify different versions of each premium plugin to use with {productname}. Use the URL query parameters to specify the version of each premium plugin to load. This approach works with both the link:{baseurl}/cloud-deployment-guide/editor-and-features[{productname} editor and premium plugins deployment via {cloudname}] or just the link:{baseurl}/cloud-deployment-guide/features-only[premium plugins deployment from {cloudname}].
...
| link:{baseurl}/docs/plugins/premium/checklist/[Checklist]
| `checklist`
...
| link:{baseurl}/plugins/premium/tinydrive/[Tiny Drive]
| `tinydrive`
...
When deploying link:{baseurl}/cloud-deployment-guide/features-only[only premium plugins from {cloudname}], some features are served from {cloudname} and some features served from a self-hosted installation. There are two ways to achieve this: `plugins.min.js` and `cloud-plugins.min.js`.

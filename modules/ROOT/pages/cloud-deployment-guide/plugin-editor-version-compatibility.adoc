...
Use the following matrix in conjunction with the optional parameters for specifying editor version and xref:cloud-deployment-guide/editor-plugin-version.adoc[plugin version] to ensure compatibility.
...
IMPORTANT: Ensure the plugins are supported in your editor version when deploying premium plugins only using xref:cloud-deployment-guide/features-only.adoc[{cloudname}].

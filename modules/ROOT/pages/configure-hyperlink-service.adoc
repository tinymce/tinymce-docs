...
The following settings for the Hyperlinking premium server-side component are optional. These settings should be added to the `+application.conf+` file described in link:configure-required-services.html[Required configuration for the server-side components].

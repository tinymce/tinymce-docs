...
----
. Pass the configuration file to the Java application server. Assuming you've saved your `+logback.xml+` file in `+/etc/opt/tinymce+`, follow link:premium-server-side-guide.html#step4passtheconfigurationfiletothejavaapplicationserver[step 4] and link:premium-server-side-guide.html#step5restartthejavaapplicationserver[step 5] on the _Install Server-side Components_ page to set the following JVM system property on your Java application server:
+
....

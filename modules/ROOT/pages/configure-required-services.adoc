...
:title_nav: Required configuration :description: Configuration options for premium server-side components.
...
____
*Important:* When the services are matching against the `+allowed-origins+` list, the `+scheme+` (e.g. http) and the `+port+` (e.g. 800) are *ignored* by default. Customize this using link:#allowed-originsignore-portoptional[`+ephox.allowed-origins.ignore-port+`] option.
____
...
Regular expressions can be used alongside link:#wildcardsupport[wildcards] for specifying `+allowed-origins.origins+`. To use a regular expression, start and end the expression with the forward-slash `+'/'+` character.

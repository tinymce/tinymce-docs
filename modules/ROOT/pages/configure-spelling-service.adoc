...
The following settings for the Spelling premium server-side component are optional. These settings should be added to the `+application.conf+` file described in link:configure-required-services.html[Required configuration for the server-side components].
...
For information on creating custom dictionaries, see: link:custom-dictionaries-for-tiny-spellchecker.html#creatingcustomdictionaryfiles[Adding custom dictionaries - Creating custom dictionary files].

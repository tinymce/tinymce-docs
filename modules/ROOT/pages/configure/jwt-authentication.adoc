...
The private/public key pair is created in the user's link:{accountsignuppage}[Tiny account page], but only the public key is stored on Tiny web server. The user needs to store the private key in their backend.
...
* *name* - _(required)_ Full name of the user that will be used for presentation inside Tiny link:{rootDir}plugins/drive.html[Drive]. When the user uploads a file, this name is presented as the creator of that file.

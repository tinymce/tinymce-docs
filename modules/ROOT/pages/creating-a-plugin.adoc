...
:title_nav: Create a plugin :description_short: Introducing plugin creation, with an example. :description: A short introduction to creating plugins for TinyMCE along with an example plugin. :keywords: plugin plugin.js plugin.min.js tinymce.js
...
. In each translation file, add translation strings by passing an object containing key-value pairs of source strings and translation strings to the link:{{site.baseurl}}/apis/tinymce/root_tinymce/#addi18n[`+tinymce.addI18n()+` API].

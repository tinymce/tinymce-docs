...
`+onSetup+` is used to listen to the editor's link:events.html#editorcoreevents[`+NodeChange+` event] to disable the button when the cursor is inside a `+time+` element (or "node"). This ensures it is not possible to insert a `+time+` element into another `+time+` element.

...
|fetch |(success: (menu) => void) => void |Drop-down menu |required |A callback function that should be passed a list of <<choicemenuitems, choice menu items>> for the dropdown menu.
...
|onSetup |(api) => (api) => void |All |optional |default: () => () => \{} - Function invoked when the button is rendered. For details, see: <<usingonsetup, Using `+onSetup+`>>.
|===

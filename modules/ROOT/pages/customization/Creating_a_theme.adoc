...
If you want you may add theme specific options and settings, but remember to prefix them in the following format in order to provide a unique name space: `"<your theme>_<option>"` ("mytheme_someoption", for example). Use xref:api/class_tinymce.Editor.adoc#getparam[tinyMCE.getParam] to retrieve a custom theme option value.

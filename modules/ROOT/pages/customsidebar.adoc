...
When a new sidebar is registered, a corresponding toolbar button for toggling the sidebar open and close is also created using the same name. This button can then be included in the toolbar by adding the sidebar name to the link:toolbar-configuration-options.html#toolbar/[`+toolbar+`] option.
...
The `+icon+` specifies an icon for the sidebar toggle button. The icon should be the name of an icon provided by the {productname} skin or a link:tinymce.editor.ui.registry.html#addicon/[custom icon].

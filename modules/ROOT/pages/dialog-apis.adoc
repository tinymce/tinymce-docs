...
|`+redial(config) => void+` |Calling `+redial()+` and passing a dialog body configuration will destroy and recreate the body of the dialog. See the link:#redial[Redial] documentation.
|`+showTab(name: string) => void+` |*This method only applies to link:{{site.baseurl}}/how-to-guides/creating-custom-ui-components/dialogs/dialog-components/#tabpanel[tab panel] dialogs.* Calling `+showTab()+` and passing the `+name+` of a tab will make the dialog switch to the named tab.
|===
...
To redial a dialog, pass a new dialog configuration to the `+redial()+` method from the link:{{site.baseurl}}/how-to-guides/creating-custom-ui-components/dialogs/dialog-components/#dialoginstanceapimethods[dialog instance API].
...
See the link:{{site.baseurl}}/how-to-guides/creating-custom-ui-components/dialogs/dialog-examples/#interactiveexampleusingredial[redial example] for more information on how to use Redial.

...
The link:{baseurl}/how-to-guides/creating-custom-ui-components/dialogs/dialog-components/#dialoginstanceapimethods[dialog instance API] contains the `+showTab("tabName")+` method, which allows for programmatic tab switching using the registered `+name+` of a tab.

=== Dialog configuration event callbacks

A dialog can be configured with an link:{baseurl}/how-to-guides/creating-custom-ui-components/dialogs/dialog-configuration/#configurationoptions[`+onTabChange+`] callback. This function is called when the user changes tabs. It is passed the dialog instance API and a `+details+` object which contains `+newTabName+` and `+oldTabName+`.
...
A *collection* is a layout component that creates a panel containing a collection of small buttons in the dialog body. For example, this component is used in the dialogs for the link:{baseurl}/plugins-ref/opensource/charmap/[`+charmap+`] and link:{baseurl}/plugins-ref/opensource/emoticons/[`+emoticons+`] plugins.
...
To populate the collection with collection items, specify an array of items in the dialog's link:{baseurl}/how-to-guides/creating-custom-ui-components/dialogs/dialog-apis/#dialogdataandstate[`+initialData+`] property. To update the items in the collection, use the link:{baseurl}/how-to-guides/creating-custom-ui-components/dialogs/dialog-components/#dialoginstanceapimethods[dialog API's] `+setData()+` method. Each item should contain a `+text+`, `+value+`, and `+icon+` property. For example:
...
A *button* is a clickable component that can contain text or an icon. There are two types of buttons (primary and secondary buttons), though the only difference is that they are styled differently. Primary buttons are intended to stand out. The color will depend on the chosen link:{baseurl}/how-to-guides/customizing-the-editor-appearance/customize-ui/#skins[skin].

____
*Note*: Panel buttons are different to link:{baseurl}/how-to-guides/creating-custom-ui-components/dialogs/dialog-footer-buttons/[dialog footer buttons].
____
...
include::partial$misc/admon_predefined_icons_only[] *When configured, the button will display the icon instead of text.*
...
____
*Note*: To replace the entire dialog body with an iframe that loads its content from a URL, use a link:{baseurl}/how-to-guides/creating-custom-ui-components/dialogs/urldialog/[URL dialog].
____
...
To set the iframe's content on dialog open, specify document HTML as a string in the dialog's link:{baseurl}/how-to-guides/creating-custom-ui-components/dialogs/dialog-apis/#dialogdataandstate[`+initialData+`] property. To update the iframe's content, use the link:{baseurl}/how-to-guides/creating-custom-ui-components/dialogs/dialog-components/#dialoginstanceapimethods[dialog API's] `+setData()+` method. For example:
...
____
*Note*: The filepicker button will only appear if link:{baseurl}/content/file-image-upload/#file_picker_callback[`+file_picker_callback+`] is configured.
____

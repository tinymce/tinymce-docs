...
To populate the collection with collection items, specify an array of items in the dialog's xref:dialog-apis.adoc#dialogdataandstate[`+initialData+`] property. To update the items in the collection, use the xref:dialog-components.adoc#dialoginstanceapimethods[dialog API's] `+setData()+` method. Each item should contain a `+text+`, `+value+`, and `+icon+` property. For example:

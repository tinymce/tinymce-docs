...
|items |array |required |An array of xref:panelcomponents[panel components] to display inside the panel.
|===

NOTE: Panel components in different tabs with the same `+name+` will use the same value in the xref:dialog-apis.adoc#dialogdataandstate[dialog's data object]. This allows for transference of data between tabs. For example, the `+charmap+` and `+emoticons+` plugin dialogs contain a search input field in each tab. By assigning the same name to all the search fields, user data entered on one tab will be transferred when the user changes tabs.
...
[[dialoginstanceapimethods]]
=== Dialog instance API methods

The xref:dialog-apis.adoc#dialogapimethods[dialog instance API] contains the `+showTab("tabName")+` method, which allows for programmatic tab switching using the registered `+name+` of a tab.

=== Dialog configuration event callbacks

A dialog can be configured with an xref:dialog-configuration.adoc#configurationoptions[`+onTabChange+`] callback. This function is called when the user changes tabs. It is passed the dialog instance API and a `+details+` object which contains `+newTabName+` and `+oldTabName+`.
...
A *collection* is a layout component that creates a panel containing a collection of small buttons in the dialog body. For example, this component is used in the dialogs for the link:charmap.html[`+charmap+`] and link:emoticons.html[`+emoticons+`] plugins.
...
To populate the collection with collection items, specify an array of items in the dialog's link:dialog-apis.html#dialogdataandstate[`+initialData+`] property. To update the items in the collection, use the link:dialog-components.html#dialoginstanceapimethods[dialog API's] `+setData()+` method. Each item should contain a `+text+`, `+value+`, and `+icon+` property. For example:
...
NOTE: Panel buttons are different to link:dialog-footer-buttons.html[dialog footer buttons].
...
include::partial$misc/admon_predefined_icons_only.adoc[] *When configured, the button will display the icon instead of text.*
...
To set the iframe's content on dialog open, specify document HTML as a string in the dialog's xref:dialog-apis.adoc#dialogdataandstate[`+initialData+`] property. To update the iframe's content, use the xref:dialog-apis.adoc#dialogapimethods[dialog API's] `+setData()+` method. For example:

...
____
*Note*: Panel components in different tabs with the same `+name+` will use the same value in the link:dialog-apis.html#dialogdataandstate[dialog's data object]. This allows for transference of data between tabs. For example, the `+charmap+` and `+emoticons+` plugin dialogs contain a search input field in each tab. By assigning the same name to all the search fields, user data entered on one tab will be transferred when the user changes tabs.
____
...
The link:dialog-components.html#dialoginstanceapimethods[dialog instance API] contains the `+showTab("tabName")+` method, which allows for programmatic tab switching using the registered `+name+` of a tab.

=== Dialog configuration event callbacks

A dialog can be configured with an link:dialog-configuration.html#configurationoptions[`+onTabChange+`] callback. This function is called when the user changes tabs. It is passed the dialog instance API and a `+details+` object which contains `+newTabName+` and `+oldTabName+`.
...
____
*Note*: To replace the entire dialog body with an iframe that loads its content from a URL, use a link:urldialog.html[URL dialog].
____
...
To set the iframe's content on dialog open, specify document HTML as a string in the dialog's link:dialog-apis.html#dialogdataandstate[`+initialData+`] property. To update the iframe's content, use the link:dialog-components.html#dialoginstanceapimethods[dialog API's] `+setData()+` method. For example:
...
____
*Note*: The filepicker button will only appear if link:file-image-upload.html#file_picker_callback[`+file_picker_callback+`] is configured.
____

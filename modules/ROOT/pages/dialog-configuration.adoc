...
By default a dialog presents as a modal object in the center of the editor viewport.

This presented position can be changed for a given dialog by adding an `inline` value to the dialog’s configuration.
...
=== Basic example with the inline value added and set to toolbar

...
:title_nav: Basic configuration :description: How to create a basic dialog for TinyMCE. :keywords: dialog dialogapi api
...
* *Body:* The body of the dialog. The body component can be a link:{{site.baseurl}}/how-to-guides/creating-custom-ui-components/dialogs/dialog-components/#panel[panel] or a link:{{site.baseurl}}/how-to-guides/creating-custom-ui-components/dialogs/dialog-components/#tabpanel[tab panel], which can contain an array of link:{{site.baseurl}}/how-to-guides/creating-custom-ui-components/dialogs/dialog-components/#panelcomponents[panel components] such as buttons, inputs and text.
* *Buttons:* An array of link:{{site.baseurl}}/how-to-guides/creating-custom-ui-components/dialogs/dialog-footer-buttons/[footer buttons] that are displayed in the dialog's footer.
...
|body |`+panel+` or `+tabpanel+` component |required |The specification for the link:{{site.baseurl}}/how-to-guides/creating-custom-ui-components/dialogs/dialog-components/[body component].
|buttons |`+FooterButton[]+` |required |An array of link:{{site.baseurl}}/how-to-guides/creating-custom-ui-components/dialogs/dialog-footer-buttons/[footer buttons] to render in the footer of the dialog.
...
|onTabChange |`+(dialogApi, details) => void+` |optional |*This method only applies to link:{{site.baseurl}}/how-to-guides/creating-custom-ui-components/dialogs/dialog-components/#tabpanel[tab panel] dialogs.* Function invoked when the user changes tabs. `+details+` is an object that contains `+newTabName+` and `+oldTabName+`.
|===

For more information on the `+dialogApi+` object that is passed to some of the configuration options, see the link:{{site.baseurl}}/how-to-guides/creating-custom-ui-components/dialogs/dialog-components/#dialoginstanceapimethods[dialog instance API] documentation.

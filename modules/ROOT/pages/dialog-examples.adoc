...
In the `+onSubmit+` callback, the dialog instance API that is passed into the callback is used to call `+getData()+`. This function returns the dialog's data store, from which we are able to get the state of the `+isdog+` checkbox and the value of the `+catdata+` input. This information is used to construct a sentence which is then inserted into the editor. Finally, `+close()+` is called to manually close the dialog.

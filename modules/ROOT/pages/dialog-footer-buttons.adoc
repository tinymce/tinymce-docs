...
To toggle between a button's disabled and enabled states, use `+enable(name)+` or `+disable(name)+` from the xref:dialog-components.adoc#dialoginstanceapimethods[dialog instance API], where `+name+` is the identifier the button was configured with.

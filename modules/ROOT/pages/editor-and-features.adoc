...
* xref:tinydrive-plugin.adoc[{cloudfilemanager}]
...
* xref:editimage.adoc[Image Tools (with configured image proxy)]
...
To change the user interface language with a language pack, use the xref:content-localization.adoc#language[language configuration option].

Alternatively, link:{gettiny}/language-packages/[download a language pack] to enable a language other than English (US). xref:content-localization.adoc#language_url[Specify its location] with the `language_url` configuration option.

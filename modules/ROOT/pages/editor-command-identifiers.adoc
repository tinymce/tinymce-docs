...
| FormatBlock
| Toggles the format of the current selection. The value passed in should be the format name. If no format is specified, the paragraph (`<p>`) format will be toggled. For a list of options, see: xref:content-formatting.adoc#built-informats[Content formatting options - Built-in formats].
...
| mceToggleFormat
| Toggles a specified format by name. The value is the name of the format to toggle. For a list of options, see: xref:content-formatting.adoc#built-informats[Content formatting options - Built-in formats].

| ToggleSidebar
| Closes the currrent sidebar, or toogles the sidebar if the sidebar name is provided as a value (`_<sidebar-name>_`).

| ToggleToolbarDrawer
| Toggles the Toolbar Drawer. For information on toolbars, see: xref:editor-appearance.adoc#toolbar[User interface options - Toolbar].
include::partial$misc/requires_5_5v.adoc[]
...
The following command requires the xref:premium-advcode.adoc[Advanced Code (`advcode`)] plugin.
...
The following commands require the xref:premium-casechange.adoc[Case Change (`casechange`)] plugin.
...
The following commands require the xref:comments.adoc[Comments (`tinycomments`)] plugin.
...
The following commands require the xref:premium-export.adoc[Export (`export`)] plugin.
...
The following commands require the xref:premium-formatpainter.adoc[Format Painter (`formatpainter`)] plugin.
...
The following command requires the xref:fullpage.adoc[Full Page (`fullpage`)] plugin.
...
The following commands require the xref:imagetools.adoc[Image Tools (`imagetools`)] plugin.
...
The following commands require the xref:paste.adoc[Paste (`paste`)] plugin.
...
The following commands require the xref:premium-permanentpen.adoc[Permanent Pen (`permanentpen`)] plugin.
...
The following command requires the xref:print.adoc[Print (`print`)] plugin.
...
The following command requires the xref:spellchecker.adoc[Spell Checker (`spellchecker`)] plugin.
...
The following commands require the xref:toc.adoc[Table of Contents (`toc`)] plugin.

...
[NOTE]
====
The `+skin+` option requires the skin files to be in the default location (`+TINYMCE_BASE/skins/ui/${skinName}+`). For custom locations or when using NPM packages without bundling, refer to xref:editor-skin.adoc[Skin options] for more information.
====
...
[NOTE]
====
The `+icons+` option requires the icon pack files to be in the default location (`+TINYMCE_BASE/icons/${iconPackName}/icons.js+`). For custom locations or when using NPM packages without bundling, refer to xref:editor-icons.adoc[Icon options] for more information.
====

...
See the docs on link:{rootDir}enterprise/embed-media/mediaembed-server-config/#configureacustomendpoint[configuring a custom endpoint] for details on getting the Enhanced Media Embed server to utilise your custom API.
...
The other option for developing a custom API endpoint is to return JSON in the link:{rootDir}enterprise/embed-media/mediaembed-server-integration/#ephoxenhancedmediaembedformat[Tiny Enhanced Media Embed format].
...
* HTTP 200 (OK): link:{rootDir}enterprise/embed-media/mediaembed-server-integration/#ephoxembedobj[`EphoxEmbedObj`]
* HTTP 400 (User Error): link:{rootDir}enterprise/embed-media/mediaembed-server-integration/#errorobj[`ErrorObj`]
* HTTP 503 (Upstream Error): link:{rootDir}enterprise/embed-media/mediaembed-server-integration/#errorobj[`ErrorObj`]
* HTTP 500 (Unexpected Error): link:{rootDir}enterprise/embed-media/mediaembed-server-integration/#errorobj[`ErrorObj`]
...
* `links` (required)
 ** link:{rootDir}enterprise/embed-media/mediaembed-server-integration/#linksobj[LinksObj]
* `rel` (optional)
 ** link:{rootDir}enterprise/embed-media/mediaembed-server-integration/#relobj[RelObj]
* `media` (optional)
 ** link:{rootDir}enterprise/embed-media/mediaembed-server-integration/#mediaobj[MediaObj]
* `html` (optional)
...
* `primary` (required)
 ** Array of link:{rootDir}enterprise/embed-media/mediaembed-server-integration/#primaryrel[PrimaryRel]s
* `technical` (required)
 ** Array of link:{rootDir}enterprise/embed-media/mediaembed-server-integration/#technicalrel[TechnicalRel]s
* `source` (required)
 ** Array of link:{rootDir}enterprise/embed-media/mediaembed-server-integration/#sourcerel[SourceRel]s
...
* `players` (required)
 ** Array of link:{rootDir}enterprise/embed-media/mediaembed-server-integration/#linkobj[LinkObj]s
* `thumbnails` (required)
 ** Array of link:{rootDir}enterprise/embed-media/mediaembed-server-integration/#linkobj[LinkObj]s
* `apps` (required)
 ** Array of link:{rootDir}enterprise/embed-media/mediaembed-server-integration/#linkobj[LinkObj]s
 ...
 ** Integer
* `width` (required)
 ** link:{rootDir}enterprise/embed-media/mediaembed-server-integration/#dimensionboundobj[DimensionBoundObj]
* `height` (required)
 ** link:{rootDir}enterprise/embed-media/mediaembed-server-integration/#dimensionboundobj[DimensionBoundObj]

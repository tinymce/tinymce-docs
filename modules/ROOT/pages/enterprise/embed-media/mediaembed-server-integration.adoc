...
See the docs on xref:enterprise/embed-media/mediaembed-server-config.adoc#configureacustomendpoint[configuring a custom endpoint] for details on getting the Enhanced Media Embed server to utilize your custom API.
...
The other option for developing a custom API endpoint is to return JSON in the xref:enterprise/embed-media/mediaembed-server-integration.adoc#ephoxenhancedmediaembedformat[{productname} Enhanced Media Embed format].

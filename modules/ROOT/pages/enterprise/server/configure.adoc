...
* link:{rootDir}enterprise/embed-media/mediaembed-server-config.html[Enhanced Media Embed]
* link:{rootDir}enterprise/check-links/#linkcheckersdkquicksetup[Link Checker]
...
Assuming you've saved your `logback.xml` file in `/etc/opt/tinymce`, follow link:{rootDir}enterprise/server/#step4passtheconfigurationfiletothejavaapplicationserver[step 4] and link:{rootDir}enterprise/server/#step5restartthejavaapplicationserver[step 5] on the _Install Server-side Components_ page to set the following JVM system property on your Java application server:

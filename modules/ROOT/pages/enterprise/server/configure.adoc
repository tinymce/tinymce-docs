...
The Enhanced Media Embed server-side component require additional configuration, which can be found on the xref:enterprise/embed-media/mediaembed-server-config.adoc[Enhanced Media Embed page].
...
. As the only character (e.g. pass:q[`*`]). This will allow *any* Origin to access the server-side components.
...
The xref:plugins/opensource/imagetools.adoc[image proxy service] has some optional configuration to set a maximum size for images proxied. Images beyond this size it will not be proxied. Please note that the `http.request-timeout-seconds` above also applies to requests made by the image proxy service.
...
For information on creating custom dictionaries, see: xref:enterprise/check-spelling/custom.adoc#creatingcustomdictionaryfiles[Adding custom dictionaries - Creating custom dictionary files].
...
. Create a logging configuration XML file. Save the snippet below as `logback.xml` after replacing pass:[{$LOG_LOCATION}] with the full path to the destination log file (e.g. /var/log/tinymce_server_components.log).
+
...
. Pass the configuration file to the Java application server. Assuming you've saved your `logback.xml` file in `/etc/opt/tinymce`, follow xref:enterprise/server/index.adoc#step4passtheconfigurationfiletothejavaapplicationserver[`step 4`] and xref:enterprise/server/index.adoc#step5restartthejavaapplicationserver[`step 5`] on the *Install Server-side Components* page to set the following JVM system property on your Java application server:
+
[source]
----

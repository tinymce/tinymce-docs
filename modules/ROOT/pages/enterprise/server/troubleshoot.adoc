...
. The `application.conf` file is incorrect. Please go back and follow the steps listed in the xref:enterprise/server/index.adoc#step3createaconfigurationfile[installation guide]. This is the most common problem - often the origins are specified without the port numbers and this can cause things to fail, eg: use `+http://localhost:8080+` instead of `+http://localhost+`. After making changes to the `application.conf` file, please restart your Java web server (e.g. Jetty or Tomcat).
...
. The `application.conf` file is correct, and the services are working, but the server-side component URLs that the editor uses are not quite right. Refer to the xref:plugins/premium/tinymcespellchecker.adoc[Spell Checker Pro], xref:plugins/opensource/imagetools.adoc[Image Tools], xref:plugins/premium/mediaembed.adoc[Enhanced Media Embed] and xref:plugins/premium/linkchecker.adoc[Link Checker] plugin pages for help.

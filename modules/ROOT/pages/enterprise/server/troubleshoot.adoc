...
. The `application.conf` file is correct, and the services are working, but the server-side component URLs that the editor uses are not quite right. Refer to the link:{{site.baseurl}}/plugins/premium/tinymcespellchecker/[Spell Checker Pro], link:{{site.baseurl}}/plugins/opensource/imagetools/[Image Tools], link:{{site.baseurl}}/plugins/premium/mediaembed/[Enhanced Media Embed] and link:{{site.baseurl}}/plugins/premium/linkchecker[Link Checker] plugin pages for help.
. The browser is sending a different origin than expected (and configured in `application.conf`). Refer to step 6 of link:{{ site.baseurl }}/enterprise/server/troubleshoot/#usingbrowsertoolingtoinvestigateservicesissues[Using browser tooling to investigate services issues]
...
image::{{ site.baseurl }}/images/spell-checking-browser-tools.png[Using browser tools to investigate services issues]
...
image::{{ site.baseurl }}/images/spell-checking-server_status.png[Spell Checking Server Status]
...
image::{{ site.baseurl }}/images/spell-checking-jetty-settings.png[Spell Checking Server Settings on Jetty]
...
image::{{ site.baseurl }}/images/spell-checking-curl-windows-1.png[Curl on Windows step 1]

. Copy the path of the folder to where the 'curl.exe' is in:

image::{{ site.baseurl }}/images/spell-checking-curl-windows-2.png[Curl on Windows step 2]
...
image::{{ site.baseurl }}/images/spell-checking-curl-windows-3.png[Curl on Windows step 3]

. Once in the folder enter 'curl --version' (without quotes) and ensure you get a valid version

image::{{ site.baseurl }}/images/spell-checking-curl-windows-4.png[Curl on Windows step 4]

== Need more help?

If you are still experiencing problems, please contact link:{{site.supporturl}}[{{site.supportname}}].

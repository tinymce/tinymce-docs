...
|AutocompleterUpdate |`+{ lookupData: AutocompleteLookupData[] }+` |Fired when content is changed and the active autocompleter needs to be update the suggestions shown.
|AutocompleterEnd |N/A |Fired when the active autocompleter menu has been flagged to close.
|===

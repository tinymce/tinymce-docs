...
| ResizeEditor
| N/A
| Fired when the editor is resized, either by the resize handles or the auto-resize plugin. As of {productname} 5.10.6, this event is also fired when fullscreen mode is toggled. Toggling fullscreen mode requires the `fullscreen` plugin to be enabled.

...
|SwitchMode |`+{ mode: string }+` |Fired when the editor mode is changed. The available modes are "design" and "readonly". Additional modes can be registered using {productname} API link:tinymce.editormode.html#register['tinymce.activeEditor.mode.register()'].
...
|ProgressState |`+{ state: boolean, time?: number }+` |Fired when a change to the editor progress state is scheduled using the {productname} API link:tinymce.editor.html#setprogressstate[`+tinymce.activeEditor.setProgressState()+`].
...
|SetAttrib |`+{ attrElm: Element, attrName: string, attrValue: string }+` |Fired when an attribute is updated using the editor link:/apis/tinymce.dom/tinymce.dom.domutils/[DOMUtils API].
...
|BeforeOpenNotification |`+{ notification: NotificationSpec }+` |Fired before a notification is displayed. For information on the Notifications Specification (`+NotificationSpec+`), see: link:creating-custom-notifications.html[Create custom notifications].
|OpenNotification |`+{ notification: NotificationApi }+` |Fired after a notification is displayed. For information on the Notifications API (`+NotificationApi+`), see: link:creating-custom-notifications.html[Create custom notifications].
|===
...
The following events are provided by the link:autosave.html[Autosave plugin].
...
The following event is provided by the link:charmap.html[Character Map plugin].
...
The following event is provided by the link:comments.html[Comments plugin].
...
The following event is provided by the link:formatpainter.html[Format Painter plugin].
...
The following event is provided by the link:fullscreen.html[Fullscreen plugin].
...
The following event is provided by the link:importcss.html[Import CSS plugin].
...
The following event is provided by the link:linkchecker.html[Link Checker plugin].
...
The following event is provided by the link:lists.html[Lists plugin].
...
The following event is provided by the link:mediaembed.html[Media Embed plugin].
...
The following events are provided by the link:paste.html[Paste plugin].
...
The following events are provided by the link:permanentpen.html[Permanent Pen plugin].
...
The following events are provided by the link:powerpaste.html[PowerPaste plugin].
...
The following events are provided by the link:spellchecker.html[Spell Checker plugin].
...
The following event is provided by the link:visualblocks.html[Visual Blocks plugin].
...
The following event is provided by the link:visualchars.html[Visual Characters plugin].
...
The following event is provided by the link:wordcount.html[Word Count plugin].

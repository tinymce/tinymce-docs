= {pluginname} plugin (v2)
:plugincode: exportpdf
:pluginname: Export to PDF
:pluginfilename: export-to-pdf-v2
:navtitle: Export to PDF v2
:description: The {pluginname} feature provides the ability to generate a PDF file directly from the editor using the nested configuration structure.
...
For more information on the exportpdf_token_provider option, see xref:exportpdf-v2.adoc#exportpdf-token-provider[exportpdf_token_provider].
...
tinymce.init({
  ...
  plugins: 'exportpdf',
  toolbar: 'exportpdf',
  exportpdf_service_url: 'http://localhost:8080/',
  exportpdf_converter_options: {
    version: '2',
    document: {
      size: 'Letter',
...
include::partial$configuration/exportpdf_converter_options_v2.adoc[leveloffset=+1]

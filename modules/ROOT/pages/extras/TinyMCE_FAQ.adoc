...
. https://www.tiny.cloud/docs-3x/Installation/[Fully detailed]
...
This will convert all URLs within the same domain to relative URLs. The URLs will be relative from the https://www.tiny.cloud/docs-3x/reference/configuration/document_base_url/[document_base_url].
...
This will convert all relative URLs to absolute URLs. The URLs will be absolute based on the https://www.tiny.cloud/docs-3x/reference/configuration/document_base_url/[document_base_url].
...
This will convert all relative URLs to absolute URLs. The URLs will be absolute based on the https://www.tiny.cloud/docs-3x/reference/configuration/document_base_url/[document_base_url] with domain.
...
We recommend that you have a look at the https://www.tiny.cloud/docs-3x/reference/configuration/content_css/[content_css] option, this enables you to switch the CSS file TinyMCE uses for its editing area with a file with your CSS rules for font size and so forth.
...
TinyMCE produced P tags on enter by default since all desktop and online word processors use the concept of paragraphs on enter and it also makes your content semantic. If you for some reason need BR elements use the https://www.tiny.cloud/docs-3x/reference/configuration/forced_root_block/[forced_root_block] option in TinyMCE 3.5.
...
You need to specify a content css file with the https://www.tiny.cloud/docs-3x/reference/configuration/content_css/[content_css] option in order to change the default text color or background of the editor. The specified CSS is loaded after the default CSS so you can use it to override. A good idea to check the CSS status of elements is to install Firebug addon to Firefox.

[[edit-a-full-page-with-html-and-head-tags-in-tinymce]]
== Edit a full page (with <html> and <head> tags) in TinyMCE?
anchor:editafullpagewithhtmlandheadtagsintinymce[historical anchor]

By default TinyMCE is only interested in your page's content which is contained within the document's <body> part. If you want to have TinyMCE edit every part of your document, especially the <head> section, then you'll need the https://www.tiny.cloud/docs-3x/reference/plugins/fullpage/[fullpage plugin].

...
Sign up for a free API key provides entitlements to premium xref:editimage.adoc[Image Tools] plugin. A {cloudname} image proxy server must already be configured. Premium plugins can be purchased at any time once the API key has been set up.

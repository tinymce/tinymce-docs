...
== Footnotes and {productname} Previews

*File > Preview* presents footnote-specific in-document links, but these links do not work in the Preview.


== Footnotes and exporting {productname} documents to PDF

In-document footnote links are not preserved when a {productname} document is exported to a PDF file.

The footnotes and footnote text are preserved, but the clickable links between them are not. In an exported PDF file, the superscripted footnote numbers are not clickable.

And the carets set between a footnote number and footnote text, which act as links back to a superscripted footnote number in a {productname} document, are removed in the exported PDF file.
...
Footnote entries are not automatically removed after removing an image caption containing a footnote.

When an image caption with an attached footnote is removed (by unchecking the _Show caption_ checkbox in the **Insert/Edit Image* dialog), the associated footnote text should be automatically removed.

As of this release, however, this automatic removal does not happen.

The orphan footnote text can be deleted manually. It can also be removed by clicking the `footnotesupdate` button in the toolbar.

...
[arabic]
. xref:#selectorconfiguration[_(Required)_ The Selector configuration]
. xref:#pluginconfiguration[The Plugin configuration]
. xref:#toolbarconfiguration[The Toolbar configuration]
. xref:#menuandmenubarconfiguration[The Menu and Menu Bar configuration]
...
The selector can target most block elements when the editor is used in xref:general-configuration-guide/use-tinymce-inline.adoc[inline editing mode]. Inline mode edits the content in place, instead of replacing the element with an `+iframe+`.
...
For information on the differences between the classic and inline editing modes, see: xref:general-configuration-guide/use-tinymce-inline.adoc[Setup inline editing mode].
...
* For additional toolbar options, including disabling the toolbar and adding multiple toolbars, see: xref:configure/editor-appearance.adoc#toolbar[User interface options - toolbar].
* For a list of all available toolbar items, see: xref:advanced/available-toolbar-buttons.adoc[Toolbar Buttons Available for {productname}].

[[menuandmenubarconfiguration]]
...
* For information on the `+menu+` setting, see: xref:configure/editor-appearance.adoc#menu[User interface options - menu].
* For information on the `+menubar+` setting, see: xref:configure/editor-appearance.adoc#menubar[User interface options - menubar].
* For a list of all available menu items, see: xref:advanced/available-menu-items.adoc[Menu Items Available for {productname}].
...
// TODO: The next three links have no adoc counter part, so they are broken for now
* For a full list of configuration options, see: the [Configuration options reference]({productname}/configure/).
* For a full list of Plugins and their options, see: [Add plugins to TinyMCE]({productname}/plugins/).
* For information on the UI elements, see: [User interface components]({productname}/ui-components/).

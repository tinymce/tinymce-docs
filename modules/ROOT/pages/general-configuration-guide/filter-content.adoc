...
{productname} contains built-in formats that can be overridden. More information about these default controls is in the xref:general-configuration-guide/basic-setup.adoc[Basic setup] part of this guide.
...
| exact
| Disables the xref:configure/editor-appearance.adoc#style_formats_merge[Style merging] feature when used. `exact` is needed for some CSS inheritance issues such as text-decoration for underline/strikethrough.

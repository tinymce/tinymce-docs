...
IMPORTANT: The language settings will not work unless the language pack is placed in `langs/` directory. The xref:configure/localization.adoc#language_url[language_url] configuration option is another method to enable additional language settings.

== Step 3

Set the language option in {productname}'s configuration to the language code in the list on xref:configure/localization.adoc#language[this page].
...
link:{gettiny}/language-packages/[Download the Chinese language] pack to test this functionality. Install a _local instance_ of {productname} from the link:{gettiny}[downloads page]. Follow the xref:general-configuration-guide/advanced-install.adoc#self-hostedinstall[self-hosted install instructions].

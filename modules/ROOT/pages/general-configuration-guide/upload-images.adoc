...
Local images that are added through other means are also uploaded using this function, such as images added by drag and drop when using the link:{baseurl}/plugins/opensource/paste/#paste_data_images[`paste_data_images`] configuration property, or using the {companyname} link:{baseurl}/plugins/premium/powerpaste/[*PowerPaste* plugin].

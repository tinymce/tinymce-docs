...
Impressive! We now have a button on the toolbar users can click to get into source code mode, but we lost our default toolbar controls. We explained why this happens back in the link:{rootDir}general-configuration-guide/basic-setup.html[Basic Setup], so if this behavior surprises you, please re-read that section.
...
Don't worry if you can't remember all these values. The link:{rootDir}plugins/index.html[plugins section] goes into great detail explaining all the plugins options, toolbars, and menu controls.
...
As we noted at the start TinyMCE is an incredibly powerful, flexible, and customizable rich text editor. Once you've finished the Get Started guide, deep dive into the link:{rootDir}plugins/index.html[plugins] and link:{rootDir}configure/index.html[configuration options].
...
| *3*
| Advanced List
| link:{rootDir}plugins/advlist.html[advlist]
...
| *7*
| Color Picker
| link:{rootDir}plugins/colorpicker.html[colorpicker]

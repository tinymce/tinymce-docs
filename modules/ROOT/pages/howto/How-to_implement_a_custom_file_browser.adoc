...
Make sure you close your file browser window with tinyMCEPopup.close(). This ensures that any alterations by a plugin (as is the case with the https://www.tiny.cloud/docs-3x/reference/plugins/inlinepopups/[inline popup-plugin]) won't break.
...
If you use the https://www.tiny.cloud/docs-3x/reference/plugins/inlinepopups/[inlinepopups plugin] then your popup won't carry your window's title automatically. This is due to the fact that the inlinepopups plugin creates iframes in layers. To get your window title shown correctly you'll need to add some JavaScript code inside your custom file browser's template like shown below:
...
If you don't use the https://www.tiny.cloud/docs-3x/reference/plugins/inlinepopups/[inlinepopups plugin] then this might be interesting for you.

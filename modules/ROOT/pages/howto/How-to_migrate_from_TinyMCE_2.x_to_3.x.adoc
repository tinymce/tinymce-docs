...
Another change is that the xref:api/index.adoc[new API] uses the observer pattern internally: this enables you to more easily trap events and add custom logic before/after a specific action. You can now add an click event by using patterns like tinyMCE.activeEditor.onClick.add.
...
xref:api/index.adoc[See the API for details!]
...
* New: xref:api/html/class_tinymce.adoc#addi18n[tinyMCE.addI18n]('en.pluginName',{ stringName: 'string_value', ... stringName_n: 'stringValue_n'});
...
Remove the getControlHTML() function, buttons are now added in init() function using xref:api/class_tinymce.Editor.adoc#addbutton[addButton()] function.
...
button_name is used to refer to the button when changing its state (selected, disabled, normal) title is name of the language dependent string (note that # symbol is omitted). To refer to theme lang string use 'themeName.stringName' cmd is command name (leave it as it was) class refers to CSS style class. By default button images are now taken from image map (themes/advanced/img/icons.gif). Although it is possible to enter icon filename (see: xref:api/ui/class_tinymce.ui.Button.adoc[Button constructor] it is better to add your icon to the image map and use CSS class_name to refer to it. CSS class `'class_name'` should be defined in `/themes/advanced/skins/default/ui.css` (eg. `.defaultSkin span.mce_print {background-position:-440px -20px}`) (replace `defaultskin` with whatever skin you are using)

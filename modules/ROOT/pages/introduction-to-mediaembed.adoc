...
For the moment the *Enhanced Media Embed* plugin has to be used in conjunction with the link:media.html[media] plugin, so:
...
The service URL is already configured with link:/how-to-guides/cloud-deployment-guide/editor-and-features/[{cloudname}]. Simply specify the `+media+` and `+mediaembed+` plugins, and optionally a `+mediaembed_max_width+`
...
A link:{pricingpage}/[{enterpriseversion} subscription] includes the ability to download and install the *Media Embed* plugin and a *WAR* file to access the service backend. Please follow these link:premium-server-side-guide.html#step6setupeditorclientinstancestousetheserver-sidefunctionality[instructions] to configure the *WAR* file.

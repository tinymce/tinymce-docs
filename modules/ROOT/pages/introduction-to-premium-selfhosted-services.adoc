...
After you've completed the steps on this page to xref:step-2-deploy-server-side-components[Deploy server-side components], xref:step-3-create-a-configuration-file[Create a configuration file] and xref:step-4-pass-the-configuration-file-to-the-java-application-server[Pass the configuration file to the Java application server], the application server may need to be restarted to pick up all your changes. Turn it off and on again now, just to be safe.

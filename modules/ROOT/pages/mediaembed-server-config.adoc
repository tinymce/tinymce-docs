...
Once you have the link:/how-to-guides/premium-server-side-guide/[server-side component] installed, additional configuration to your `+application.conf+` file is required. (Don't forget to restart the Java application server after updating the configuration.)
...
A summary card is an embeddable snippet of code which is generated based on what the Enhanced Media Embed service can work out about the content at the URL. See the integration docs for link:/plugins-ref/premium/mediaembed/mediaembed-server-integration/[Enhanced Media Embed Server] for further details.

...
Merge tags can be inserted as-you-type; selected from a drop-down list when a specified prefix is typed; or selected and inserted from the searchable Merge tags toolbar menu button.
...
It is possible to change the visual appearance of the Merge tags within the editor using a custom CSS file provided though the xref:add-css-options.adoc#content_css[content_css] option. Here is an example on how to style the various elements of the merge tag.

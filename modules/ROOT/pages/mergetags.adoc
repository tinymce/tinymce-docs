...
Even though this fixed string uses the same default delimiting characters as individual merge tags, this fixed string does not interfere or otherwise interact with any {pluginname} configuration.

The Advanced Templates plugin replaces the Insertion Point Marker string with a hook element before inserting a template containing the string in to a {productname} instance.

Consequently, the the Advanced Templates Insertion Point Marker string is never seen by the {pluginname} plugin.

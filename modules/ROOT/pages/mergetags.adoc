= Merge Tags plugin
:navtitle: Merge Tags
:description: Enables Merge Tags functionality in the {productname} editor
:keywords: merge, tags, tag, mergetags, mergetag, mergetags_list, mergetags_prefix, mergetags_suffix
:pluginname: Merge Tags
:plugincode: mergetags
:pluginminimumplan: tiertwo

include::partial$misc/admon-premium-plugin.adoc[]

The {pluginname} plugin allows the user to insert a merge tag from a drop-down list when a specified prefix is typed, leaving a non-editable variable wrapped in a prefix and a suffix making the tag easily identifiable.

The {pluginname} plugin provides an xref:autocompleter.adoc[autocompleter] for adding a merge tag without using the toolbar button or menu item. The autocompleter is triggered by typing a specified prefix, controlled by the xref:#mergetags_prefix[`+mergetags_prefix+`] option which by default is `{{`, with any further characters filtering the list of merge tags.

== Interactive Example

// live demo

include::partial$misc/purchase-premium-plugins.adoc[]

== Basic setup

[source,js]
----
tinymce.init({
  selector: 'textarea',  // change this value according to your HTML
  plugins: 'mergetags',
  toolbar: 'mergetags',
});
----

== Options

include::partial$configuration/mergetags_list.adoc[leveloffset=+1]

include::partial$configuration/mergetags_prefix.adoc[leveloffset=+1]

include::partial$configuration/mergetags_suffix.adoc[leveloffset=+1]

include::partial$misc/plugin-toolbar-button-id-boilerplate.adoc[]

include::partial$misc/plugin-menu-item-id-boilerplate.adoc[]

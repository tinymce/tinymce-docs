...
The API methods for adding xref:ui-components/toolbarbuttons.adoc#howtocreatecustomtoolbarbuttons[Custom toolbar buttons] have changed for TinyMCE 5. The methods have been moved from link:{url}/docs-4x/api/tinymce/tinymce.editor/[`editor.*`] to link:{baseurl}/api/tinymce.editor.ui/tinymce.editor.ui.registry/[`editor.ui.registry.*`]. The toolbar button type `listbox` has been removed and a toggle button type has been added. The button types available in TinyMCE 5 are:

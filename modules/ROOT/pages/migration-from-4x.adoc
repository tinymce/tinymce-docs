...
The API methods for adding xref:ui-components/toolbarbuttons.adoc#howtocreatecustomtoolbarbuttons[Custom toolbar buttons] have changed for TinyMCE 5. The methods have been moved from link:{url}/docs-4x/api/tinymce/tinymce.editor/[`+editor.*+`] to xref:apis/tinymce.editor.ui.registry.adoc[`+editor.ui.registry.*+`]. The toolbar button type `listbox` has been removed and a toggle button type has been added. The button types available in TinyMCE 5 are:
...
* A new method, xref:apis/tinymce.editor.ui.registry.adoc#addToggleMenuItem[`addToggleMenuItem`] has been added to the options. The `addToggleMenuItem` is a method for creating toggle menu items similar to the xref:ui-components/typesoftoolbarbuttons.adoc#togglebutton[toggle toolbar button].

...
For information on using the latest development and testing builds, see: xref:editor-plugin-version.adoc[Cloud deployment guide - Specify editor & plugin versions].

[[editorchangestomethods]]
=== Editor: Changes to API Methods

All TinyMCE 4 API methods for creating UI components have been removed. New methods have been added for TinyMCE 5. For information on creating and customizing UI components, see: xref:ui-components.adoc[User interface components].
...
| xref:content-appearance.adoc#custom_colors[`custom_colors`]
...
| xref:editor-appearance.adoc#height[`height`]
...
| xref:editor-appearance.adoc#width[`width`]
| Supported number values.
...
xref:file-image-upload.adoc#file_picker_callback[`file_picker_callback`] provides a callback function, removing the requirement for an additional function to manually find and update the element or fire events. `file_picker_callback` can also update other fields by passing a second argument to update other fields in the dialog. For information on using `file_picker_callback`, see: xref:file-image-upload.adoc#file_picker_callback[Image & file upload options - `file_picker_callback`].
...
| Silver xref:use-tinymce-inline.adoc[Inline]

| link:{url}/docs-4x/themes/inlite/[Inlite(Distraction-free Editor)]
| Silver (xref:use-tinymce-distraction-free.adoc[distraction-free] configuration)

| link:{url}/docs-4x/themes/mobile/[Mobile]
| Silver (xref:tinymce-for-mobile.adoc[responsive to small screen touch devices])
|===
...
The Inlite theme features are provided by the xref:quickbars.adoc[Quick Toolbar (`quickbars`)] plugin for TinyMCE 5. The Inlite link:{url}/docs-4x/themes/inlite/#quicklink[`quicklink`] functionality is now provided by xref:contextform.adoc[Context Forms].
...
----
This will provide a similar but improved xref:use-tinymce-distraction-free.adoc[distraction-free] experience in TinyMCE 5.
...
The {productname} 4 xref:tinymce-for-mobile.adoc#thelegacymobiletheme[Mobile theme] was deprecated in {productname} 5.1. The mobile-optimized editor is loaded on mobile devices. For information on the new mobile experience, see: xref:tinymce-for-mobile.adoc[{productname} mobile].
...
xref:creating-a-skin.adoc[Skins] should be used for custom styling in TinyMCE 5.
...
| xref:typesoftoolbarbuttons.adoc[Toolbar Buttons]
...
| xref:creating-custom-menu-items.adoc#basicmenuitems[Menu item]
...
| xref:customsidebar.adoc[Sidebar]
...
| xref:contextform.adoc[Context form]

| `editor.ui.registry.addContextMenu: (name, spec)`
| xref:contextmenu.adoc[Context menu]

| `editor.ui.registry.addMenuButton: (name, spec)`
| xref:typesoftoolbarbuttons.adoc#menubutton[Menu Button]

| `editor.ui.registry.addNestedMenuItem: (name, spec)`
| xref:creating-custom-menu-items.adoc#nestedmenuitems[Nested menu item]

| `editor.ui.registry.addSplitButton: (name, spec)`
| xref:typesoftoolbarbuttons.adoc#splitbutton[Split Button]

| `editor.ui.registry.addToggleButton: (name, spec)`
| xref:typesoftoolbarbuttons.adoc#togglebutton[Toggle Button]

| `editor.ui.registry.addToggleMenuItem: (name, spec)`
| xref:creating-custom-menu-items.adoc#togglemenuitems[Toggle menu item]
...
The API methods for adding xref:custom-toolbarbuttons.adoc#howtocreatecustomtoolbarbuttons[Custom toolbar buttons] have changed for TinyMCE 5. The methods have been moved from link:{url}/docs-4x/api/tinymce/tinymce.editor/[`+editor.*+`] to xref:apis/tinymce.editor.ui.registry.adoc[`+editor.ui.registry.*+`]. The toolbar button type `listbox` has been removed and a toggle button type has been added. The button types available in TinyMCE 5 are:

* xref:typesoftoolbarbuttons.adoc#basicbutton[Basic button]
* xref:typesoftoolbarbuttons.adoc#togglebutton[Toggle button]
* xref:typesoftoolbarbuttons.adoc#splitbutton[Split button]
* xref:typesoftoolbarbuttons.adoc#menubutton[Menu button]

link:{url}/docs-4x/demo/custom-toolbar-listbox/[Listbox] toolbar buttons are not supported in TinyMCE 5. The recommended replacement toolbar button type is the xref:typesoftoolbarbuttons.adoc#splitbutton[*Split* button].

==== Changed toolbar button API methods

The following methods for creating custom toolbar buttons have been moved for TinyMCE 5. For information on how to use the new methods, see: xref:custom-toolbarbuttons.adoc[Toolbar buttons].
...
| xref:creating-custom-menu-items.adoc#togglemenuitems[Adds a custom toolbar toggle button].

| `editor.ui.registry.addSplitButton()`
| xref:typesoftoolbarbuttons.adoc#splitbutton[Adds a custom toolbar split button].

| `editor.ui.registry.addMenuButton()`
| xref:typesoftoolbarbuttons.adoc#menubutton[Adds a custom toolbar menu button].
|===

For information on how to use these methods, see: xref:typesoftoolbarbuttons.adoc[Types of toolbar buttons].
...
| xref:typesoftoolbarbuttons.adoc#basicbutton[`onAction`]
...
| xref:typesoftoolbarbuttons.adoc#onsetupexplanation[`onSetup`]
...
link:{url}/docs-4x/advanced/creating-a-custom-button/#basicbutton[`onclick`] is now xref:typesoftoolbarbuttons.adoc#basicbutton[`onAction`]. The callback function given to `onAction` takes a `buttonApi` argument which is an object that contains helper functions.
...
Each type of toolbar button has a different set of API functions. For information using toolbar buttons, see: xref:typesoftoolbarbuttons.adoc[Types of toolbar buttons].
...
* link:{url}/docs-4x/advanced/creating-a-custom-button/#togglebutton[`onpostrender`] was only processed when the editor was created, xref:typesoftoolbarbuttons.adoc#basicbuttonexampleandexplanation[`onSetup`] runs every time a component is rendered. For example: `onSetup` for a menu item is processed every time the menu rendered.
* `onSetup` has an API containing helper functions. Each xref:typesoftoolbarbuttons.adoc[type of toolbar button] has a different API.
...
The Context Toolbar accepts toolbar buttons to the editor using the xref:typesoftoolbarbuttons.adoc#basicbutton[`addButton`], xref:typesoftoolbarbuttons.adoc#togglebutton[`addToggleButton`], xref:typesoftoolbarbuttons.adoc#splitbutton[`addSplitButton`], or xref:typesoftoolbarbuttons.adoc#menubutton[`addMenuButton`] functions.
...
For information on Context Toolbars, see: xref:contexttoolbar.adoc[Context toolbar].
...
* A new method, xref:apis/tinymce.editor.ui.registry.adoc#addToggleMenuItem[`addToggleMenuItem`] has been added to the options. The `addToggleMenuItem` is a method for creating toggle menu items similar to the xref:typesoftoolbarbuttons.adoc#togglebutton[toggle toolbar button].
* The concept of `context` has been removed from menu item configurations. The xref:editor-appearance.adoc#menu[`menu` setting] provides this functionality for TinyMCE 5.
...
*More information*
* For information on using the `menu` setting, see: xref:editor-appearance.adoc#menu[User interface options - menu].
* For an example of the default menu items, see: xref:editor-appearance.adoc#examplethetinymcedefaultmenuitems[User interface options - Example: The TinyMCE Default Menu Items].
* For a list of the available menu controls provided by {productname}, see: xref:available-menu-items.adoc[Menu Items Available for {productname}].
...
For information on how to use these methods, see: xref:creating-custom-menu-items.adoc[Custom menu items].
...
| xref:dialog.adoc#configurationoptions[`initialData`]
| An object containing the initial value for the dialog components.

| xref:dialog.adoc#configurationoptions[`onCancel`]
| A callback that is called when the dialog is cancelled without submitting any changes.

| xref:dialog.adoc#configurationoptions[`onTabChange`]
| A callback that is called when switching tabs in a xref:dialog-components.adoc#tabpanel[TabPanel].
|===
...
| `onchange`
| xref:dialog.adoc#configurationoptions[`onChange`]
...
For information about the new dialog configuration, see the xref:dialog.adoc[Dialog] and xref:dialog-components.adoc[Dialog components] documentation.
...
For information on the new Dialog API, see: xref:dialog.adoc#dialoginstanceapi[UI components - Dialog instance API].
...
| `file`
| xref:urldialog.adoc#configurationoptions[`url`]
...
| xref:urldialog.adoc#configurationoptions[`onCancel`]
| A callback that is called when the dialog is cancelled without submitting any changes.

| xref:urldialog.adoc#configurationoptions[`onMessage`]
...
For information about the new URL dialog configuration, see: xref:urldialog.adoc[URL dialog].
...
| Context menus are now part of the {productname} core. For information on adding custom context menus, see: xref:contextmenu.adoc[UI components - Context menu].

| link:{url}/docs-4x/plugins/textcolor/[TextColor]
| The text color functionality is now part of the {productname} core. For information on using and customizing the text color settings, see: xref:content-appearance.adoc#textcoloroptions[Content appearance options - Text color options].

| link:{url}/docs-4x/plugins/colorpicker/[ColorPicker]
| The color picker is now part of the {productname} core and is enabled by default. For information on disabling the custom color picker, see: xref:content-appearance.adoc#custom_colors[Content appearance options - `custom_colors`].
|===
...
For information on using context menus and the default context menu configuration, see: xref:contextmenu.adoc[UI components - Context menu].
...
For information on Context Menus, see xref:contextmenu.adoc[UI components - Context menu].
...
xref:spellchecker.adoc#spellchecker_callback[`spellchecker_callback`] has been updated to remove a legacy format for the `success` callback, which accepted a mapping object of misspelled words to suggestions. For example:
...
For information on the `spellchecker_callback` setting, see: xref:spellchecker.adoc#spellchecker_callback[Spell Checker plugin - spellchecker_callback]

...
NOTE: While {productname} 6.0 honours the `height` setting in {productname} configuration, to prevent the editor UI from disappearing during re-sizing, the `min_height` value (`100px`) cannot be overridden. This obviates an unrecoverable bug, that prevents users being able to interact with the editor, from presenting.

...
=== `disabled` wording renamed in all {productname} APIs
...
Four plugins were removed as per the previous https://www.tiny.cloud/docs/release-notes/6.0-upcoming-changes/[{productname} 5 announcement, window="_blank"]:
...
=== `.disabled` wording renamed to `.enabled` in {productname} UI classes
...
NOTE: While {productname} 6.0 honours the `height` setting in {productname} configuration, to prevent the editor UI from disappearing during re-sizing, the `min-height` value (100px) cannot be overridden. This obviates an unrecoverable bug, that prevents users being able to interact with the editor, from presenting.

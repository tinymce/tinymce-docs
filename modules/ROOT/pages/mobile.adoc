...
One way to think about configuring TinyMCE mobile is that it is a subset of config options within your standard `tinymce` init. When on a mobile device, anything included in the link:{rootDir}configure/editor-appearance/#mobile[mobile] section will be used instead of the equivalent values in the top-level of settings of your config.
...
TinyMCE mobile supports a small subset of the toolbar items supported by the main mode. The toolbar is specified in the link:{rootDir}configure/editor-appearance/#mobile[mobile] section also.
...
Below are the toolbar items currently supported, with any plugins/configuration required. Note, that the `list` functions require the link:{rootDir}plugins/lists.html[lists] plugin and <<,styleselect>> requires configuring link:{rootDir}configure/content-formatting/#style_formats[style_formats].
...
|
| link:{rootDir}configure/content-formatting/#style_formats[style_formats]

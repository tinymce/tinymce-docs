...
In the snippet below, we use TinyMCE Cloud to instantiate the editor. If you're using the Self-hosted package, replace `cdn.tiny.cloud` with your path to `tinymce.min.js`.
...
NOTE:  If you're new to TinyMCE Cloud, link:{blogsite}how-to-get-tinymce-cloud-up-in-less-than-5-minutes/[this post] will help you get the editor instantiated quickly.
...
One way to think about configuring TinyMCE mobile is that it is a subset of config options within your standard `tinymce` init. When on a mobile device, anything included in the xref:configure/editor-appearance.adoc#mobile[mobile] section will be used instead of the equivalent values in the top-level of settings of your config.
...
TinyMCE mobile supports a small subset of the toolbar items supported by the main mode. The toolbar is specified in the xref:configure/editor-appearance.adoc#mobile[mobile] section also.
...
Below are the toolbar items currently supported, with any plugins/configuration required. Note, that the `list` functions require the xref:plugins/lists.html[lists] plugin and <<,styleselect>> requires configuring link:{rootDir}configure/content-formatting.adoc#style_formats[style_formats].
...
|
| xref:configure/content-formatting.adoc#style_formats[style_formats]

...
The Comments 2.0 plugin provides the ability to start or join a conversation by adding comments to the content within the TinyMCE editor. The Comments 2.0 plugin is built upon the link:{rootDir}advanced/annotations.html[Annotations API] and uses annotations to create comment threads (conversations).

Comments 2.0 is a premium plugin from Tiny. Please see the link:{rootDir}enterprise/tiny-comments.html[Premium features] section for all the buying options.
...
* *Callback Mode* - This is the default mode in Comments 2.0. This mode is used to configure storage and save comments on the user's server. This option provides a choice to configure the storage settings to either persist comments immediately or save them at the same time as the content. Additional callbacks are required to be configured to use Comments in the callback mode. Refer to the link:{rootDir}advanced/configuring-comments-callbacks.html[configuring callbacks for comments] section for more information.
...
For more information on configuring menu items refer to the link:{rootDir}configure/editor-appearance/#toolbar[toolbar] and link:{rootDir}configure/editor-appearance/#menu[menu] sections.
...
For more information on TinyMCE formats, refer to the link:{rootDir}configure/content-formatting/#formats[formats] section.
...
Check out the link:{rootDir}demo/comments/#comments20demo[Comments demo] to try out this new feature.

...
Note that the `figure` element needs some custom CSS added to render properly. This is what we use in the internal `content.css` within TinyMCE, and can be overridden with your own custom link:{rootDir}configure/content-appearance/#content_css[`content_css`] stylesheet.

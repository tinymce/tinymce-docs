...
NOTE: The Spell Checker plugin is self-hosted _only_. The Spell Checker Pro plugin is provided for some {cloudname} plans. For information on the Spell Checker Pro plugin, see: xref:plugins/premium/tinymcespellchecker.adoc[Spell Checker Pro plugin].
...
These settings affect the execution of the `spellchecker` plugin by modifying how spellchecking is handled. If you need an introduction to setting up server-side spell checking with {productname} please xref:general-configuration-guide/spell-checking.adoc[refer to this section] in the General Configuration Guide.

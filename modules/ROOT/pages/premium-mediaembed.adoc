...
The service URL is already configured with xref:editor-and-features.adoc[{cloudname}].
...
A link:{pricingpage}[{enterpriseversion} subscription] includes the ability to download and install the *Media Embed* plugin and a *WAR* file to access the service backend. Please follow these xref:server.adoc#step6setupeditorclientinstancestousetheserver-sidefunctionality[instructions] to configure the *WAR* file.

...
Any CSS classes defined in your CSS file will show up in the styles dropdown lists (unless you explicitly set what shows up in the styles dropdown list with the https://www.tiny.cloud/docs-3x/api/configuration/theme_advanced_styles/[theme_advanced_styles] option), be they in the main editor window or in a plugin's dialogue window (for example, the https://www.tiny.cloud/docs-3x/api/plugins/advimage/[advimage plugin] dialogue window) because TinyMCE will generate them dynamically based on your CSS file.

...
We strongly recommend users to utilize the xref:api/class_tinymce.Editor.adoc#isdirty[isDirty] method to determine if the editor contents were changed or not since this is much more exact because it doesn't need undo levels/blurring, etc.

It is important to note that the `onchange` callback will not fire on each keystroke due to performance considerations. Executing the onchange callback all the time is just a bad design practice and it wastes the users CPU. It's better to check the xref:api/class_tinymce.Editor.adoc#isdirty[isDirty] state before leaving the page in a document `onbeforeunload` event. This is how the autosave plugin functions. The xref:api/class_tinymce.Editor.adoc#isdirty[isDirty] approach is the recommended way of tracking if an editor instance has been modified or not.

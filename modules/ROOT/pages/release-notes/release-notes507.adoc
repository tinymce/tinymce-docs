...
For more information on the new `help_tabs` setting and API refer to the link:{baseurl}/plugins/opensource/help/[documentation].
...
For more information on `imagetools_fetch_image` refer to the link:{baseurl}/plugins/opensource/imagetools/#imagetools_fetch_image[documentation].
...
For more information on new toolbar button and menu item refer to the link:{baseurl}/plugins/opensource/table/#table_grid[documentation].
...
For more information on *Enhanced Media Embed* refer to the full link:{baseurl}/plugins/premium/mediaembed/#mediaembed_inline_styles[documentation].
...
The autocompleter functionality builtin to TinyMCE and used by the link:{baseurl}/plugins/premium/mentions/[*Mentions*], link:{baseurl}/plugins/opensource/charmap/[*Charmap*] and link:{baseurl}/plugins/opensource/emoticons/[*Emoticons*] plugins, has been improved to highlight matched text results, search across fragmented text nodes and support matching results that contain one or more spaces.
...
For more information on the configuration options for `link_assume_external_targets` refer to the link:{baseurl}/plugins/opensource/link/#link_assume_external_targets[documentation].

== Known issues

=== Premium Skins and Icon Packs

Using the cloud hosted premium content skins, in combination with the link:{baseurl}/plugins/opensource/preview/[preview] plugin, will cause nothing to render in the preview dialog. This is because the `Origin` header sent to the server is `null`, due to how the preview dialog is implemented in TinyMCE. This will be fixed in the next release of TinyMCE.

...
For information on the `editor.hasPlugin()` API, see: xref:api/tinymce/tinymce.editor.adoc#hasPlugin[tinymce.Editor APIs - `hasPlugin`].
...
For information on the `unload` and `unloadAll` APIs, see: xref:api/tinymce.dom/tinymce.dom.stylesheetloader.adoc[tinymce.dom.StyleSheetLoader].
...
* The `editor.insertContent()` API, see: xref:api/tinymce/tinymce.editor.adoc#insertContent[tinymce.Editor APIs - `insertContent`].
* The `editor.selection.setContent()` API, see: xref:api/tinymce.dom/tinymce.dom.selection.adoc#setContent[tinymce.dom.Selection APIs - `setContent`].

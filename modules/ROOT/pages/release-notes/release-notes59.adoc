...
* The `language` toolbar button, see: link:{baseurl}/advanced/available-toolbar-buttons/#thecoretoolbarbuttons[Available Toolbar Buttons - The core toolbar buttons].
* The `language` menu item, see: link:{baseurl}/advanced/available-menu-items/#thecoremenuitems[Available Menu Items - The core menu items].
* The `content_langs` option, see: link:{baseurl}/configure/localization/#content_langs[Content appearance options - `content_langs`].
* The `Lang` editor command, see: link:{baseurl}/advanced/editor-command-identifiers/#coreeditorcommands[Commands available for {productname} - Core Editor commands].
...
For information on these commands, see: link:{baseurl}/plugins/opensource/table/#commands[Table - Commands]
...
`tableclass`:: Allows the user to add or remove a class from the selected table. Available classes are defined by the link:{baseurl}/plugins/opensource/table/#table_class_list[`table_class_list` option].

`tablecellclass`:: Allows the user to add or remove a class from selected cells. Available classes are defined by the link:{baseurl}/plugins/opensource/table/#table_cell_class_list[`table_cell_class_list` option].

`tablecellvalign`:: Allows the user to set the vertical alignment for the content of selected cells.

`tablecellborderwidth`:: Allows the user to set the border width on selected table cells. The available widths are set with the link:{baseurl}/plugins/opensource/table/#table_border_widths[`table_border_widths` option].

`tablecellborderstyle`:: Allows the user to set the border style on selected table cells. The available styles are set with the link:{baseurl}/plugins/opensource/table/#table_border_styles[`table_border_styles` option].

`tablecaption`:: Toggles the caption on the selected table.

`tablecellbackgroundcolor`:: Allows the user to change the background color of selected cells. The available colors are set by the link:{baseurl}/plugins/opensource/table/#table_background_color_map[`table_background_color_map` option] or link:{baseurl}/configure/content-appearance/#color_map[`color_map` option].

`tablecellbordercolor`:: Allows the user to change the border color of selected cells. The available colors are set by the link:{baseurl}/plugins/opensource/table/#table_border_color_map[`table_border_color_map` option] or link:{baseurl}/configure/content-appearance/#color_map[`color_map` option].
...
For information on these buttons, see: link:{baseurl}/advanced/available-toolbar-buttons/#tableplugin[Available toolbar buttons]
For information on these menu items, see: link:{baseurl}/advanced/available-menu-items/#tableplugin[Available menu items]
...
`table_background_color_map`:: Allows setting a specific set of background colors to be used by the `tablecellbackgroundcolor` toolbar button and menu item, overriding the defaults and the colors provided by the link:{baseurl}/configure/content-appearance/#color_map[`color_map` option]. For information on this option, see: link:{baseurl}/plugins/opensource/table/#table_background_color_map[Table options - table_background_color_map].

`table_border_color_map`:: Allows setting a specific set of border colors to be used by the `tablecellbordercolor` toolbar button and menu item, overriding the defaults and the colors provided by the link:{baseurl}/configure/content-appearance/#color_map[`color_map` option]. For information on this option, see: link:{baseurl}/plugins/opensource/table/#table_border_color_map[Table options - table_border_color_map].

`table_border_widths`:: Allows setting a specific set of widths to be used by the `tablecellborderwidth` toolbar button and menu item. For information on this option, see: link:{baseurl}/plugins/opensource/table/#table_border_widths[Table options - table_border_widths].

`table_border_styles`:: Allows setting a specific set of HTML border styles to be used by the `tablecellborderstyle` toolbar button and menu item. For information on this option, see: link:{baseurl}/plugins/opensource/table/#table_border_styles[Table options - table_border_styles].
...
* A new `iframe_aria_text` option for customizing the `title` attribute on the editor iframe. For details, see: link:{baseurl}/configure/accessibility/#iframe_aria_text[Accessibility options - `iframe_aria_text`].
...
image::{baseurl}/images/context-toolbar-improvements.gif[Enchanced context toolbar behavior]

For information on context toolbars, see: link:{baseurl}/ui-components/contexttoolbar/[UI components - Context toolbar].

=== New `toolbar_sticky_offset` option for customizing sticky toolbars

The new `toolbar_sticky_offset` option allows the main toolbar to "dock" at a specified offset from the top or bottom of the view, depending on the toolbar location (set using the link:{baseurl}/configure/editor-appearance/#toolbar_location[`toolbar_location` option]).

For information on the `toolbar_sticky_offset` option, see: link:{baseurl}/configure/editor-appearance/#toolbar_sticky_offset[User interface options - toolbar_sticky_offset].
...
This release adds row numbering functionality, allowing users to add (and remove) row numbering columns on tables. For details, see: link:{baseurl}/plugins/premium/advtable/#addingrownumberingtoatable[Advanced Tables plugin - Adding row numbering to a table]. This includes:
...
For information on using the Real-time Collaboration (RTC) plugin, see: link:{baseurl}/rtc/[Real-time Collaboration plugin].
...
* Improved the error messages shown in the developer console for the `SpellcheckError` event, see: link:{baseurl}/plugins/premium/tinymcespellchecker/#events[Spell Checker Pro - `Events`].
...
* For details on enabling Hunspell support, see: link:{baseurl}/enterprise/server/self-hosting-hunspell/[Add Hunspell dictionaries to Spell Checker Pro].
* For a list of supported Spell Checker Pro languages, see: link:{baseurl}/plugins/premium/tinymcespellchecker/#supportedlanguages[Spell Checker Pro plugin - Supported languages].

The Spelling service now includes a new configuration option, allowing custom dictionaries to periodically deploy changes. For details, see: link:{baseurl}/enterprise/server/configure/#dynamic-custom-dictionariesoptional[Configure server-side components: `dynamic-custom-dictionaries`].

All the services now include an `ignore-port` port option to allow ports specified in `allowed-origins` to be enforced. For details, see: link:{baseurl}/enterprise/server/configure/#allowed-originsignore-portoptional[Configure server-side components: `allowed-origins.ignore-port` (optional)].
...
For information on the deprecation of the free TinyMCE Spell Checker plugin, see: link:{baseurl}/release-notes/release-notes54/#thefreetinymcespellcheckerplugin[TinyMCE 5.4 Release notes - Deprecated features].

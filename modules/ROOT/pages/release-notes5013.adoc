...
For more information on the `color_map` setting, refer to the xref:content-appearance.adoc#color_map[documentation].
...
and fixed issues is available in the xref:pages-changelog.adoc#version5013august62019[changelog].
...
TinyMCE 5.0.13 updates the default colors defined in the xref:content-appearance.adoc#color_map[`color_map`] setting to include colors that are useful for both highlights and regular text.
...
TinyMCE 5.0.13 has updated modal dialogs to be non-draggable by default. To re-enable draggable modals, add the xref:editor-appearance.adoc#draggable_modal[`draggable_modal`] setting to the TinyMCE init configuration.
...
TinyMCE 5.0.13 improves the nonbreaking space plugin by wrapping any nonbreaking space characters inserted by the plugin in a span to prevent the character being converted to a regular space while typing. To disable this behavior, add the new xref:nonbreaking.adoc#nonbreaking_wrap[`nonbreaking_wrap: false`] setting to the TinyMCE init configuration.
...
For information on *Tiny Comments* refer to the full xref:comments.adoc[documentation].

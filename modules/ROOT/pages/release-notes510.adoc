...
* Added `getRowType` property to the series generator `info` argument. For details, see xref:advtable.adoc#generatorinfo[`GeneratorInfo`].
...
* The `SpellcheckerIgnoreAll` event now includes a `language` property. For details, see xref:tinymcespellchecker.adoc#spellcheckerignoreallevent[Spell Checker Pro - SpellcheckerIgnoreAll Event].

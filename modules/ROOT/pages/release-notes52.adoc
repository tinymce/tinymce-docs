...
For information on using the `toolbar_location` setting, see: xref:editor-appearance.adoc#toolbar_location[User interface options - `toolbar_location`].
...
For information on the `toolbar_mode` option, see: xref:editor-appearance.adoc#toolbar_mode[User interface options - `toolbar_mode`].
...
For information on using the `toolbar_groups` setting, see: xref:editor-appearance.adoc#toolbar_groups[User interface options - `toolbar_groups`].
For information on using the `addGroupToolbarButton` API, see: xref:typesoftoolbarbuttons.adoc#grouptoolbarbutton[Types of toolbar buttons - Group button].
...
For information on using the `placeholder` setting, see: xref:editor-appearance.adoc#placeholder[User interface options - `placeholder`].
...
* xref:premium-a11ychecker.adoc#a11y_advanced_options[The Accessibility Checker plugin - `a11y_advanced_options`].
* xref:image.adoc#a11y_advanced_options[The Image plugin - `a11y_advanced_options`].

=== Modify the Quick Image toolbar using the `quickbars_image_toolbar` setting

The *quickbars_image_toolbar* option configures the Quick Image toolbar provided by the xref:quickbars.adoc[quickbars plugin]. To change the buttons on the Quick Image toolbar, provide a space-separated string of xref:available-toolbar-buttons.adoc[toolbar button names]. To disable the Quick Image toolbar, set `quickbars_image_toolbar` to `false`.

For information on using the `quickbars_image_toolbar` setting, see: xref:quickbars.adoc#quickbars_image_toolbar[Quick Toolbars plugin - `quickbars_image_toolbar`].
...
For information on using the `codesample_global_prismjs` setting, see: xref:codesample.adoc#codesample_global_prismjs[Code sample plugin - `codesample_global_prismjs`].
...
For an updated list of {productname} icons, see: xref:editor-icon-identifiers.adoc[Available icons].
...
* For information on using this setting with the Autolink plugin, see: xref:autolink.adoc#link_default_protocol[Autolink plugin - `link_default_protocol`].
* For information on using this setting with the Link plugin, see: xref:link.adoc#link_default_protocol[Link plugin - `link_default_protocol`].
...
For information on the `a11y_advanced_options` setting, see: xref:premium-a11ychecker.adoc#a11y_advanced_options[Accessibility Checker plugin].
...
For information on the checklist plugin, see: xref:premium-checklist.adoc[Checklist plugin].
...
For information on the `linkchecker_preprocess` option, see: xref:premium-linkchecker.adoc#linkchecker_preprocess[Link Checker plugin - `linkchecker_preprocess`].
...
For information on inlining page embed styles, see: xref:premium-pageembed.adoc#tiny_pageembed_inline_styles[Page Embed plugin - Inline styles for page embeds].
...
*PowerPaste* 5.2.1 provides a fix for PowerPaste not respecting the xref:integration-and-setup.adoc#cache_suffix[`cache_suffix` setting].
...
* 3 new skins: xref:naked-demo.adoc['naked'], xref:outside-demo.adoc['outside'], and xref:snow-demo.adoc['snow'].
...
Set `toolbar_drawer` or `toolbar_mode` to an alternative mode until the issue is resolved. For a list of alternative toolbar modes, see: xref:editor-appearance.adoc#toolbar_mode[User interface options - `toolbar_mode`].

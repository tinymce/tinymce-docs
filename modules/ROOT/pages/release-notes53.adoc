...
* The `toolbar_location` option, see: xref:editor-appearance.adoc#toolbar_location[User interface options - toolbar_location].
* {productname} in inline mode, see: xref:use-tinymce-inline.adoc#enablinginlineeditingmode[Setup inline editing mode].
...
For information on toggle menu items, see: xref:menuitems.adoc#togglemenuitems[Custom menu items - Toggle menu items].
...
For information on enabling the Find and Replace dialog, see: xref:searchreplace.adoc[the Search and Replace plugin].
...
* The Quick Toolbars plugin, see: xref:quickbars.adoc[The Quick Toolbars plugin].
* Creating custom context toolbars, see: xref:contexttoolbar.adoc[Context toolbars].
...
For information on the Accessibility Checker plugin, see: xref:premium-a11ychecker.adoc[Accessibility Checker plugin].
...
For a list of Comments plugin menu items, see: xref:comments_toolbars_menus.adoc#menuitems[Comments plugin - Menu items].

=== Enhanced Media Embed

The {productname} 5.3 release includes accompanying changes affecting the {productname} *self-hosted* *Enhanced Media Embed* service. {cloudname} users are unaffected. For details, see: xref:accompanying-premium-self-hosted-server-side-component-changes[Accompanying premium self-hosted server-side component changes].

=== Image Tools Proxy

The {productname} 5.3 release includes accompanying changes affecting the {productname} *self-hosted* *Image Tools Proxy* service. {cloudname} users are unaffected. For details, see: xref:accompanying-premium-self-hosted-server-side-component-changes[Accompanying premium self-hosted server-side component changes].

=== Link Checker

The {productname} 5.3 release includes accompanying changes affecting the {productname} *self-hosted* *Link Checker* service. {cloudname} users are unaffected. For details, see: xref:accompanying-premium-self-hosted-server-side-component-changes[Accompanying premium self-hosted server-side component changes].
...
* The PowerPaste plugin, see: xref:premium-powerpaste.adoc[PowerPaste plugin].
* The PowerPaste `paste_preprocess` option, see: xref:premium-powerpaste.adoc#paste_preprocess[PowerPaste plugin - `paste_preprocess`].
* The PowerPaste `paste_postprocess` option, see: xref:premium-powerpaste.adoc#paste_postprocess[PowerPaste plugin - `paste_postprocess`].
...
The {productname} *self-hosted* *Spell Checker Pro* service has also been updated. {cloudname} users are unaffected. For details, see: xref:accompanying-premium-self-hosted-server-side-component-changes[Accompanying premium self-hosted server-side component changes].

For information on Spell Checker Pro plugin, see: xref:premium-tinymcespellchecker.adoc[Spell Checker Pro plugin].
...
From {productname} 5.3, premium self-hosted bundles include the files required to build and deploy the server-side components as Docker containers. For information on Docker deployments of the server-side components, see: xref:dockerservices.adoc[Containerized service deployments].

For information on deploying the server-side components using Docker, see: xref:dockerservices.adoc[Containerized service deployments].

=== Regular Expression support for allowed-origins

Version 2.3.0 of the {productname} server-side components adds support for using regular expressions to specify `allowed-origins`. For information on using regular expressions for specifying allowed origins, see: xref:server-configure.adoc#regularexpressionssupport[Configure server-side components - Regular Expressions support].
...
For information on configuring the same-origin option, see: xref:server-configure.adoc#allowed-originssame-originoptional[Configure server-side components - allowed-origins.same-origin (optional)].
...
For information on configuring the alternative http timeout settings, see: xref:server-configure.adoc#alternativehttptimeoutsettings[Configure server-side components - Alternative http timeout settings].
...
For information on configuring the Link Checker service, see: xref:server-configure.adoc#link-checkingoptional[Configure server-side components - `link-checking`]
...
* The `images_dataimg_filter` option, see: xref:file-image-upload.adoc#images_dataimg_filter[Image & file options - images_dataimg_filter].

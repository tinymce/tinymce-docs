...
For information on toggle menu items, see: xref:creating-custom-menu-items.adoc#togglemenuitems[Custom menu items - Toggle menu items].
...
For information on using the language setting and a list of supported languages for {cloudname} and premium self-hosted deployments, see: xref:content-localization.adoc#language[Localization options - language].
...
For a list of Comments plugin menu items, see: xref:comments-toolbars-menus.adoc#menuitems[Comments plugin - Menu items].
...
* The PowerPaste plugin, see: xref:introduction-to-powerpaste.adoc[PowerPaste plugin].
* The PowerPaste `paste_preprocess` option, see: xref:introduction-to-powerpaste.adoc#paste_preprocess[PowerPaste plugin - `paste_preprocess`].
* The PowerPaste `paste_postprocess` option, see: xref:introduction-to-powerpaste.adoc#paste_postprocess[PowerPaste plugin - `paste_postprocess`].
...
For information on Spell Checker Pro plugin, see: xref:introduction-to-tiny-spellchecker.adoc[Spell Checker Pro plugin].
...
From {productname} 5.3, premium self-hosted bundles include the files required to build and deploy the server-side components as Docker containers. For information on Docker deployments of the server-side components, see: xref:bundle-intro-setup.adoc[Containerized service deployments].
...
For information on configuring the same-origin option, see: xref:configure-required-services.adoc#allowed-originssame-originoptional[Configure server-side components - allowed-origins.same-origin (optional)].
...
For information on configuring the alternative http timeout settings, see: xref:configure-required-services.adoc#alternativehttptimeoutsettings[Configure server-side components - Alternative http timeout settings].
...
For information on configuring the Link Checker service, see: xref:configure-required-services.adoc#link-checkingoptional[Configure server-side components - `link-checking`]

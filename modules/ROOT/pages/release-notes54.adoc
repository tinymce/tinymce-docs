...
* Adds a new xref:table.adoc#commands[`mceTableApplyCellStyle`] command for applying selected styles to table cells.
* Adds a new xref:table.adoc#table_header_type[`table_header_type`] option for setting the HTML structure used for table header rows.
* Adds table row, column, and cell type commands and queries.
* Adds a new xref:table.adoc#table_sizing_mode[`table_sizing_mode`] option and a new xref:table.adoc#commands[`mceTableSizingMode`] command for setting the method for measuring table cell width: `fixed`, `relative`, or `responsive`.
* Extends the xref:table.adoc#commands[`mceInsertTable`] command for adding tables without the dialog.

For information on the table plugin, see: xref:table.adoc[Table plugin].

For a list of:

* Menu items, including table-related menu items and menu sections, see: xref:available-menu-items.adoc[Menu Items Available for {productname}].
* Available icons, including table-related icons, see: xref:editor-icon-identifiers.adoc[Available icons].
* Table plugin APIs, see: xref:table.adoc#api[Table plugin - APIs].
* Table plugin commands, see: xref:table.adoc#commands[Table plugin - Commands].
...
For list of icons provided with {productname}, see: xref:editor-icon-identifiers.adoc[Available icons].

For information on:

* Creating custom UI components, see: xref:ui-components.adoc[User interface components].
* Creating and adding a custom icon pack, see: xref:creating-an-icon-pack.adoc[Create an icon pack for TinyMCE].
...
For information on the `block_unsupported_drop` option, see: xref:file-image-upload.adoc#block_unsupported_drop[Image & file options - `block_unsupported_drop`].
...
* xref:premium-tinymcespellchecker.adoc[The Spellchecker Pro premium plugin].
* xref:spell-checking.adoc#browser-basedspellchecking[Browser-based spell checking in {productname}].
...
The `table_responsive_width` option has been deprecated with the release of {productname} 5.4. This option has been replaced by xref:table.adoc#table_sizing_mode[`table_sizing_mode`].
...
* The `anchor_top` option, see: xref:url-handling.adoc#anchor_top[URL handling options - `anchor_top`].
* The `anchor_bottom` option, see: xref:url-handling.adoc#anchor_bottom[URL handling options - `anchor_bottom`].
...
For information on the `typeahead_urls` option, see: xref:advanced-editing-behavior.adoc#typeahead_urls[Advanced editing behaviors - `typeahead_urls`].

...
* Creating custom {productname} user interface components; such as toolbar buttons, menu items, and dialogs; see: xref:ui-components.adoc[User interface components].
* Creating a plugin for {productname}, see: xref:creating-a-plugin.adoc[Create a plugin for {productname}].
* The {productname} APIs, see: xref:api/[The {productname} API Reference].
...
* The `lineheight` toolbar button, see: xref:available-toolbar-buttons.adoc#thecoretoolbarbuttons[Available Toolbar Buttons - The core toolbar buttons].
* The `lineheight` menu item, see: xref:available-menu-items.adoc#thecoremenuitems[Available Menu Items - The core menu items].
* The `LineHeight` editor command, see: xref:editor-command-identifiers.adoc#coreeditorcommands[Commands available for {productname} - Core Editor commands].
* The `LineHeight` query command value, see: xref:editor-command-identifiers.adoc#availablequerycommandvalues[Commands available for {productname} - Available query command values].
...
For information on the new persistent toolbar option, see: xref:editor-appearance.adoc#toolbar_persist[User interface options - `toolbar_persist`].
...
* The Toolbar Drawer, see: xref:editor-appearance.adoc#toolbar_mode[User interface options - `toolbar_mode`].
* Commands and query commands available in {productname}, see: xref:editor-command-identifiers.adoc[Commands Available for TinyMCE].
...
* The `image_list` option, see: xref:image.adoc#image_list[Image plugin - `image_list`].
* The `image_class_list` option, see: xref:image.adoc#image_class_list[Image plugin - `image_class_list`].
* The `link_class_list` option, see: xref:link.adoc#link_class_list[Link plugin - `link_class_list`].
* The `link_list` option, see: xref:link.adoc#link_list[Link plugin - `link_list`].
* The `table_class_list` option, see: xref:table.adoc#table_class_list[Table plugin - `table_class_list`].
* The `table_cell_class_list` option, see: xref:table.adoc#table_cell_class_list[Table plugin - `table_cell_class_list`].
* The `table_row_class_list` option, see: xref:table.adoc#table_row_class_list[Table plugin - `table_row_class_list`].
...
For information on the `fullscreen_native` option, see: xref:fullscreen.adoc#fullscreen_native[Full Screen Plugin - `fullscreen_native`].
...
For information on the `contextmenu_avoid_overlap` option, see: xref:editor-appearance.adoc#contextmenu_avoid_overlap[User interface options - `contextmenu_avoid_overlap`].
...
For information on the `table_use_colgroups` option, see: xref:table.adoc#table_use_colgroups[Table plugin - `table_use_colgroups`].
...
For information on the Table plugin menu items, see: xref:table.adoc#menuitems[Table plugin - Menu items].
...
For information on the `table_column_resizing` option, see: xref:table.adoc#table_column_resizing[Table plugin - `table_column_resizing`].
...
* For information on the bundled CSS files, see: xref:customize-ui.adoc#customizingtheeditableareawithcontent_css[Customizing the editable area with `content_css`].
...
For information on the listbox dialog component, see: xref:dialogcomponents.adoc#listbox[Dialog components - listbox].
...
* The Image Tools context toolbar, see: xref:imagetools.adoc#imagetools_toolbar[Image Tools plugin - `imagetools_toolbar`].

For a list of available:

* Image Tools toolbar buttons, see: xref:imagetools.adoc#toolbarbuttons[Image Tools plugin - Toolbar buttons].
* {productname} icons, see: xref:editor-icon-identifiers.adoc[Icons Available for TinyMCE].
...
For information on the `media` plugin, see: xref:media.adoc[Media plugin].
...
* A new `a11ychecker_ignored_rules` option for filtering Accessibility Checker rules using the rule identifiers. For information on the `a11ychecker_ignored_rules` option, see: xref:premium-a11ychecker.adoc#a11ychecker_ignored_rules[Accessibility Checker plugin - `a11ychecker_ignored_rules`].
* A new `a11ychecker_issue_url_callback` option to allow custom URLs to be used for the "more info" links on accessibility checks. For information on the `a11ychecker_issue_url_callback` option, see: xref:premium-a11ychecker.adoc#a11ychecker_issue_url_callback[Accessibility Checker plugin - `a11ychecker_issue_url_callback`].
* Adds the rule identifier (`id`) to the data returned by the `getReport()` API method. For information on the `getReport()` API method, see: xref:premium-a11ychecker.adoc#getreport[Accessibility Checker plugin - `getReport()` API method].
* A new `a11ychecker_filter_issues` option for filtering Accessibility Checker report results. For information on the `a11ychecker_filter_issues` option, see: xref:premium-a11ychecker.adoc#a11ychecker_filter_issue[Accessibility Checker plugin - `a11ychecker_filter_issues`].
...
For information on the Advanced Code Editor plugin, see: xref:premium-advcode.adoc[Advanced Code Editor plugin].
...
For information on the Export plugin, see: xref:premium-export.adoc[Export plugin].
...
For information on the PowerPaste plugin, see: xref:premium-powerpaste.adoc[PowerPaste plugin].
...
For information on using the {productname} premium icon packs, see: xref:premium-skins-and-icon-packs.adoc#howtouseaniconpack[Tiny Skins and Icon Packs - How to use an icon pack].

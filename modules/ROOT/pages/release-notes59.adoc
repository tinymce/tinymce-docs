...
This release adds row numbering functionality, allowing users to add (and remove) row numbering columns on tables. For details, see: xref:premium-advtable.adoc#addingrownumberingtoatable[Advanced Tables plugin - Adding row numbering to a table]. This includes:
...
* Improved the error messages shown in the developer console for the `SpellcheckError` event, see: xref:premium-tinymcespellchecker.adoc#events[Spell Checker Pro - `Events`].
...
* For a list of supported Spell Checker Pro languages, see: xref:premium-tinymcespellchecker.adoc#supportedlanguages[Spell Checker Pro plugin - Supported languages].

The Spelling service now includes a new configuration option, allowing custom dictionaries to periodically deploy changes. For details, see: xref:server-configure.adoc#dynamic-custom-dictionariesoptional[Configure server-side components: `dynamic-custom-dictionaries`].

All the services now include an `ignore-port` port option to allow ports specified in `allowed-origins` to be enforced. For details, see: xref:server-configure.adoc#allowed-originsignore-portoptional[Configure server-side components: `allowed-origins.ignore-port` (optional)].
...
For information on the `content_langs` option, see: xref:premium-tinymcespellchecker.adoc#content_langs[Spell Checker Pro - `content_langs`].

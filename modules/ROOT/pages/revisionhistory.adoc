...
* `Saved`: These revisions are fetched from the client's storage upon opening the {pluginname} view, using the `revisionhistory_fetch` and `revisionhistory_fetch_revision` options.

...
Listening for `+SetContent+` alone saves the AI-generated result. Because the snapshot covers everything changed since the previous revision, it can also include human edits made before the AI edit:
...
To keep AI-generated and human content apart, listen for both events. `+BeforeSetContent+` saves the content that preceded each AI edit as its own revision, and `+SetContent+` saves the AI-generated result, producing a finer-grained history:

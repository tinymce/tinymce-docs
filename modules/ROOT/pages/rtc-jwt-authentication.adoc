...
This example shows how to set up a Node.js express handler that produces the tokens. It requires you to install the Express web framework and the `jsonwebtoken` Node module. For instructions on setting up a basic Node.js Express server and adding {productname}, see: xref:expressjs.adoc[Integrating TinyMCE into an Express JS App].

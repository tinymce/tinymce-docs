...
As noted in the link:rtc-options-required.html#rtc_document_id[Document ID configuration option], if a document ID is not known to the server, new initial data will be uploaded. If two users manage to do this at the same time, one will be forced to wait for the other to establish the session. This has implications for configurations using a dynamic link:rtc-options-required.html#rtc_encryption_provider[`+rtc_encryption_provider+`].
...
For a list of supported algorithms, see: link:jwt-authentication.html#supportedalgorithms[RTC JWT Authentication - Supported algorithms].
...
A "JWK set" is a structure that contains public keys for validating JWT signatures. At least one public key in the set must match the private key used to sign the provided identity token for RTC. This error means a public key could not be found in the {cloudname} account of the {companyname} API key used to load the RTC plugin. For information on creating public/private key pairs, see: link:jwt-authentication.html#privatepublickeypairsfortinycloudservices[JWT Authentication].
...
* The payload part does not contain all link:jwt-authentication.html#claims[required claims]

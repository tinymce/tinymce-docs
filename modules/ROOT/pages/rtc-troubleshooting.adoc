...
As noted in the link:{baseurl}/plugins-ref/premium/rtc/configuration/rtc-options-required/#rtc_document_id[Document ID configuration option], if a document ID is not known to the server, new initial data will be uploaded. If two users manage to do this at the same time, one will be forced to wait for the other to establish the session. This has implications for configurations using a dynamic link:{baseurl}/plugins-ref/premium/rtc/configuration/rtc-options-required/#rtc_encryption_provider[`+rtc_encryption_provider+`].
...
For a list of supported algorithms, see: link:{baseurl}/plugins-ref/premium/rtc/jwt-authentication/#supportedalgorithms[RTC JWT Authentication - Supported algorithms].
...
* The payload part does not contain all link:{baseurl}/plugins-ref/premium/rtc/jwt-authentication/#claims[required claims]

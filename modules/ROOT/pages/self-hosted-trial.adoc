...
tinymce.init({
  ...
  tinymceai_token_provider: async () => {
    await fetch(`https://demo.api.tiny.cloud/1/your-trial-api-key/auth/random`, { method: 'POST', credentials: 'include' });
...
Replace the `+'your-trial-license-key'+` placeholder with the trial license key issued from the *Self-hosted* tab of the Customer Portal.

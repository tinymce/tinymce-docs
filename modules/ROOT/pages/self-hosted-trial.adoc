...
Replace the `+'your-trial-license-key'+` placeholder with the trial license key issued from the *Self-hosted* tab of the Customer Portal. The token provider URL uses `+no-api-key+`, a placeholder that is automatically replaced with the Cloud API key once the account is created. The Cloud API key is only visible on the *Cloud* tab of the Customer Portal after the account exists.

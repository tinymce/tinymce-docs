...
For information on creating custom dictionaries, see: xref:custom.adoc#creatingcustomdictionaryfiles[Adding custom dictionaries - Creating custom dictionary files].
...
. [[step2passtheconfigurationfiletothejavaapplicationserver]] Pass the configuration file to the Java application server. Assuming you've saved your `logback.xml` file in `/etc/opt/tinymce`, follow xref:server.adoc#step4passtheconfigurationfiletothejavaapplicationserver[`step 4`] and xref:server.adoc#step5restartthejavaapplicationserver[`step 5`] on the *Install Server-side Components* page to set the following JVM system property on your Java application server:
+
[source]
----

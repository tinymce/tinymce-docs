...
NOTE: The Spell Checker plugin is self-hosted _only_. The Spell Checker Pro plugin is provided for some {cloudname} plans. For information on the Spell Checker Pro plugin, see: xref:introduction-to-tiny-spellchecker.adoc[Spell Checker Pro plugin].
...
These settings affect the execution of the `spellchecker` plugin by modifying how spellchecking is handled. If you need an introduction to setting up server-side spell checking with {productname} please xref:spell-checking.adoc[refer to this section] in the General Configuration Guide.

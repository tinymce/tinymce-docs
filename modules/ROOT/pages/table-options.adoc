...
* Table is an open-source plugin that provides improved table handling through UI components, including dialogs, context menu, context toolbar, menu items and toolbar buttons. For information, see the xref:table.adoc[Table plugin].
...
NOTE: Existing tables and tables added using `+setContent+` or paste are not affected by these options.

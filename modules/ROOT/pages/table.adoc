...
The Table plugin adds table management functionality to {productname}, including dialogs, context menu, context toolbar, menu items, and toolbar buttons.
...
For available events provided by the core table functionality, see xref:events.adoc#table-events[Table Events].

== Commands

NOTE: For additional commands that are provided by the core table functionality, see xref:editor-command-identifiers.adoc#core-commands-for-tables[Core Table Commands].
...
For available query command values provided by the core table functionality, see xref:editor-command-identifiers.adoc#query-command-values-tables[Core Table Query Command Values].

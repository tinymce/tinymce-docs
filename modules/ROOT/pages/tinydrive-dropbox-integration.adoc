...
[arabic]
. Copy the key from `+App key+` this will then be used as the key for the link:dropbox-and-google-drive.html#tinydrive_dropbox_app_key[tinydrive_dropbox_app_key].
...
You configure the Dropbox API key by setting the link:dropbox-and-google-drive.html#tinydrive_dropbox_app_key[tinydrive_dropbox_app_key] like in the example below.
...
For information on other {cloudfilemanager} config options refer to the link:dropbox-and-google-drive.html#configuringwithdropbox[configuration] section of this document.

...
For the accessibility standards {productname} is developed against and the Accessibility Conformance Reports available for download, see xref:accessibility-conformance.adoc[Accessibility conformance].
...
[[user-interface-navigation-order]]
== User interface navigation order

The *Tab* key moves focus to the next user interface section, and *Shift+Tab* moves focus to the previous section. The sections are reached in the following order:

. Menu bar
. Each toolbar group
. Sidebar
. Element path, in the status bar
. Word count toggle button, in the status bar
. Branding link, in the status bar
. Editor resize handle, in the status bar

A section that is not present is skipped. Within a section, the arrow keys move between the individual controls and cycle back to the start when the end of the section is reached.

NOTE: When the status bar has focus and no sidebar is visible, pressing *Shift+Tab* moves focus to the first toolbar group rather than the last.
...
[[navigating-element-path]]
=== Navigating the element path

To focus on the element path in the status bar, press *Alt+F11* (or *⌥+F11*). The element path is exposed as a WAI-ARIA `group` with a localized accessible name (`Element Path` in English). Use the *Left Arrow* and *Right Arrow* keys to move between segments. To return focus to the content area, use the *Esc* key.

...
* **"Translate"** category
+
The REST API uses the single action `'translate'` with a `language` parameter in the request body (`args: { language: string }`). Use language names such as `english`, `spanish`, `russian`, `swedish`, `german`, `japanese`, `portuguese`, `korean`, `italian`, or `chinese`. For the plugin UI, configure languages via `tinymceai_languages`.

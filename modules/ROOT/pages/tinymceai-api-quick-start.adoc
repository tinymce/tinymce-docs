...
The aim of this article is to get users up and running with the {pluginname} service.
...
The next step is to create a security token endpoint in the application. This endpoint securely authorizes end users to use {pluginname} features based on their permissions and access rights.
...
Cloud Services can reside in either US or EU region or in both. The region is set per subscription and cannot be changed for existing environments. For Custom plan with multi-region, the region can be chosen during environment creation. This topic is addressed in more detail in the Environment management guide.

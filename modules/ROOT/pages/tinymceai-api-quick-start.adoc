...
* For production, implement a backend that issues short-lived JWTs to authorize end users of the application for {productname} Cloud Services, using the access credentials from the Customer Portal. See xref:tinymceai-jwt-authentication-intro.adoc[JWT Authentication].
...
The next step for production is to create a security token endpoint in the application backend. That endpoint securely authorizes end users to use {pluginname} features based on their permissions and access rights.

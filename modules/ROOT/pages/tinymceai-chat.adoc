...
The Chat feature is available as part of the `+tinymceai+` plugin for {productname} and through the Conversations API. This page covers both:

* Plugin sections describe out-of-the-box user-facing features and configuration (xref:tinymceai.adoc#options-chat[Chat configuration]).
...
* `tinymceai_allow_model_selection`: Control what models users can select from available models (defaults to `true`).

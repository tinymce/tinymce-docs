...
tinymce.init({
  ...
  tinymceai_token_provider: () => fetch('/api/token').then(r => r.json())
});
----
...
For optimization tips and the full file limits reference, see xref:tinymceai-limits.adoc#file-limits[File limits] in the Limits documentation.

...
{pluginname} supports multiple AI models from different providers. Model selection and configuration can be done through the plugin configuration and UI or through the REST API. This page covers model capabilities, configuration options, and limitations that apply to both plugin and API usage.
...
* **Plugin Configuration**: Use `tinymceai_default_model` and `tinymceai_allow_model_selection` options to configure which models are available to users in the plugin UI. See xref:tinymceai.adoc#options-chat[Chat configuration options] for full details.
...
* **`id`**: Model identifier for `tinymceai_default_model` and API `model` fields.

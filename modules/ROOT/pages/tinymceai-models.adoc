...
The JWT `auth.ai.permissions` array includes xref:tinymceai-permissions.adoc#model-permissions[model permissions] that define which models are allowed for the token. That allowed set applies to both the plugin and the REST API.

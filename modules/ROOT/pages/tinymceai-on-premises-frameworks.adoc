...
In production the editor page and the AI service almost always live on different origins (`\https://app.yourcompany.com` vs `\https://ai.yourcompany.com`). The AI service must respond with CORS headers that permit the editor origin; otherwise the browser blocks every request.
...
|Ensure the proxy passes the `Origin` header to the AI service.
|===

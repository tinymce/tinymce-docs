...
:description_short: Five-minute Docker Compose quick start.
...
Confirms the AI service container is running and connected to the database and Redis.
...
Confirms the token server can sign a valid JWT using the API Secret and Environment ID.
...
Confirms the full chain: JWT verification, permissions, environment registration, LLM provider authentication, and SSE streaming.

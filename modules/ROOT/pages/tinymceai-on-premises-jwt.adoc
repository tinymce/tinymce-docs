...
:description_short: HS256 JWT authentication and permissions.
:keywords: AI, on-premises, JWT, authentication, HS256, multi-tenant, token, permissions, security
...
Configure the {productname} editor to fetch a token from the application endpoint. The plugin calls the provider on demand and re-fetches when the token nears expiry.

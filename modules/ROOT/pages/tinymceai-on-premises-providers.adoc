...
For production deployments, always set `MODELS` explicitly regardless of provider type. This ensures the editor model picker displays meaningful names, the correct provider handles each request, and JWT permissions can reference specific model IDs.

...
[arabic]
. The client application requests a signed token from the token endpoint.
. The client sends the JWT and prompt to the AI service over HTTPS.
...
When used with TinyMCE, the `tinymceai` plugin handles steps 1, 2, and 4 automatically through the `tinymceai_token_provider` callback.

...
* Translation Review is API-only (not listed in the Review sidebar); see the xref:tinymceai-review-plugin.adoc#built-in-reviews[Translation row] in the table above. For translation from the editor, see **Translate** in xref:tinymceai-actions-plugin.adoc#quick-actions-reference-table[Quick Actions reference table].
====
...
Built-in system reviews including API names, descriptions, and whether each type is available in the Review sidebar or only through the API—are summarized in xref:tinymceai-review-plugin.adoc#built-in-reviews[Built-in reviews].
...
* xref:tinymceai-actions-plugin.adoc[Quick actions]: Shortcuts to individual AI operations on a selection or range (transformations, translation, chat, and custom actions).

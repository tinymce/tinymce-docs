...
[arabic]
. The `+application.conf+` file is incorrect. Please go back and follow the steps listed in the link:premium-server-side-guide.html#step3createaconfigurationfile[installation guide]. This is the most common problem - often the origins are specified without the port numbers and this can cause things to fail, eg: use `+http://localhost:8080+` instead of `+http://localhost+`. After making changes to the `+application.conf+` file, please restart your Java web server (e.g. Jetty or Tomcat).
...
. The `+application.conf+` file is correct, and the services are working, but the server-side component URLs that the editor uses are not quite right. Refer to the link:tinymcespellchecker.html[Spell Checker Pro], link:imagetools.html[Image Tools], link:mediaembed.html[Enhanced Media Embed] and link:linkchecker.html[Link Checker] plugin pages for help.
. The browser is sending a different origin than expected (and configured in `+application.conf+`). Refer to step 6 of link:troubleshoot.html#usingbrowsertoolingtoinvestigateservicesissues[Using browser tooling to investigate services issues]

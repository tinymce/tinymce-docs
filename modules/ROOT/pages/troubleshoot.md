...
3. The `application.conf` file is correct, and the services are working, but the server-side component URLs that the editor uses are not quite right. Refer to the [Spell Checker Pro]({{site.baseurl}}/plugins-ref/premium/tinymcespellchecker/), [Image Tools]({{site.baseurl}}/plugins-ref/opensource/imagetools/), [Enhanced Media Embed]({{site.baseurl}}/plugins-ref/premium/mediaembed/) and [Link Checker]({{site.baseurl}}/plugins-ref/premium/linkchecker/) plugin pages for help.

...
`onSetup` is used to listen to the editor's xref:events.adoc#editorcoreevents[`NodeChange` event] to disable the button when the cursor is inside a `time` element (or "node"). This ensures it is not possible to insert a `time` element into another `time` element.
...
Use the following demo xref:custom-menu-toolbar-button.adoc[here] for help using the menu toolbar button.

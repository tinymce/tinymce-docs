...
Use the following demo xref:custom-toolbar-menu-button.adoc[here] for help using the menu toolbar button.
...
A group toolbar button is a toolbar button that contains a collection of other toolbar buttons that are displayed in a pop-up when clicked. The style of toolbar shown is based on the current xref:editor-appearance.adoc#toolbar_mode[toolbar mode]. For example, if `toolbar_mode` is set to `floating`, the toolbar group pop-up will appear in a floating shelf.
...
| required
| A string of space separated toolbar button names, or an array of xref:editor-appearance.adoc#addingtoolbargrouplabels[labelled toolbar buttons].

...
A *collection* is a layout component that creates a panel containing a collection of small buttons in the dialog body. For example, this component is used in the dialogs for the link:{baseurl}/plugins/opensource/charmap[`charmap`] and link:{baseurl}/plugins/opensource/emoticons[`emoticons`] plugins.

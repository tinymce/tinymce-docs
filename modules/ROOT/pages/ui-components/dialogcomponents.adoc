...
The body of a dialog must be either a `panel` (a single panel) or a `tabpanel` (a collection of panels). Each panel can contain panel components, which are either xref:layoutcomponents[layout components] or xref:basiccomponents[basic components] such as inputs, buttons and text.

[[panel]]
=== panel

The basic dialog type is a *panel* dialog. A panel is a container for xref:panelcomponents[interactive panel components]. A panel type dialog only has one panel.
...
A *tabpanel* dialog contains multiple panels, and a tab navigation menu on the left-hand side of the dialog to allow for switching between panels. Each panel can contain different xref:panelcomponents[panel components], allowing for complex dialogs.
...
| array
| required
| An array of xref:panelcomponents[panel components] to display inside the panel.
|===

NOTE: Panel components in different tabs with the same `name` will use the same value in the xref:ui-components/dialog.adoc#dialogdataandstate[dialog's data object]. This allows for transference of data between tabs. For example, the `charmap` and `emoticons` plugin dialogs contain a search input field in each tab. By assigning the same name to all the search fields, user data entered on one tab will be transferred when the user changes tabs.
...
The xref:ui-components/dialog.adoc#dialoginstanceapi[dialog instance API] contains the `showTab("tabName")` method, which allows for programmatic tab switching using the registered `name` of a tab.

==== Dialog configuration event callbacks

A dialog can be configured with an xref:ui-components/dialog.adoc#configurationoptions[`onTabChange`] callback. This function is called when the user changes tabs. It is passed the dialog instance API and a `details` object which contains `newTabName` and `oldTabName`.
...
Panels can contain xref:layoutcomponents[layout components] and xref:basiccomponents[basic components], which include components for displaying information and user interaction and input.
...
A *collection* is a layout component that creates a panel containing a collection of small buttons in the dialog body. For example, this component is used in the dialogs for the xref:plugins/opensource/charmap.adoc[`charmap`] and xref:plugins/opensource/emoticons.adoc[`emoticons`] plugins.
...
To populate the collection with collection items, specify an array of items in the dialog's xref:ui-components/dialog.adoc#dialogdataandstate[`initialData`] property. To update the items in the collection, use the xref:ui-components/dialog.adoc#dialoginstanceapi[dialog API's] `setData()` method. Each item should contain a `text`, `value`, and `icon` property. For example:
...
NOTE: To replace the entire dialog body with an iframe that loads its content from a URL, use a xref:ui-components/urldialog.adoc[URL dialog].
...
To set the iframe's content on dialog open, specify document HTML as a string in the dialog's xref:ui-components/dialog.adoc#dialogdataandstate[`initialData`] property. To update the iframe's content, use the xref:ui-components/dialog.adoc#dialoginstanceapi[dialog API's] `setData()` method. For example:
...
// TODO: the xref in the next line should be fixed automatically once #2328 gets merged
NOTE: The filepicker button will only appear if xref:configure/file-image-upload.adoc#file_picker_callback[`file_picker_callback`] is configured.

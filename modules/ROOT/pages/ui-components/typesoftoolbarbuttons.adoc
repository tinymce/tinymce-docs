...
* xref:basicbutton[Basic button]
* xref:togglebutton[Toggle button]
* xref:menubutton[Menu button]
* xref:splitbutton[Split button]
* xref:grouptoolbarbutton[Group toolbar button]
...
NOTE: See xref:onsetupexplanation[below] for details on how to configure `onSetup`.
...
`onSetup` is used to listen to the editor's xref:advanced/events.adoc#editorcoreevents[`NodeChange` event] to disable the button when the cursor is inside a `time` element (or "node"). This ensures it is not possible to insert a `time` element into another `time` element.
...
Use the following demo xref:demo/custom-toolbar-menu-button.adoc[here] for help using the menu toolbar button.
...
| Drop-down menu
| required
| A callback function that should be passed a list of xref:choicemenuitems[choice menu items] for the dropdown menu.
...
[[splitbuttonexampleandexplanation]]
...
A group toolbar button is a toolbar button that contains a collection of other toolbar buttons that are displayed in a pop-up when clicked. The style of toolbar shown is based on the current xref:configure/editor-appearance.adoc#toolbar_mode[toolbar mode]. For example, if `toolbar_mode` is set to `floating`, the toolbar group pop-up will appear in a floating shelf.
...
| required
| A string of space separated toolbar button names, or an array of xref:configure/editor-appearance.adoc#addingtoolbargrouplabels[labelled toolbar buttons].

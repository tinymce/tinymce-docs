...
NOTE: xref:ui-components/dialog.adoc[Standard {productname} dialogs] should suffice for most use cases, and may be simpler to configure.
...
* *Buttons:* (Optional) An array of xref:footerbuttons[footer buttons] that are displayed in the dialog's footer.
...
| FooterButton[]
| optional
| An array of xref:footerbuttons[buttons] to render in the footer of the dialog.
...
For more information on the `dialogApi` object that is passed to some of the configuration options, see the xref:urldialoginstanceapi[URL dialog instance API] documentation.
...
* A *Custom* type button will invoke the `onAction` callback function provided in the dialog configuration, and pass it the button's `name` in the `details` object. This allows developers to create a click handler for each *Custom* type footer button in the dialog. See the xref:ui-components/dialog.adoc#interactiveexampleusingredial[Redial example] for an example of how to use this with standard dialogs.

NOTE: Unlike xref:ui-components/dialog.adoc[standard dialogs], URL dialogs do not have a *Submit* type footer button, and therefore do not have an `onSubmit` callback.

See the xref:configurationoptions[URL dialog configuration options] documentation for more information on event callbacks.

[[urldialoginstanceapi]]
== URL dialog instance API

When a URL dialog is created, a dialog instance API is returned. For example, `const instanceApi = editor.windowManager.openUrl(config);`. The URL dialog API instance is also passed to some of the xref:configurationoptions[dialog configuration options].
...
Similarly, to send messages from {productname} back to the external page, the `sendMessage()` function from the xref:urldialoginstanceapi[URL dialog instance API] can be used to send messages, and then in the external page an event listener can be added to receive the messages:

...
Local images that are added through other means are also uploaded using this function, such as images added by drag and drop when using the xref:paste.adoc#paste_data_images[`+paste_data_images+`] configuration property, or using the {companyname} link:powerpaste.html[*PowerPaste* plugin].
...
An example PHP upload handler implementation is available link:/how-to-guides/image-handling-guide/php-upload-handler/[here].
...
include::partial$configuration/images_upload_url.adoc[]
...
include::partial$configuration/images_reuse_filename.adoc[]
...
The link:/how-to-guides/image-handling-guide/php-upload-handler/[PHP Upload Handler Script] provided here configures CORS in the `+$accepted_origins+` variable. Configure CORS at the http://www.w3.org/wiki/CORS_Enabled#At_the_Web_Application_level...[web application layer] or the http://www.w3.org/wiki/CORS_Enabled#At_the_HTTP_Server_level...[HTTP server layer].

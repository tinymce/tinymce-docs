...
Local images that are added through other means are also uploaded using this function, such as images added by drag and drop when using the link:paste.html#paste_data_images[`+paste_data_images+`] configuration property, or using the {companyname} link:powerpaste.html[*PowerPaste* plugin].

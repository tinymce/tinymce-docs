...
Local images that are added through other means are also uploaded using this function, such as images added by drag and drop when using the link:{baseurl}/plugins-ref/opensource/paste/#paste_data_images[`+paste_data_images+`] configuration property, or using the \{\{site.companyname}} link:{baseurl}/plugins-ref/premium/powerpaste/[*PowerPaste* plugin].
...
include::partial$misc/admon_svg_not_supported[]

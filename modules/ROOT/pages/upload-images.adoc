...
Local images that are added through other means are also uploaded using this function, such as images added by drag and drop when using the xref:paste.adoc#paste_data_images[`paste_data_images`] configuration property, or using the {companyname} xref:powerpaste.adoc[*PowerPaste* plugin].

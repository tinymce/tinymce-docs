...
image::{{site.baseurl}}/images/JWT_Flow.svg[JSON Web Token Call Flow]
...
. The {{site.cloudname}} Server sends a response, indicating that content submission was successful (or unauthorized if necessary).
{% else %}

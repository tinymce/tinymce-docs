...
For instructions on generating a key pair locally, see: link:generate-rsa-key-pairs.html[Creating a private/public key pair for Tiny Cloud].
...
\{% endif %} Once a public key has been generated, add the public key to the {cloudname} API key at: link:{accountpageurl}/jwt/[{accountpage} - JWT Keys].

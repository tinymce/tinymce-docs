...
. Add a public key to the {accountpage}, link:https://www.tiny.cloud/auth/login/[login].
. Set up a JSON Web Token (JWT) Provider endpoint using link:{accountjwturl}[{accountpage} - JWT Keys]
...
* The endpoint or server produces a response containing the token (JSON or plain text). {pluginname} will submit the token with requests to the AI service.

...
* The endpoint or server produces a plain text response with the token. {pluginname} will submit the token with requests to the AI service.

[NOTE]
====
Unlike some other TinyMCE plugins, {pluginname} expects the token as a plain text string, not a JSON object. Make sure your endpoint returns the token directly as text.
====

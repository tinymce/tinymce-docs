...
This property allows you to define shortcuts with `+selection: true+` which should only operate on context, when the user has selected content. Alternatively, shortcuts with `+selection: false+` will only be used with no selection, allowing you to define shortcuts for generating content from a specific prompt, without any context.

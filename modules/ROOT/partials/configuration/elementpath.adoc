...
For assistive technologies, the element path container uses `role="group"` with an `aria-label` (localized; `Element Path` in English). For keyboard access from the content area, see xref:tinymce-and-screenreaders.adoc#navigating-element-path[Accessible navigation guide].

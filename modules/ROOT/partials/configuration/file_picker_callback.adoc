...
Once you define `file_picker_callback`, small browse button will appear along the fields of supported file types (see xref:plugins/opensource/image.adoc#file_picker_types[file_picker_types]). When user clicks the button, {productname} will automatically call the callback with three arguments:
...
NOTE: The following example demonstrates how you can use `file_picker_callback` API, but doesn't pick any real files. Check xref:demo/file-picker.adoc[Basic Local File Picker] demo for a more functional example.

...
include::partial$configuration/builtinformats.adoc[leveloffset=-2]
...
All three format types can be used with the `+formats+` configuration option. They can also be used to specify a new format item in the xref:user-formatting-options.adoc#style_formats[`+style_formats+`] configuration option. However, a format that is specified using `+formats+` is then registered with the editor, and can be referred to by name in `+style_formats+` rather than needing to specify the format again.
...
A registered format can also be used by name with the built-in xref:user-formatting-options.adoc#using-custom-formats[Using custom formats] for an example.
...
*Default value:* `+none+` for `+Selector+` formats and `+empty+` for all other format types.

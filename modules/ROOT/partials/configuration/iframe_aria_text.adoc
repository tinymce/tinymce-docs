...
The `+title+` attribute is read by screen-readers to help users identify the editor. This option only applies to {productname} classic (iframe) mode and has no effect on link:inline-editor-options.html#inline[`+inline+` editors].

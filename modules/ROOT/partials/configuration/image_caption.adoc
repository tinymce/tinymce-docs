...
Note that the `+figure+` element needs some custom CSS added to render properly. This is what we use in the internal `+content.css+` within {productname}, and can be overridden with your own custom xref:add-css-options.adoc#content_css[`+content_css+`] stylesheet.

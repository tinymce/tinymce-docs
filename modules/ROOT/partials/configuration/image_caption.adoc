...
Note that the `figure` element needs some custom CSS added to render properly. This is what we use in the internal `content.css` within {productname}, and can be overridden with your own custom xref:configure/content-appearance.adoc#content_css[`content_css`] stylesheet.

...
[NOTE]
====
Each string in the array _must_ be in the format of `+example.com+`. Do not include protocols (for example, `+http://, https://+`) or any trailing slashes in your domains.
====

[NOTE]
====
`{plugincode}_cors_hosts` is *not* required when enabling this plugin via xref:editor-and-features.adoc[{cloudname}].
====

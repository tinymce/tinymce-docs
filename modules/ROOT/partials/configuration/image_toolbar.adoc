...
The *quickbars_image_toolbar* option configures the Quick Image toolbar provided by the xref:plugins/opensource/quickbars.adoc[quickbars plugin]. To change the buttons on the Quick Image toolbar, provide a space-separated string of xref:advanced/available-toolbar-buttons.adoc[toolbar button names]. The Quick Image toolbar is intended for image-related buttons, but any xref:advanced/available-toolbar-buttons.adoc[{productname} toolbar buttons] or xref:ui-components/toolbarbuttons.adoc[custom toolbar buttons] are allowed.

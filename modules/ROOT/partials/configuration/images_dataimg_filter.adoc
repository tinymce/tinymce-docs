...
The *images_dataimg_filter* option is used to filter `+<img>+` elements before they are passed to link:{{site.baseurl}}/content/file-image-upload/#images_upload_handler[`+image_upload_handler+`] or link:{{site.baseurl}}/content/file-image-upload/#images_upload_url[`+images_upload_url+`]. If the callback function provided returns `+false+` for an image, the image will not be uploaded.

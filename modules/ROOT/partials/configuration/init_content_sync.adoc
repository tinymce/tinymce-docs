...
[NOTE]
====
using the https://html.spec.whatwg.org/multipage/dynamic-markup-insertion.html#document.write()[`document.write`] method is https://developer.mozilla.org/en-US/docs/Web/API/Document/write[strongly discouraged].
====
...
[NOTE]
====
When {productname} is running in xref:use-tinymce-inline.adoc[inline mode] (ie, when `+inline: true+` is set), setting `init_content_sync: true` has no effect.
====

...
[NOTE]
====
{productname} in inline mode will not initialize on the following elements: `+area+`, `+base+`, `+basefont+`, `+br+`, `+col+`, `+frame+`, `+hr+`, `+img+`, `+input+`, `+isindex+`, `+link+`, `+meta+`, `+param+`, `+embed+`, `+source+`, `+wbr+`, `+track+`, `+colgroup+`, `+option+`, `+table+`, `+tbody+`, `+tfoot+`, `+thead+`, `+tr+`, `+th+`, `+td+`, `+script+`, `+noscript+`, `+style+`, `+textarea+`, `+video+`, `+audio+`, `+iframe+`, `+object+`, and `+menu+`.
====

For more information on the differences between the editing modes, please see this page xref:use-tinymce-inline.adoc[here].

...
If you in your handler would like fall back to the default media embed logic you can simply call the `resolve` callback with an object where the `html` property is set to an empty string, like this: `resolve({html: ''})`.

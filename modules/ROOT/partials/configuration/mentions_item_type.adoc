...
For information on the properties required for the user object provided to link:#mentions_fetch[`+mentions_fetch+`], see: link:#userproperties[User properties].
...
The following table describes the properties available for user objects provided to the link:#mentions_fetch[`+mentions_fetch+` callback]. Properties may be required, optional, or not available; depending on the link:#mentions_item_type[`+mentions_item_type+`] and link:#mentions_select[`+mentions_select+`] options.

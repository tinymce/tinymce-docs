...
This option is used if the xref:relative_urls[`relative_urls`] option is set to `false` and only applies to links with the same domain as the xref:document_base_url[`document_base_url`].

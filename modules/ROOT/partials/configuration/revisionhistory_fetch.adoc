...
The plugin uses the `revisionhistory_fetch` function to retrieve the saved versions for a document.
...
[NOTE]
The {pluginname} plugin does not sort the result before displaying. This allows the integrators to sort the result according to their preference. It is recommended that the result should be sorted in reverse chronological order as it is more intuitive for the users when they can only identify a version by its date.

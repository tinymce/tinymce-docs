...
For information on generating and serving JWTs for {productname} RTC, see: link:{baseurl}/rtc/jwt-authentication/[Real-time Collaboration (RTC) JWT Authentication Setup].
...
| `token`
| `string`
| A generated JWT token. This token should be signed with a private key as described in link:{baseurl}/rtc/jwt-authentication/#jwtendpointrequirements[JWT authentication].
|===

...
By default, a user's unique ID (`+userId+`, the `+sub+` field from their link:{baseurl}/plugins-ref/premium/rtc/jwt-authentication/#requiredjwtclaimsforreal-timecollaboration[JWT]) will be displayed as the username in remote caret tooltips.
...
Return data : | Field | Type | Description | |-------|:----:|-------------| | `+fullName+` | `+string+` | The full display name of user. For example: `+"John Doe"+`. | | any custom field | `+any+` | Extra user data for use in the <<rtc_client_connected, `+rtc_client_connected+` API>>. |

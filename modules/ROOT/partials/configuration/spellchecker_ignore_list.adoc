...
[NOTE]
====
Languages specified as keys in the `+spellchecker_ignore_list+` object must match the configured xref:introduction-to-tiny-spellchecker.adoc#spellchecker_languages[Spellchecker Languages].
====

*Type:* `+Object+` or `+Array+` of `+Strings+`

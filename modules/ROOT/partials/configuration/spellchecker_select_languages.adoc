...
This option specifies the languages that can be set for content when working with multi-language content. The specified languages will be available from the xref:premium-tinymcespellchecker.adoc#toolbarbuttons[`language` toolbar drop-down menu button].  For a list of available languages, see: xref:premium-tinymcespellchecker.adoc#supportedlanguages[Supported languages].

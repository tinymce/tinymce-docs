...
This option specifies the languages that can be set for content when working with multi-language content. The specified languages will be available from the link:#toolbarbuttons[`+language+` toolbar drop-down menu button]. For a list of available languages, see: link:#supportedlanguages[Supported languages].

...
* *Style:* The item must have a `+title+` and the necessary fields to link:content-formatting.html#formattype[specify a new format]. An optional `+name+` can be provided for the style, which will be prefixed with `+custom-+` to ensure that it does not override the default editor formats. If given a unique `+name+`, the style can be used with the formatting API. The item will be rendered in the UI as a clickable item that applies the given format.
* *Style reference:* The item must have a `+title+` and a `+format+` which refers to a pre-registered editor link:content-formatting.html#formats[format]. The item will be rendered in the UI as a clickable item that applies the given format.
...
To merge custom styles with the default `+styles_format+` values, set link:user-formatting-options.html#style_formats_merge[`+style_formats_merge+`] to `+true+`.
...
* Override the built-in link:content-formatting.html#formats[formats].
* Add some custom styles to the `+styleselect+` dropdown toolbar button and the `+formats+` menu item using the link:user-formatting-options.html#style_formats[style_formats] configuration option.

...
This option is used to specify the default values for the table cell background color picker, which can be opened with the `+tablecellbackgroundcolor+` toolbar button or menu item. If no values are defined, the toolbar button and menu item will use the values or default values of the link:{baseurl}/content/user-formatting-options/#color_map[`+color_map+` option]. This option does not modify the background color picker in the table dialogs. This option accepts Hex, RGB and HSL color values.

The link:{baseurl}/content/user-formatting-options/#custom_colors[custom color picker] is not available for the `+tablecellbackgroundcolor+` toolbar button or menu item.

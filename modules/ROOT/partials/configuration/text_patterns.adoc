...
*Type:* `+false+` or `+Array+` of `+Objects+`
...
NOTE: Inline patterns are executed on either pressing kbd:[Space] or kbd:[Enter].
...
* `+{ start: '*', end: '*', format: 'italic' }+` - Entering text between `+*+` and then pressing kbd:[Space] will result in the `+italic+` format being applied to the text between the `+*+` symbols.
* `+{ start: '**', end: '**', format: 'bold' }+` - Entering text between `+**+` and then pressing kbd:[Space] will result in the `+bold+` format being applied.
...
NOTE: Block patterns are executed on kbd:[Space] by default, but can be configured to be executed on kbd:[Enter].
...
NOTE: Replacement patterns are executed on either pressing kbd:[Space] or kbd:[Enter].
...
* Typing `+---+` and then either pressing kbd:[Space] or kbd:[Enter] will insert a horizontal rule block.
* Typing `+(c)+` and then either pressing kbd:[Space] or kbd:[Enter] will insert an inline copyright symbol.

...
A common use is to map each tool name to a human-readable message. For example, the callback can display `+Accessing Atlassian resources...+` while the `+atlassian-getAccessibleAtlassianResources+` tool runs.
...
tinymce.init({
  ...
  tinymceai_tool_data_callback: (event) => {
    ...
    const statusMessages = {
      'atlassian-getAccessibleAtlassianResources': 'Accessing Atlassian resources...'
    };

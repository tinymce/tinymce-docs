...
These options configure the Quick Actions menu, which provides one-click AI transformations such as improve writing, fix grammar, translate, and change tone.
...
Array of language options for the Translate submenu within Quick Actions. Each item has `+title+` (displayed in the menu) and `+language+` (language code sent to the API, for example `+'english'+`, `+'chinese'+`).

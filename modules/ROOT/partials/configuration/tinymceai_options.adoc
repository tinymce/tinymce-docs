...
** `+command+` (`+String+`): The editor command run when the button is selected. Selecting the button runs `+editor.execCommand(command, false, value)+`. Use one of the Quick Actions and Chat commands, or the core `+ToggleSidebar+` command, listed in xref:editor-command-identifiers.adoc#tinymceai[TinyMCE AI].
** `+value+` (optional): The argument passed to the command. The accepted value matches the command, for example a language label for `+TinyMCEAIQuickActionTranslate+` or a `+{ prompt }+` object for `+TinyMCEAIChatPrompt+`.
** `+icon+` (optional `+String+`): The name of the icon shown on the button. When omitted, an icon is selected based on the command.
...
tinymce.init({
  ...
  tinymceai_chat_welcome_actions: [
    ...
    { title: 'Translate to Swedish', command: 'TinyMCEAIQuickActionTranslate', value: 'swedish' },
    { title: 'Draft a reply', command: 'TinyMCEAIChatPrompt', value: { prompt: 'Draft a reply to this message.' } }
  ],
  // Required for authentication
  tinymceai_token_provider: () => {
    return fetch('/api/token').then(r => r.json());

...
NOTE: The size of toolbar groups affects the behavior of the toolbar drawer when the `toolbar_mode` is set to `'floating'` (default value) or `'sliding'`. For more information, see: <<toolbar_mode,the `toolbar_mode` setting>>.
...
Alternatively, you may specify multiple toolbars through the use of the <<toolbar_n,toolbar(n)>> option.

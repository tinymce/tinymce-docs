...
NOTE: The size of toolbar groups affects the behavior of the toolbar drawer when the `toolbar_mode` is set to `'floating'` (default value) or `'sliding'`. For more information, see: xref:editor-appearance.adoc#toolbar_mode[the `toolbar_mode` setting].

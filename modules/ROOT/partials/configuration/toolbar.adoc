...
[NOTE]
====
The size of toolbar groups affects the behavior of the toolbar drawer when the `+toolbar_mode+` is set to `+'floating'+` (default value) or `+'sliding'+`. For more information, see: xref:toolbar-configuration-options.adoc#toolbar_mode[the `+toolbar_mode+` option].
====

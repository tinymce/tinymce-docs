...
The `toolbar_location` option is used to position the <<toolbar,toolbar>> and <<menubar,menubar>>. The default behavior is to position the toolbar and menu bar above the content area. Setting this option to `bottom` positions the toolbar and menu bar below the content area. This option works in xref:general-configuration-guide/use-tinymce-classic.adoc[classic editor] (Iframe) mode and inline mode.

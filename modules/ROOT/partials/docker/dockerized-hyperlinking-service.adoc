...
----
+
For information on the `http` configuration setting, see: xref:server-configure.adoc#httpoptional[Configure server-side components - `http`].
...
. _Optional_: Configure the service to use a HTTP proxy by updating `config/ephox-hyperlinking-docker-env.conf`. See: xref:server-configure.adoc#proxyoptional[Configure server-side components - proxy].
...
----
+
For information on `allowed-origins`, see: xref:server-configure.adoc#allowed-originsrequired[Configure server-side components - allowed-origins].

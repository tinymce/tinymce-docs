...
\{% endif %} The TinyMCE hyperlinking server-side component can be deployed using Docker or a container orchestration application such as Kubernetes. {companyname} provides a pre-configured package for creating a Hyperlinking Docker image.

include::partial$misc/admon_dont-push-docker-images.adoc[]
...
____
*Note*: This procedure assumes that a {productname} self-hosted enterprise bundle has been extracted as described in link:bundle-intro-setup.html#downloadandextractthetinymceself-hostedbundle[Download and extract the {productname} Self-hosted Bundle].
____
...
[arabic]
. Go to [{accountpage}]({accountpageurl}/) > My Downloads and download either:
...
----
+
For information on the `+http+` configuration setting, see: link:configure.html#httpoptional[Configure server-side components - `+http+`].
. _Optional_: Update the link-checking cache configuration, as described in link:configure.html#hyperlinkingservicesettings[Link Checker self-hosted quick setup].
. _Optional_: Configure the Enhanced Media Embed Service, as described in link:mediaembed-server-config.html[Configure Enhanced Media Embed Server].
. _Optional_: Configure the service to use a HTTP proxy by updating `+config/ephox-hyperlinking-docker-env.conf+`. See: link:configure.html#proxyoptional[Configure server-side components - proxy].
...
----
+
For information on `+allowed-origins+`, see: link:configure.html#allowed-originsrequired[Configure server-side components - allowed-origins].

...
----
+
For information on the `+image-proxy+` configuration setting, see: link:{baseurl}/how-to-guides/premium-server-side-guide/configure/#image-proxyoptional[Configure server-side components - `+image-proxy+`].
...
. _Optional_: Configure the service to use a HTTP proxy by updating `+config/ephox-image-proxy-docker-env.conf+`. See: link:{baseurl}/how-to-guides/premium-server-side-guide/configure/[Configure server-side components].

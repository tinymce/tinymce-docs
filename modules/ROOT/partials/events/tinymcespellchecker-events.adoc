The following events are provided by the link:{baseurl}/plugins/premium/tinymcespellchecker/[Spell Checker Pro plugin].
...
| Fired when all instances of a word (in a certain language) have been marked as ignored.{% if site.productmajorversion < 6 %} *Note:* The `language` property is available for {productname} 5.10 and later.{% endif %}

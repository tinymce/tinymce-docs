...
//do we have a helm chart that we can link to?
=== Setting up the application using Kubernetes with Helm charts

Use the provided Helm chart to set up the {pluginname} On-Premises.
...
You can read more about using authorization in the xref:individual-export-to-pdf-on-premises.adoc#authorization[authorization] section.
...
** If the license is invalid, you’ll get a wrong license key error in the logs and the application will not run.
...
You can also use the demo page available on link:http://localhost:8080/demo[http://localhost:8080/demo] to generate an example PDF file.

\{% if productSource == 'cloud' %} \{% assign sourceName = site.cloudname | prepend: "the " %} \{% elsif productSource == 'composer' %} :sourceName: the Composer package manager \{% elsif productSource == 'npm' %} :sourceName: npm or Yarn \{% elsif productSource == 'bower' %} :sourceName: Bower \{% elsif productSource == 'nuget' %} :sourceName: Nuget \{% elsif productSource == 'zip' %} \{% assign sourceName = site.productname | append: " .zip package" | prepend: "a " %} \{% endif %}

\{% if productSource == 'cloud' %} :scriptSource: site.cdnurl  \{% else %} :scriptSource: /path/or/uri/to/tinymce.min.js \{% endif %}
...
liveDemo::default[ ] \{% if productSource == 'cloud' %}
...
Signing up for a \{\{site.cloudname}} API key will also provide a trial of the link:{baseurl}/plugins-ref/premium/[Premium Plugins]. \{% endif %}

== Save the content from the editor

To retrieve content from the editor, either process the content with a form handler or use the link:{baseurl}/apis/tinymce/tinymce.editor/#getcontent[getContent API].

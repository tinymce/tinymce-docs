...
* xref:upgrading-tinymce-self-hosted-using-a-package-manager[Upgrading {productname} Self-hosted using a package manager].
endif::[]
* xref:upgrading-tinymce-self-hosted-manually[Upgrading {productname} Self-hosted manually].
...
=== Upgrading {productname} Self-hosted using a package manager
...
For commercial customers using premium plugins, also upgrade the `+tinymce-premium+` package:

[source,sh,subs="attributes+"]
----
npm install tinymce-premium@latest --save
----

[NOTE]
====
The `+tinymce+` and `+tinymce-premium+` packages are always released together with matching version numbers. Upgrading both packages together ensures compatibility.
====
...
=== Upgrading {productname} Self-hosted manually

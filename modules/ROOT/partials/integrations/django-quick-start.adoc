...
\{% if productSource == "cloud" %} To load {productname} from the {cloudname}, update the project’s `+settings.py+` file:
...
\{% else %} To download {productname} and use it with the `+django-tinymce+` package:

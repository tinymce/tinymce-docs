...
. Install the required Node.js components, including https://laravel.com/docs/11.x/vite[Vite]:
+
[source,sh]
----
npm install
...
. Run Vite to copy {productname} to the `+public/js/+` directory:
+
[source,sh]
----
npm run build
...
. Create a new reusable https://laravel.com/docs/11.x/blade#components[component (`+blade.php+`)] for the {productname} configuration, such as:
+
[source,sh]
----
...
----
<script src="{scriptsource}" referrerpolicy="origin" crossorigin="anonymous"></script>
<script>
  tinymce.init({

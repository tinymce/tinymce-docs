...
1. Add {productname} to the project using Composer:
...
{% endif %}
{% if productSource != "cloud" %}
1. Run Laravel Mix to copy {productname} to the `public/js/` directory:
...
{% endif %}
1. Create a new reusable https://laravel.com/docs/8.x/blade#components[component (`blade.php`)] for the {productname} configuration, such as:
...
{% if productSource == "cloud" %}
1. Replace `no-api-key` in the {productname} source script with your {cloudname} API Key.

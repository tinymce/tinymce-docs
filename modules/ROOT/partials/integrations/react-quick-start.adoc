...
\{% include integrations/bundling-integration.md package='tinymce-react' depth='1' %} \{% else %} 5. \{\{site.productname}} can be self-hosted by either: link:#deployingtinymceindependent[Deploying \{\{site.productname}} independent of the React application], or link:#bundle[Bundling \{\{site.productname}} with the React application]. * Deploying \{\{site.productname}} independent of the React application. To use a \{\{site.productname}} instance that has been deployed independent of the React application, use: link:#usingthetinyscriptsrcprop[the `+tinymceScriptSrc+` prop], or link:#usinganhtmlscripttag[an HTML `+<script>+` tag]. * Using the `+tinymceScriptSrc+` prop:
...
** \{\{site.productname}} integration, see: link:{{site.baseurl}}/how-to-guides/environment-setup/react/react-ref/[React framework Technical Reference].

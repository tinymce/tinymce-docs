...
// If using premium plugins, copy tinymce-premium into the same tinymce folder
// Note: This will overwrite root-level files (license.txt, README.md, CHANGELOG.md, etc.) with the premium package versions
if (fse.existsSync(path.join(topDir, 'node_modules', 'tinymce-premium'))) {
  fse.copySync(path.join(topDir, 'node_modules', 'tinymce-premium'), path.join(topDir, 'public', 'tinymce'), { overwrite: true });
}
----
+
.package.json

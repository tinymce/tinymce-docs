...
. Use the https://github.com/vitejs/vite[Vite] package and the https://github.com/vitejs/vite-plugin-react-swc[React SWC plugin] to create a new React + Typescript project named `+tinymce-react-demo+`. You can also use the `react-swc` template, if you don't wish to use Typescript.
+
[source,sh]
----
npm create vite@5 tinymce-react-demo -- --template react-swc-ts
----
...
. Using a text editor, open `+./src/App.tsx+` and replace the contents with:
+
[source,tsx]
----

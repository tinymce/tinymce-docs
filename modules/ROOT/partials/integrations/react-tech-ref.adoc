...
<<apikey,apiKey>>:: The {cloudname} API key. When loading from {cloudname}, use this prop to remove the "This domain is not registered..." warning message.

<<cloudchannel,`cloudChannel`>>:: The channel of {productname} used when loading from {cloudname}.
...
<<tinymcescriptsrc,`tinymceScriptSrc`>>:: The URL to use for sourcing {productname}, when loading a self-hosted version of {productname}.
...
<<init,`init`>>:: Additional options passed to {productname} when it is initialized.
...
<<oneditorchange,`onEditorChange`>>:: An event handler for detecting editor changes. Useful when implementing {productname} as a controlled component.
...
None of the configuration props are *required* for the {productname} React component; however, if the `apiKey` prop is not configured when loading from {cloudname}, a warning message will be shown in the editor. For guidance about which props to use, see: <<configuringtheeditor,Configuring the editor>>.
...
For detailed information on using `onEditorChange`, see: <<usingthetinymcereactcomponentasacontrolledcomponent,Using the {productname} React component as a controlled component>>.
...
For detailed information on using the `value` prop, see: <<usingthetinymcereactcomponentasacontrolledcomponent,Using the {productname} React component as a controlled component>>.

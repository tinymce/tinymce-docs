...
. Update `+App.vue+` as below. *Note*: Remember to point `tinymceScriptSrc` to `/tinymce/tinymce.min.js`
+
[source,jsx]
----
<script setup>
...
ifeval::["{productSource}" != "zip"]
. Update `+App.vue+`, then import and add `+Editor+` component from `+@tinymce/tinymce-vue+` as below:
...
. Update the `+api-key+` prop in the Editor element with your link:{accountsignup}/[{cloudname} API key].
+
[source,jsx]
----

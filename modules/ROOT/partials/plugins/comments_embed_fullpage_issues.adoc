...
Comments can cause an issue if the link:fullpage.html[Full Page] plugin `+fullpage+` appears before Comments plugin `+tinycomments+` in the plugin list, and `+tinycomments+` is configured to use `+embedded mode+`.

...
include::partial$misc/requires_5_9v.adoc[] The `content_langs` option replaces the `spellchecker_select_languages` option. The value of the `spellchecker_select_languages` option, or its default, are used as a fallback for `content_langs`.
...
* Either the `code` property is one of the xref:plugins/premium/tinymcespellchecker.adoc#supportedlanguages[Supported Languages] or the `customCode` override property is defined.
* The `customCode` property is one of the xref:plugins/premium/tinymcespellchecker.adoc#supportedlanguages[Supported Languages] if it is defined.

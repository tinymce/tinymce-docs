...
* link:{baseurl}/plugins/opensource/emoticons/[Emoticons].
* link:{baseurl}/plugins/opensource/charmap/[Character Map].

Plugins relying on the link:{baseurl}/ui-components/autocompleter/[autocompleter API] or keypress events do not work on Android. Android intercepts content and processes content prior to insertion into the editor.
...
NOTE: This issue affects the link:{baseurl}/plugins/premium/comments/[Comments plugin].
...
For the comments plugin, the comments sidebar can be collapsed using the `showcomments` toolbar button (image:{baseurl}/images/icons/comment.svg[comment.svg]).

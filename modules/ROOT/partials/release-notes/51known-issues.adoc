...
Plugins relying on the xref:autocompleter.adoc[autocompleter API] or keypress events do not work on Android. Android intercepts content and processes content prior to insertion into the editor.
...
NOTE: This issue affects the xref:comments.adoc[Comments plugin].

...
The link:{{site.baseurl}}/plugins/opensource/table/[table plugin] has been updated to provide _touch selection handles_ on touch devices. The touch selection handles appear when users double-tap a table cell, and allow users to select a range of table cells on touch devices.
...
For the list of icons included in {{site.productname}}, see: link:{{site.baseurl}}/advanced/editor-icon-identifiers/[Available icons].

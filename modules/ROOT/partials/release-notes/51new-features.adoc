...
* Using the `silver` theme for mobile, see: xref:tinymce-for-mobile.adoc[{productname} Mobile].
* Using the legacy `mobile` theme, see: xref:tinymce-for-mobile.adoc#thelegacymobiletheme[{productname} Mobile - The legacy mobile theme].
...
For information on using `inputMode`, see: xref:dialog-components.adoc#inputmode[Dialog components - `inputMode`].
...
* xref:editor-appearance.adoc#toolbar_drawer[`toolbar_drawer`] - defaults to `false` on mobile devices. The toolbar will xref:side-scrollingtoolbarsonmobile[side-scroll by default].
...
The xref:table.adoc[table plugin] has been updated to provide _touch selection handles_ on touch devices. The touch selection handles appear when users double-tap a table cell, and allow users to select a range of table cells on touch devices.
...
The styling on xref:typesoftoolbarbuttons.adoc#splitbutton[split buttons] has been updated to include more padding so they are easier to interact with on touch devices.
...
For information on the Sticky Toolbars, see: xref:editor-appearance.adoc#toolbar_sticky[Enabling Sticky Toolbars].
...
For information on using the `referrer_policy` setting, see: xref:integration-and-setup.adoc#referrer_policy[Integration and setup options - `referrer_policy`].
...
For information on using the dark version of the default skin, see: xref:customize-ui.adoc#skins[Customizing the editor UI - Skins].
...
For information on the `toolbar_drawer` setting, see: xref:editor-appearance.adoc#toolbar_drawer[User interface options - `toolbar_drawer`].
...
For the list of icons included in {productname}, see: xref:editor-icon-identifiers.adoc[Available icons].

...
The link:{{site.baseurl}}/enterprise/tiny-comments/[Comments 2.0] is a premium plugin built on the link:{{site.baseurl}}/advanced/annotations/[Annotations API] to create comment threads (conversations). Comments 2.0 provides the ability to start or join a conversation by adding comments to the content within the TinyMCE editor.

Refer to the link:{{site.baseurl}}/plugins/premium/comments/[Comments 2.0] documentation, for more information.

=== Context menu

The link:{{site.url}}/docs-4x/plugins/contextmenu/[Context menu] is no longer a plugin, it is part of the core and always enabled. Where TinyMCE 4.x only supported adding registered menu items, the new context menu also allows plugins to register "sections" of the context menu. These sections are dynamic and may show or hide depending on the cursor position when the context menu is opened. For example, the default context menu config is now `'link image editimage table spelling'` which are all plugin section references, not menu items.

The context menu supports both individual menu items and dynamic context menu sections. Refer to the link:{{site.baseurl}}/ui-components/contextmenu/[context menu] documentation, for more information.
...
Refer to the link:{{site.baseurl}}/ui-components/contexttoolbar/[Context toolbar] documentation, for more information.
...
Refer to the link:{{site.baseurl}}/ui-components/customsidebar/[Custom sidebars] documentation for more information.

=== Dialog

The link:{{site.baseurl}}/ui-components/dialog/[Dialog] API allows showing dialogs (sometimes referred to as modals) in the user application. For example, the plugin that is used to view the source code is an example of a simple dialog that displays the HTML code from the content.
...
The link:{{site.baseurl}}/enterprise/formatpainter/[Format Painter] premium plugin allows copying and pasting formatting (such as font style and size) from one location to another. The Format Painter is capable of working with a wide variety of formats and styles such as inline & block formats and table styles.

The Format Painter may be used through keyboard shortcuts or a toolbar button. Refer to the link:{{site.baseurl}}/plugins/premium/formatpainter/[Format Painter] documentation, for more information.
...
The link:{{site.baseurl}}/enterprise/mentions/[Mentions] plugin makes it possible for users to send notifications using *@username* in the document.

Mentions is available for download through link:{{site.baseurl}}/cloud-deployment-guide/editor-and-features/[TinyMCE Cloud]. Include the mentions parameter in the tinymce.init script. Refer to the link:{{site.baseurl}}/plugins/premium/mentions/[Mentions] documentation, for more information on setup and configuration.
...
The link:{{site.baseurl}}/enterprise/pageembed/[Page Embed] premium plugin allows inserting content from another source, such as an advertisement, on a Web page.
...
Refer to the link:{{site.baseurl}}/plugins/premium/pageembed/[Page Embed] documentation, for more information.

=== Permanent Pen 1.0

The link:{{site.baseurl}}/enterprise/permanentpen/[Permanent Pen] is a premium plugin that allows adding comments or responses in emails or other documents, without having to change the text color or style.
...
Refer to the link:{{site.baseurl}}/plugins/premium/permanentpen/[Permanent Pen] documentation, for more information.
...
`Border` input field in the table properties dialog is now called `Border width` for clarity. Refer to the link:{{site.baseurl}}/plugins/opensource/table/[Tables] documentation, for more information. Tables are enriched with the following new additions:
...
Refer to the link:{{site.baseurl}}/ui-components/toolbarbuttons/[Toolbar buttons] documentation, for more information.

...
The link:{baseurl}/enterprise/tiny-comments/[Comments 2.0] is a premium plugin built on the link:{baseurl}/advanced/annotations/[Annotations API] to create comment threads (conversations). Comments 2.0 provides the ability to start or join a conversation by adding comments to the content within the TinyMCE editor.

Refer to the link:{baseurl}/plugins/premium/comments/[Comments 2.0] documentation, for more information.
...
The context menu supports both individual menu items and dynamic context menu sections. Refer to the link:{baseurl}/ui-components/contextmenu/[context menu] documentation, for more information.
...
Refer to the link:{baseurl}/ui-components/contexttoolbar/[Context toolbar] documentation, for more information.
...
Refer to the link:{baseurl}/ui-components/customsidebar/[Custom sidebars] documentation for more information.

=== Dialog

The link:{baseurl}/ui-components/dialog/[Dialog] API allows showing dialogs (sometimes referred to as modals) in the user application. For example, the plugin that is used to view the source code is an example of a simple dialog that displays the HTML code from the content.
...
The link:{baseurl}/enterprise/formatpainter/[Format Painter] premium plugin allows copying and pasting formatting (such as font style and size) from one location to another. The Format Painter is capable of working with a wide variety of formats and styles such as inline & block formats and table styles.

The Format Painter may be used through keyboard shortcuts or a toolbar button. Refer to the link:{baseurl}/plugins/premium/formatpainter/[Format Painter] documentation, for more information.
...
The link:{baseurl}/enterprise/mentions/[Mentions] plugin makes it possible for users to send notifications using *@username* in the document.

Mentions is available for download through link:{baseurl}/cloud-deployment-guide/editor-and-features/[TinyMCE Cloud]. Include the mentions parameter in the tinymce.init script. Refer to the link:{baseurl}/plugins/premium/mentions/[Mentions] documentation, for more information on setup and configuration.
...
The link:{baseurl}/enterprise/pageembed/[Page Embed] premium plugin allows inserting content from another source, such as an advertisement, on a Web page.
...
Refer to the link:{baseurl}/plugins/premium/pageembed/[Page Embed] documentation, for more information.

=== Permanent Pen 1.0

The link:{baseurl}/enterprise/permanentpen/[Permanent Pen] is a premium plugin that allows adding comments or responses in emails or other documents, without having to change the text color or style.
...
Refer to the link:{baseurl}/plugins/premium/permanentpen/[Permanent Pen] documentation, for more information.
...
`Border` input field in the table properties dialog is now called `Border width` for clarity. Refer to the link:{baseurl}/plugins/opensource/table/[Tables] documentation, for more information. Tables are enriched with the following new additions:
...
Refer to the link:{baseurl}/ui-components/toolbarbuttons/[Toolbar buttons] documentation, for more information.

...
NOTE: `editor_security` is set to `true` by default. That is, {productname} specifically sets `editor_security` option to `true`, even if this option is not declared in a {productname} configuration.

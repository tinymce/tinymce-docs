...
There are a few things you need to consider when using {cloudname}. If you wish to use any local plugins (like http://www.moxiemanager.com[MoxieManager] for example) you need to load them using the new link:{modulesDir}/configure/integration-and-setup/#external_plugins[`external_plugins`] option.

Also, if you wish to use a custom language, you need to enter the location of the language file into the new link:{modulesDir}/configure/localization/#language_url[`language_url`] option.

You may also want to select a specific version of the editor from {cloudname} and you should head on over to link:{modulesDir}/cloud-deployment-guide/editor-plugin-version[Specify editor & plugin versions] for details on that.

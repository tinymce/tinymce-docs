...
For information on:
* Using the `silver` theme for mobile, see: link:{modulesDir}/mobile/[{productname} Mobile].
* Using the legacy `mobile` theme, see: link:{modulesDir}/mobile/#thelegacymobiletheme[{productname} Mobile - The legacy mobile theme].
...
image::{modulesDir}/images/side-scrolling-context-toolbar.png[Example of the side-scrolling toolbar and contextual toolbar,380]
...
For information on using `inputMode`, see: link:{modulesDir}/ui-components/dialogcomponents/#inputmode[Dialog components - `inputMode`].
...
* link:{modulesDir}/configure/editor-appearance/#menubar[`menubar`] - defaults to `false` on mobile phones.
* link:{modulesDir}/configure/editor-appearance/#toolbar_drawer[`toolbar_drawer`] - defaults to `false` on mobile devices. The toolbar will <<sidescrollingtoolbarsonmobile,side-scroll by default>>.
* link:{modulesDir}/configure/editor-appearance/#toolbar_sticky[`toolbar_sticky`] - Sticky Toolbar is not supported on mobile devices and defaults to `false`.
* link:{modulesDir}/plugins/table/#table_grid[`table_grid`] - Table grid is not supported on mobile devices and defaults to `false`. When creating tables on mobile, a dialog is shown instead of the table grid.
* link:{modulesDir}/configure/editor-appearance/#resize[`resize`] - Resizing is not supported on mobile devices and defaults to `false`.
* link:{modulesDir}/configure/advanced-editing-behavior/#object_resizing[`object_resizing`] - Object resizing is not supported on mobile devices and defaults to `false`.

[#table-cell-selection-on-mobile]
==== Table cell selection on mobile

The link:{modulesDir}/plugins/table/[table plugin] has been updated to provide _touch selection handles_ on touch devices. The touch selection handles appear when users double-tap a table cell, and allow users to select a range of table cells on touch devices.

For example:

image::{modulesDir}/images/table_cell_touch_selector_handles.png[Touch selector handles for selecting multiple table cells,380]

[#touch-friendly-split-buttons]
==== Touch-friendly split buttons

The styling on link:{modulesDir}/ui-components/typesoftoolbarbuttons/#splitbutton[split buttons] has been updated to include more padding so they are easier to interact with on touch devices.
...
* link:{modulesDir}/general-configuration-guide/use-tinymce-inline/[Inline editing mode].
* link:{modulesDir}/general-configuration-guide/use-tinymce-distraction-free/[Distraction-free editing mode].
* link:{modulesDir}/configure/editor-appearance/#inline[`inline`].
* link:{modulesDir}/configure/editor-appearance/#toolbar_sticky[`toolbar_sticky`].
* link:{modulesDir}/plugins/table/#table_grid[`table_grid`].
* link:{modulesDir}/configure/editor-appearance/#resize[`resize`].
* link:{modulesDir}/configure/advanced-editing-behavior/#object_resizing[`object_resizing`].
...
image::{modulesDir}/images/sticky-toolbar.gif[Sticky Toolbar animation]

For information on the Sticky Toolbars, see: link:{modulesDir}/configure/editor-appearance/#toolbar_sticky[Enabling Sticky Toolbars].

[#changes-to-the-env-api-for-platform-detection]
=== Changes to the Env API for platform detection

New platform detection functions have been added to the link:{modulesDir}/api/tinymce/tinymce.env/[`Env` API], allowing for some older detection properties to be deprecated.
...
For information on using the `referrer_policy` setting, see: link:{modulesDir}/configure/integration-and-setup/#referrer_policy[Integration and setup options - `referrer_policy`].
...
For information on using the dark version of the default skin, see: link:{modulesDir}/general-configuration-guide/customize-ui/#skins[Customizing the editor UI - Skins].
...
image::{modulesDir}/images/border-width-cell-props.png[Cell Properties Dialog with new Border Width field,380]
...
For information on the `toolbar_drawer` setting, see: link:{modulesDir}/configure/editor-appearance/#toolbar_drawer[User interface options - `toolbar_drawer`].
...
For the list of icons included in {productname}, see: link:{modulesDir}/advanced/editor-icon-identifiers/[Editor icon identifiers].

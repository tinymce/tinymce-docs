...
*Callback mode* is the default mode for link:{modulesDir}/plugins/comments/[Comments 2.0]. In the callback mode, the user needs to configure storage to be able to save comments on the server. The Comments functions (create, reply, edit, delete comment, delete all conversations, and lookup) are configured differently depending upon the server-side storage configuration.
...
For more information on the Comments commercial feature, visit our link:{modulesDir}/enterprise/tiny-comments/[Premium Features] page.

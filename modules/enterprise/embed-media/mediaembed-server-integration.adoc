...
See the docs on link:{modulesDir}/enterprise/embed-media/mediaembed-server-config/#configureacustomendpoint[configuring a custom endpoint] for details on getting the Enhanced Media Embed server to utilise your custom API.

[#oembed-endpoint]
=== OEmbed endpoint

This is a popular choice and many CMSs have existing plugins that support oEmbed. While you can create custom HTML embeds this way, they cannot contain scripts. If the HTML contains a script, then the Enhanced Media Embed server will embed a link:{modulesDir}/enterprise/embed-media/mediaembed-server-config/#summarycards[summary card].
...
The other option for developing a custom API endpoint is to return JSON in the link:{modulesDir}/enterprise/embed-media/mediaembed-server-integration/#ephoxenhancedmediaembedformat[{productname} Enhanced Media Embed format].
...
* HTTP 200 (OK): link:{modulesDir}/enterprise/embed-media/mediaembed-server-integration/#ephoxembedobj[`EphoxEmbedObj`]
* HTTP 400 (User Error): link:{modulesDir}/enterprise/embed-media/mediaembed-server-integration/#errorobj[`ErrorObj`]
* HTTP 503 (Upstream Error): link:{modulesDir}/enterprise/embed-media/mediaembed-server-integration/#errorobj[`ErrorObj`]
* HTTP 500 (Unexpected Error): link:{modulesDir}/enterprise/embed-media/mediaembed-server-integration/#errorobj[`ErrorObj`]
...
* `links` (required)
 ** link:{modulesDir}/enterprise/embed-media/mediaembed-server-integration/#linksobj[LinksObj]
* `rel` (optional)
 ** link:{modulesDir}/enterprise/embed-media/mediaembed-server-integration/#relobj[RelObj]
* `media` (optional)
 ** link:{modulesDir}/enterprise/embed-media/mediaembed-server-integration/#mediaobj[MediaObj]
* `html` (optional)
...
* `primary` (required)
 ** Array of link:{modulesDir}/enterprise/embed-media/mediaembed-server-integration/#primaryrel[PrimaryRel]s
* `technical` (required)
 ** Array of link:{modulesDir}/enterprise/embed-media/mediaembed-server-integration/#technicalrel[TechnicalRel]s
* `source` (required)
 ** Array of link:{modulesDir}/enterprise/embed-media/mediaembed-server-integration/#sourcerel[SourceRel]s
...
* `players` (required)
 ** Array of link:{modulesDir}/enterprise/embed-media/mediaembed-server-integration/#linkobj[LinkObj]s
* `thumbnails` (required)
 ** Array of link:{modulesDir}/enterprise/embed-media/mediaembed-server-integration/#linkobj[LinkObj]s
* `apps` (required)
 ** Array of link:{modulesDir}/enterprise/embed-media/mediaembed-server-integration/#linkobj[LinkObj]s
 ...
 ** Integer
* `width` (required)
 ** link:{modulesDir}/enterprise/embed-media/mediaembed-server-integration/#dimensionboundobj[DimensionBoundObj]
* `height` (required)
 ** link:{modulesDir}/enterprise/embed-media/mediaembed-server-integration/#dimensionboundobj[DimensionBoundObj]

...
| link:{modulesDir}/enterprise/check-spelling/[Spellchecking]
| ephox-spelling.war
| Spell checking service for {productname} Enterprise.

| link:{modulesDir}/plugins/imagetools/[Image Tools Proxy]
| ephox-image-proxy.war
| Image proxy service for the Image Tools plugin.

| link:{modulesDir}/enterprise/embed-media/[Enhanced Media Embed], link:{modulesDir}/enterprise/check-links/[Link Checker]
...
[IMPORTANT]
====
*Memory requirement:* Please ensure that you configure your Java Server (Tomcat/Jetty etc) with a minimum of 4GB. Please refer to link:{modulesDir}/enterprise/server/troubleshoot/#outofmemoryerrors[Out of memory errors] section of the Troubleshoot page if you require instructions on how to explicitly define how much RAM will be allocated to your Java server.
====
...
Refer to the link:{modulesDir}/enterprise/server/configure/[Configure] page for the details and come back here when you're done.
...
After you've completed the steps on this page to link:{modulesDir}/enterprise/server/#step2deployserver-sidecomponents[Deploy server-side components], link:{modulesDir}/enterprise/server/#step3createaconfigurationfile[Create a configuration file] and link:{modulesDir}/enterprise/server/#step4passtheconfigurationfiletothejavaapplicationserver[Pass the configuration file to the Java application server], the application server may need to be restarted to pick up all your changes. Turn it off and on again now, just to be safe.

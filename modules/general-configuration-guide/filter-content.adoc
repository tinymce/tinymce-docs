...
Check out the link:{modulesDir}/demo/format-custom/[custom formats example] for a demonstration of this option.
...
| exact
| Disables the link:{modulesDir}/configure/editor-appearance/#style_formats_merge[Style merging] feature when used. `exact` is needed for some CSS inheritance issues such as text-decoration for underline/strikethrough.
...
The options above are examples of the type of configuration options in the link:{modulesDir}/configure/content-filtering/[Content filtering] configuration documentation.

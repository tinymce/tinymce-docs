...
include::{includeDir}/configuration/a11y_advanced_options.adoc[leveloffset=+1]
:includedSection: false
include::{includeDir}/configuration/file-picker-callback.adoc[leveloffset=+1]

include::{includeDir}/configuration/file-picker-types.adoc[leveloffset=+1]
...
Note that the `figure` element needs some custom CSS added to render properly. This is what we use in the internal `content.css` within {productname}, and can be overridden with your own custom link:{modulesDir}/configure/content-appearance/#content_css[`content_css`] stylesheet.
...
This option adds an "Upload" tab to the image dialog allowing you to upload local images, when the link:{modulesDir}/configure/file-image-upload/#images_upload_url[`images_upload_url`] is also configured.
...
include::{includeDir}/configuration/type-ahead-urls.adoc[leveloffset=+1]

...
The Image Tools plugin is provided with all subscriptions to link:{modulesDir}/cloud-deployment-guide/editor-and-features/[{cloudname}], including an automatically configured image proxy.
...
NOTE: `imagetools_cors_hosts` is *not* required when enabling this plugin via link:{modulesDir}/cloud-deployment-guide/editor-and-features/[{cloudname}].
...
link:{pricingpage}[{cloudname}] subscription also includes proxy service written in Java. Check link:{modulesDir}/enterprise/server/[Install Server-side Components] guide for details.

NOTE: `imagetools_proxy` is *not* required when enabling this plugin via link:{modulesDir}/cloud-deployment-guide/editor-and-features/[{cloudname}]

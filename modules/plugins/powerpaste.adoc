...
To enable the {productname} *PowerPaste* plugin with link:{modulesDir}/cloud-deployment-guide/editor-and-features/[{cloudname}]:
...
To enable the *PowerPaste* plugin when using a link:{modulesDir}/advanced/usage-with-module-loaders/[module loader], follow these instructions:

. Make the PowerPaste plugin folder available over http.
. Add the path to the PowerPaste plugin folder to the link:{modulesDir}/configure/integration-and-setup/#external_plugins[external plugins] option in `tinymce.init()`.
...
include::{includeDir}/configuration/paste-merge-formats.adoc[]
...
[NOTE]
====
If you configure *_PowerPaste_* to allow local images, you can have {productname} automatically upload Base64 encoded images for conversion back to a standard image as described on the link:{modulesDir}/advanced/handle-async-image-uploads/[image upload documentation].
====

include::{includeDir}/configuration/paste-block-drop.adoc[]
...
include::{includeDir}/configuration/smart-paste.adoc[]

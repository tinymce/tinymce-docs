...
NOTE: {productname} also supports link:{modulesDir}/ui-components/urldialog/[URL dialogs]. Configuring a URL dialog is quite different to configuring a standard {productname} dialog.
...
The link:{modulesDir}/plugins/code[`code`] plugin's dialog is an example of a simple dialog. It contains only one body panel component that displays the HTML code from the content.

image::{modulesDir}/images/sourcecode.png[Source Code]

[#display-complex-information]
==== Display complex information

More complex dialogs can use link:{modulesDir}/ui-components/dialogcomponents/#tabpanel[tab panels] and various link:{modulesDir}/ui-components/dialogcomponents/#layoutcomponents[layouts] to categorise and organise information displayed to the user. For example, the link:{modulesDir}/plugins/help[`help`] and link:{modulesDir}/plugins/charmap[`character map`] plugins use dialogs with tabbed panels to categorise and separate information.

image::{modulesDir}/images/specialchars.png[Special Characters]

[#interactive-dialogs]
==== Interactive dialogs

{productname} dialogs can also contain link:{modulesDir}/ui-components/dialogcomponents/#basiccomponents[interactive components] such as buttons, checkboxes and input fields.

For example, the link:{modulesDir}/plugins/image[`image`] plugin's dialog allows for users to link to or upload an image, input a description, title, class, caption, height and width, then insert it into the editor.

image::{modulesDir}/images/interactive.png[Insert/Edit images]
...
* *Body:* The body of the dialog. The body component can be a <<panel,panel>> or a <<tabpanel,tab panel>>, which can contain an array of link:{modulesDir}/ui-components/dialogcomponents/#panelcomponents[panel components] such as buttons, inputs and text.
...
| `(dialogApi, details)` \=> void
| optional
| *This method only applies to link:{modulesDir}/ui-components/dialogcomponents/#tabpanel[tab panel] dialogs.* Function invoked when the user changes tabs. `details` is an object that contains `newTabName` and `oldTabName`.
|===
...
The body of a dialog must be either a link:{modulesDir}/ui-components/dialogcomponents/#panel[`panel`] (a single panel) or a link:{modulesDir}/ui-components/dialogcomponents/#tabpanel[`tabpanel`] (a collection of panels). Each panel can contain link:{modulesDir}/ui-components/dialogcomponents/#panelcomponents[panel components] such as inputs, buttons and text.
...
The basic dialog type is a *panel* dialog. A panel is a container for link:{modulesDir}/ui-components/dialogcomponents/#panelcomponents[panel components]. A panel type dialog only has one panel.
...
A *tabpanel* dialog contains multiple panels, and a tab navigation menu on the left-hand side of the dialog to allow for switching between panels. Each panel can contain different link:{modulesDir}/ui-components/dialogcomponents/#panelcomponents[panel components], allowing for complex dialogs.

See the link:{modulesDir}/ui-components/dialogcomponents/#tabpanel[tab panel component reference] for tab panel configuration options.
...
image::{modulesDir}/images/help.png[Help Button]

[#footer-buttons]
=== Footer buttons

A *button* is a clickable component that can contain text or an icon. There are three types of buttons (primary, secondary and menu buttons). Primary and secondary buttons will perform an action when clicked, however they are styled differently. Primary buttons are intended to stand out. The color will depend on the chosen link:{modulesDir}/general-configuration-guide/customize-ui/#skins[skin]. Menu buttons will open a menu with more options when clicked, instead of performing an action.

NOTE: Dialog footer buttons are different to link:{modulesDir}/ui-components/dialogcomponents/#button[dialog panel buttons].
...
| *This method only applies to link:{modulesDir}/ui-components/dialogcomponents/#tabpanel[tab panel] dialogs.* Calling `showTab()` and passing the `name` of a tab will make the dialog switch to the named tab.
|===
...
This data store takes the form of a JavaScript object, where the object's keys are the ``name``s of the configured panel components. For example, if a dialog contains a link:{modulesDir}/ui-components/dialogcomponents/#checkbox[checkbox] component configured with `name: myCheckbox` and the checkbox is not checked then the dialog's data object will contain `{ myCheckbox: false }`.

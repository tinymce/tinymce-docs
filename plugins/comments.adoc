...
The Comments 2.0 plugin provides the ability to start or join a conversation by adding comments to the content within the {productname} editor. The Comments 2.0 plugin is built upon the link:{baseurl}/advanced/annotations/[Annotations API] and uses annotations to create comment threads (conversations).

Comments is a premium plugin from {companyname}. Please see the link:{baseurl}/enterprise/tiny-comments/[Premium features] section for all the buying options.
...
Refer to the link:{baseurl}/advanced/configuring-comments-callbacks/[configuring callbacks for comments] section for more information.
...
Check out the link:{baseurl}/demo/comments-2[Comments demo] to try this new feature.
...
Comments can cause an issue if the link:{baseurl}/plugins/fullpage/[Full Page] plugin `fullpage` appears before Comments 2.0 plugin `tinycomments` in the plugin list, and `tinycomments` is configured to use `embedded mode`.

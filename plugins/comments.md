...
Comments can cause an issue if the [Full Page]({{site.baseurl}}/plugins/fullpage/) plugin `fullpage` appears before Comments plugin `tinycomments` in the plugin list, and `tinycomments` is configured to use `embedded mode`.

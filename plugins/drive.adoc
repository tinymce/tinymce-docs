...
To enable this functionality, add `tinydrive` to the list of plugins in the `tinymce.init` call. You also need to authenticate the user using a link:{{site.baseurl}}/configure/jwt-authentication[JSON Web Token] (JWT).
...
You can read more about how to create these tokens in the link:{{site.baseurl}}/configure/jwt-authentication/[JWT authentication] guide.

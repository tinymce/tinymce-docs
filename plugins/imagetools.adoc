...
NOTE: `imagetools_cors_hosts` is *not* required when enabling this plugin via link:{baseurl}/cloud-deployment-guide/editor-and-features/[{cloudname}].
...
link:{pricingpage}[{cloudname}] subscription also includes proxy service written in Java. Check link:{baseurl}/enterprise/server/[Install Server-side Components] guide for details.

NOTE: `imagetools_proxy` is *not* required when enabling this plugin via link:{baseurl}/cloud-deployment-guide/editor-and-features/[{cloudname}]

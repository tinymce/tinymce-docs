...
| TableModified | `{ structure?: boolean, style?: boolean }` | Fired whenever style or structural changes are made to a table (included in the event). The optional `style` and `structure` parameters describe whether the table style, structure, or both were modified. |

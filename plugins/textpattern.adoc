...
When using list commands make sure that the link:{baseurl}/plugins/lists[lists plugin] is registered for normalized behavior across browsers.

NOTE: Formats and commands must be already registered with the editor. See the link:{baseurl}/configure/content-formatting/#formats[formats] and link:{baseurl}/api/tinymce/tinymce.editorcommands/[commands] documentation for more information.

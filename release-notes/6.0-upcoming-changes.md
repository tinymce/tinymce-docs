...
Open source {{site.productname}} is re gearing for future support of collaborative technologies and we want to shed some weight on plugins that could be improved greatly on our new collaborative platform.
  - In support of this Tiny wishes to focus on solving the harder editing problems and provide them to the community through our APIS to enhance the power and flexibility {{site.productname}} provides.
...
: The `spellchecker_whitelist` option was deprecated and replaced by the [`spellchecker_ignore_list`]({{site.baseurl}}/plugins/premium/tinymcespellchecker/#spellchecker_ignore_list) option in {{site.productname}} 5.7. The new option provides additional functionality and avoids cultural sensitive naming issues.
...
: The `force_hex_style_colors` option was deprecated in {{site.productname}} 5.0. This option allowed forcing color styles to use hexadecimal instead of RGB strings.
...
types
: The `types` option is a legacy initialization option from {{site.productname}} 3 that allowed specifying which type of elements should be initialized as an editor. It was deprecated and replaced by the [`selector`]({{site.baseurl}}/configure/integration-and-setup/#selector) or [`target`]({{site.baseurl}}/configure/integration-and-setup/#target) options in {{site.productname}} 4.

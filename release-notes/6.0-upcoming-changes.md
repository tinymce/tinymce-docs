...
: The [`convert_fonts_to_spans`]({{site.baseurl}}/configure/content-filtering/#convert_fonts_to_spans) option would convert `font` elements to `span` styles to assist with the migration to newer HTML standards. Font elements were include in HTML standards until HTML 3 and have since been deprecated or removed.
...
: Deprecated in 5.3. This option can be used for an unintended use-case and is not incompatible with Real-Time Collaboration.

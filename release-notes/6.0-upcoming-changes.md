...
`toolbar_mode`
: Will no-longer accept the `false` value in {{site.productname}} 6.0 for backwards compatibility with the `toolbar_drawer` option. Use `'wrap'` instead to keep the same functionality as `false`.

`forced_root_block`
: Will no-longer accept the `false` value or an empty string value in {{site.productname}} 6.0. This value prevented various editor functionality from working and caused non-semantic HTML to be generated.

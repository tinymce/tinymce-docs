...
TinyMCE 5.0.13 improves the nonbreaking space plugin by wrapping any nonbreaking space characters inserted by the plugin in a span to prevent the character being converted to a regular space while typing. To disable this behavior, add the new link:{{site.baseurl}}/plugins/nonbreaking/#nonbreaking_wrap[`nonbreaking_wrap: false`] setting to the TinyMCE init configuration.
...
For information on *Tiny Comments* refer to the full link:{{site.baseurl}}/plugins/comments/[documentation].

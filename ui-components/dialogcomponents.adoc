...
A *collection* is a layout component that creates a panel containing a collection of small buttons in the dialog body. For example, this component is used in the dialogs for the link:{{site.baseurl}}/plugins/charmap[`charmap`] and link:{{site.baseurl}}/plugins/emoticons[`emoticons`] plugins.

...
`onSetup` is used to listen to the editor's link:{baseurl}/advanced/events/#nodechange[`NodeChange` event] to disable the button when the cursor is inside a `time` element (or "node"). This ensures it is not possible to insert a `time` element into another `time` element.
...
= include::_includes/components/choice-menu-items.adoc[]
...
include::_includes/release-notes/onSetup.adoc[]
